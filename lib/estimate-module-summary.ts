...
export function buildEstimateModuleSummary(workspace: EstimateCostWorkspace, erpSummary: EstimateErpSummary | null): EstimateModuleSummary[] {
  ...
  for (const line of workspace.costItems) {
    ...
    if (line.supplierId) group.outsourcedAmount += lineAmount;
  ...
  for (const line of workspace.otherCostLines) {
    const moduleName = line.category;
    const sectionCode = estimateOtherCostSectionCode(line.category);
    const group = ensure({ key: `other:${line.category}`, moduleName, section: `${sectionCode} ${line.category}`, targetTab: "other", focusKey: null });
    group.disciplines.add("Project");
    group.costTypes.add(line.category);
    if (line.category === "Outsource") group.outsourcedAmount += amount(line.lineTotal); else group.inHouseAmount += amount(line.lineTotal);
    group.amount += amount(line.lineTotal);
    group.lineCount += 1;
    group.refs.push({ sourceType: "OtherCostLine", sourceId: line.id });
  }

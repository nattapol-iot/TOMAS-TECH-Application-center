...
export type SupportMeta={userId:number;categories:string[];manage:boolean;queue:boolean;categoryScope:string[];members:{id:number;name:string;category_code:string;can_award:boolean;notify_email:boolean}[];users:{id:number;name:string;canManage:boolean}[];maxFileBytes:number};

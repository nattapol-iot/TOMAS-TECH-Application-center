...
export default function ReportCustomerSign({ token }: { token: string }) {
  ...
  const evidenceImageSource = useCallback((attachmentId:number)=>`${baseUrl}/api/v1/report-acknowledgments/${encodeURIComponent(token)}/evidence/${attachmentId}`,[token]);
  ...
  return <main className="report-public" lang={report.locale} translate="no">
    ...
    <ReportBodyEditor locale={report.locale} reportType={report.reportType} body={report.body} readOnly evidenceImageSource={evidenceImageSource} />

...
function NewReportModal({ bootstrap, initialType = "SERVICE", initialTemplate = null, onClose, onCreated, onOpenExisting, onDirtyChange }: { onDirtyChange?: (dirty: boolean) => void; bootstrap: BootstrapData; initialType?: ReportType; initialTemplate?: ReportTemplate | null; onClose: () => void; onCreated: (report: ReportRecord) => void; onOpenExisting: (reportId: number) => void }) {
  const t = useUiText();
  ...
  const [existingDrafts, setExistingDrafts] = useState<{ id: number; number: string; title: string }[]>([]);
  ...
  // Creating a report saves it right away (see the submit handler below), so a
  // preparer restarting after picking the wrong source would otherwise pile up
  // empty DRAFT reports with no way to tell they already have one going. Check
  // for one of their own unsubmitted drafts on the same type + source and offer
  // to reopen it instead of quietly letting another duplicate get created.
  useEffect(() => {
    if (!sourceKind || !rawId) return;
    let cancelled = false;
    void apiRequest<{ items: { id: number; number: string; title: string }[] }>(`${BASE}/existing-draft?${new URLSearchParams({ reportType, sourceKind, sourceId: rawId })}`)
      .then(response => { if (!cancelled) setExistingDrafts(response.items); })
      .catch(() => { if (!cancelled) setExistingDrafts([]); });
    return () => { cancelled = true; };
  }, [reportType, sourceKind, rawId]);
  useEffect(() => {
    let cancelled = false;
    if (step !== 2) return;
    ...
    <form noValidate={step === 1} onSubmit={event => {
      ...
      <fieldset disabled={busy} className="report-entry-fieldset">
        ...
        <div hidden={step !== 2}>
          ...
          {existingDrafts.length ? <div className="callout info" role="status">{t("คุณมีฉบับร่างที่ยังไม่ส่งของงานนี้ค้างอยู่แล้ว:")}{existingDrafts.map(draft => <button key={draft.id} type="button" className="btn ghost sm" onClick={() => onOpenExisting(draft.id)}>{draft.number} · {draft.title}</button>)}<small>{t("ยังเลือกที่จะสร้างรายงานใหม่แยกต่างหากได้ ถ้าตั้งใจทำมากกว่าหนึ่งฉบับ")}</small></div> : null}

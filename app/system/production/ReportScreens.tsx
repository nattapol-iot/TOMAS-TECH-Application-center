...
const finalSections: Section[] = [
  { key: "evidence", label: "Evidence references", repeat: true, fields: [field("description", "Description"), field("reference", "File / document reference"), field("url", "Evidence URL", "url")] },
...
export function reportSections(type: string): Section[] {
  ...
  return [...commonSections, ...(specific[type] ?? []), ...finalSections.map(section => type === "UAT" && section.key === "issues" ? punchlist : section)];
}

export function ReportBodyEditor({ locale = "th", reportType, body, onChange, readOnly = false, reusableOnly = false }: { locale?: string; reportType: string; body: ReportBody; onChange?: (value: ReportBody) => void; readOnly?: boolean; reusableOnly?: boolean }) {
  ...
  return <ReportDocumentForm locale={locale} reportType={reportType} body={body} onChange={onChange} readOnly={readOnly} reusableOnly={reusableOnly} sections={sections} />;
}
...
export function ReportScreens({ bootstrap, notify, onOpenAnalytics, onDirtyChange }: { onDirtyChange?: (dirty: boolean) => void; bootstrap: BootstrapData; notify: (message: string) => void; onOpenAnalytics?: () => void }) {
  ...
  if (selectedId !== null) return <ReportDetail onDirtyChange={onDirtyChange} key={selectedId} id={selectedId} bootstrap={bootstrap} notify={notify} onBack={() => { setSelectedId(null); void load(); }} />;
  ...
  return <div className="report-workspace">
    ...
    <Tabs tabs={[{ id: "reports", label: t("รายงานทั้งหมด") }, { id: "templates", label: t("Template ของทีม") }]} active={workspaceTab} onChange={setWorkspaceTab} />
    {workspaceTab === "templates" ? <ReportTemplateLibrary onDirtyChange={onDirtyChange} bootstrap={bootstrap} notify={notify} onUse={template => { setNewType(template.reportType); setSelectedTemplate(template); setCreating(true); }} /> : <>
...
function ReportDetail({ id, bootstrap, notify, onBack, onDirtyChange }: { onDirtyChange?: (dirty: boolean) => void; id: number; bootstrap: BootstrapData; notify: (message: string) => void; onBack: () => void }) {
  ...
  return <div className="report-workspace">
    <button className="btn ghost" type="button" onClick={() => { if (!dirty || window.confirm(t("Discard unsaved report changes?"))) onBack(); }}>{t("← Back to reports")}</button>
    ...
    <article className="report-print"><ReportDocumentHeader report={report} title={report.title} reportDate={report.reportDate} />{report.template ? <p>{t("Template:")}{report.template.name}  · V{report.template.version}</p> : null}<ReportBodyEditor locale={report.locale} reportType={report.reportType} body={report.body} readOnly /><ReportSignatureSummary report={report} /></article>
  </div>;
}

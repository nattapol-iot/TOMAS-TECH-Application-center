...
function RemoveConfirmation({record,kind,onClose,onRemoved}:{record:CrmRecord;kind:"customer"|"contact";onClose:()=>void;onRemoved:()=>void|Promise<void>}) {
  const t=useT(),[busy,setBusy]=useState(false),[error,setError]=useState("");
  const primary=kind==="contact"&&record.isPrimary===true;
  const remove=async()=>{
    if(busy||primary)return;
    setBusy(true);setError("");
    const path=kind==="customer"?`/api/v1/crm/customers/${record.id}`:`/api/v1/sales/customers/${record.customerId}/contacts/${record.id}`;
    try{await apiRequest(`${path}?${new URLSearchParams({rowVersion:text(record.rowVersion)})}`,{method:"DELETE"});}
    catch(e){setError(e instanceof ApiClientError&&["customer_in_use","contact_in_use","primary_contact_protected","concurrency_conflict"].includes(e.code)?`CRM.${e.code}`:"CRM.error");setBusy(false);return;}
    onClose();await onRemoved();
  };
  return <Modal size="sm" title={t(kind==="customer"?"CRM.removeCustomer":"CRM.removeContact")} onClose={()=>{if(!busy)onClose();}}><div className="crm-editor"><strong>{text(record.name)}</strong><p>{t(kind==="customer"?"CRM.removeCustomerHint":"CRM.removeContactHint")}</p>{primary?<p role="alert" className="alert danger">{t("CRM.primary_contact_protected")}</p>:null}{error?<p role="alert" className="alert danger">{t(error)}</p>:null}<div className="crm-editor-actions"><button type="button" className="btn default" disabled={busy} onClick={onClose}>{t("Cancel")}</button><button type="button" className="btn danger" disabled={busy||primary} onClick={()=>void remove()}>{t("CRM.remove")}</button></div></div></Modal>;
}
...
function Records({rows,columns,onOpen,onEdit,onRemove,page,onPage,onPageSize}:{rows:CrmRecord[];columns:string[];onOpen?:(r:CrmRecord)=>void;onEdit?:(r:CrmRecord)=>void;onRemove?:(r:CrmRecord)=>void;page?:Page|null;onPage?:(v:number)=>void;onPageSize?:(v:number)=>void}) {
  const t=useT();if(!rows.length)return <EmptyState icon="inbox" title="CRM.empty" message="CRM.emptyHint"/>;return <div className="table-wrap">{onPageSize&&page?<TablePageSize value={page.pageSize} onChange={onPageSize}/>:null}<table><thead><tr>{columns.map(c=><th key={c}>{t(c==="name"?"CRM.recordName":c==="title"?"CRM.reportTitle":`CRM.${c}`)}</th>)}{onEdit||onRemove?<th>{t("Actions")}</th>:null}</tr></thead><tbody>{rows.map((r,i)=><tr key={text(r.id)||i}>{columns.map((c,n)=>{const value=text(r[c]);const tagged=["stage","status","contactRole"].includes(c);return <td key={c}>{n===0&&onOpen?<button className="crm-link" onClick={()=>onOpen(r)}>{value||"—"}</button>:c==="total"||c==="expectedValue"?money(r[c]):tagged&&value?<Badge tone={toneFor(value)}>{CRM_COPY[`CRM.${value}`]?t(`CRM.${value}`):value}</Badge>:CRM_COPY[`CRM.${value}`]?t(`CRM.${value}`):value||"—"}</td>;})}{onEdit||onRemove?<td>{onEdit?<button type="button" className="btn default sm" onClick={()=>onEdit(r)}>{t("Edit")}</button>:null}{onRemove?<button type="button" className="btn danger sm" onClick={()=>onRemove(r)} aria-label={`${t("CRM.remove")} ${text(r.name)}`}>{t("CRM.remove")}</button>:null}</td>:null}</tr>)}</tbody></table><PageBar page={page} onPage={onPage}/></div>;
}
...
function ContactList({bootstrap,customerId}:{bootstrap:BootstrapData;customerId?:number}) {
  const t=useT(),[search,setSearch]=useState(""),[page,setPage]=useState(1),[pageSize,setPageSize]=useState(10),[edit,setEdit]=useState<CrmRecord|null>(null),[remove,setRemove]=useState<CrmRecord|null>(null);
  ...
  const rows=state.data?.items??[];
  return <>{remove?<RemoveConfirmation record={remove} kind="contact" onClose={()=>setRemove(null)} onRemoved={()=>{setPage(p=>rows.length===1?Math.max(1,p-1):p);state.refresh();}}/>:null}<MetricStrip items={[{label:"CRM.totalContacts",value:state.data?.total??0,tone:"blue",icon:"users"},{label:"CRM.shown",value:rows.length,tone:"slate",icon:"eye"},{label:"CRM.withEmail",value:rows.filter(r=>r.email).length,tone:"green",icon:"send"},{label:"CRM.withRole",value:rows.filter(r=>r.contactRole).length,tone:"violet",icon:"shield"}]}/><div className="toolbar crm-filters"><SearchInput value={search} onChange={v=>{setSearch(v);setPage(1);}} placeholder="CRM.search"/><span className="spacer"/>{writable?<button className="btn primary" onClick={()=>setEdit({customerId:customerId??""})}><Icon name="plus"/>{t("CRM.newContact")}</button>:null}</div><Notice error={state.error} loading={state.loading}/><Panel flush title={`${state.data?.total??0} ${t("CRM Contacts")}`}><Records rows={rows} columns={["name","customerName","position","department","email","phone","contactRole"]} onOpen={writable?setEdit:undefined} onRemove={writable?setRemove:undefined} page={state.data} onPage={setPage} onPageSize={size=>{setPageSize(size);setPage(1);}}/></Panel>
    {edit?<Editor title={edit.id?"Edit":"CRM.newContact"} initial={{...edit,nameEn:edit.nameEn||(!edit.nameTh&&!edit.nameJa?edit.name:"")}} fields={[{key:"customerId",required:true,options:bootstrap.customers.map(c=>({value:String(c.id),label:c.name}))},{key:"nameTh"},{key:"nameEn"},{key:"nameJa"},{key:"email",type:"email"},{key:"phone"},{key:"department"},{key:"position"},...(edit.id?[{key:"isPrimary",type:"checkbox"}]:[])]} onClose={()=>setEdit(null)} onSave={async v=>{await crmRequest(`/api/v1/sales/customers/${v.customerId}/contacts${edit.id?`/${edit.id}`:""}`,edit.id?"PUT":"POST",{...v,siteId:v.siteId||undefined});state.refresh();}}>{(v,set)=><ContactSite customerId={Number(v.customerId)} value={v} set={set}/>}</Editor>:null}
  </>;
}

...
export function EstimateModuleQuantityCells({ name, quantity, unit, units, disabled, onSave }: {
  ...
  const save = async () => {
    ...
    if (!Number.isFinite(value) || value <= 0 || value > 1000000 || Math.abs(value * 10000 - Math.round(value * 10000)) > 0.00001 || !selectedUnit || selectedUnit.length > 30) {
      setError("ระบุจำนวนมากกว่า 0 (ทศนิยมไม่เกิน 4 ตำแหน่ง) และหน่วยไม่เกิน 30 ตัวอักษร"); return;
    }
  ...
  return <>
    <td className="num cb-inline-quantity"><input aria-label={"จำนวนโมดูล " + name} type="number" min="0.0001" max="1000000" step="0.0001" value={draftQuantity} disabled={disabled || saving} onChange={event => setDraftQuantity(event.target.value)} onKeyDown={onKeyDown} /></td>
    <td className="cb-inline-unit">
      {addingUnit ? <input ref={node => { node?.focus(); }} aria-label={"หน่วยใหม่ " + name} placeholder="ชื่อหน่วยใหม่" maxLength={30} value={draftUnit} disabled={disabled || saving} onChange={event => setDraftUnit(event.target.value)} onKeyDown={onKeyDown} /> : <select aria-label={"หน่วยโมดูล " + name} value={draftUnit} disabled={disabled || saving} onChange={event => {
      ...
      {Number(draftQuantity) > 0 && Number(draftQuantity) !== quantity ? <small className="muted">ต้นทุน ×{Number((Number(draftQuantity) / quantity).toFixed(4))}</small> : null}

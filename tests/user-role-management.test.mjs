...
test("role management migration grants Admin permission and only column-scoped user updates", async () => {
  const [migration, login, deploy, verifier, health] = await Promise.all([
    readFile(new URL("database/migrations/037_user_role_management.sql", root), "utf8"),
  ...
  assert.match(migration, /VALUES \(37,/);
  ...
  assert.match(deploy, /:r database\/migrations\/037_user_role_management\.sql/);
  assert.match(deploy, /35, 36, 37\)\) <> 37/);
  assert.match(verifier, /version = 37/);
  assert.match(verifier, /minor_id NOT IN/);
  assert.match(verifier, /missing its column-scoped user role update grant/);
  assert.match(health, /version BETWEEN 25 AND 37/);
  assert.match(health, /COUNT\(DISTINCT CASE WHEN version BETWEEN 25 AND 37 THEN version END\)=13/);
  assert.match(health, /REQUIRED_SCHEMA_VERSION = 37/);
  assert.match(health, /35, 36, 37/);
});

...
import { buildApp } from '../../backend-node/dist/src/app.js';
import { parseEstimateWorkbook } from '../../lib/estimate-excel-import.ts';
// Explicit isolated UAT database only. Never point this runner at Team Test.
const name=process.env.EXCEL_IMPORT_UAT_DATABASE ?? 'IoTTeamCenter_ExcelImport_UAT_20260906';
assert.match(name,/^IoTTeamCenter_ExcelImport_UAT_[A-Za-z0-9_]+$/);
...
try {
 ...
 const howa=parseEstimateWorkbook(JSON.parse(readFileSync(process.env.EXCEL_IMPORT_HOWA_FIXTURE ?? 'tmp/pj260022-parser-fixture.json','utf8')),'HOWA.xlsx');
 const toyo=parseEstimateWorkbook(JSON.parse(readFileSync(process.env.EXCEL_IMPORT_TOYO_FIXTURE ?? 'tmp/pj260035-parser-fixture.json','utf8')),'TOYO_R3.xlsx');

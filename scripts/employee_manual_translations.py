...
C('start', 'Getting started and permissions || 利用開始とアクセス権限', 'Sign-in → My Profile || ログイン → My Profile', 'All employees || 全従業員',
  P('1.1 Sign in for the first time || 1.1 初回ログイン', [
    ...
    'The source and documentation baseline is 19 September 2026; screenshots were captured in Team Test on 6 September 2026. Available functions depend on the installed release, permissions and project membership. || ソース・資料の基準日は2026年9月19日、画面画像は2026年9月6日にTeam Testで撮影しました。利用可能な機能は導入版、権限、プロジェクトへの所属によって異なります。') +
...
C('customers', 'Customers, contacts and end users || 顧客・連絡先・エンドユーザー', 'Inquiry → New inquiry / Master Data → Customers || Inquiry → New inquiry / Master Data → Customers', 'Sales / customer data owners || 営業・顧客マスター担当',
  ...
  P('5.4 Start tracking sales work in CRM || 5.4 CRMで営業案件の追跡を始める', [
    'Open <b>CRM &amp; Sales → Customers · 360°</b> and search by company name or code before creating a customer, to avoid duplicates. || <b>CRM &amp; Sales → Customers · 360°</b> を開き、新規作成前に会社名またはコードで検索して重複を防ぎます。',
    'If the customer is absent, use <b>New customer</b>, save the company, then add the correct Site and Contact. || 未登録なら <b>New customer</b> で会社を保存し、実際の拠点と担当者に合うSiteとContactを追加します。',
    'When a sales opportunity begins, open <b>Opportunities → New opportunity</b>. Select customer, name, Sales Owner and Stage; add Technical Owner, expected value and close date when known and permitted. || 商談が始まったら <b>Opportunities → New opportunity</b> で顧客、案件名、Sales Owner、Stageを選び、分かる範囲と権限に応じてTechnical Owner、見込金額、完了予定日を入力します。',
    'Open the saved opportunity and verify Customer, Stage, Owner, Customer need and Expected scope before the team relies on it. || 保存したOpportunityを開き、Customer、Stage、Owner、Customer need、Expected scopeを確認してから共有情報として使用します。',
    'Use <b>Pipeline</b> to review work by Stage and <b>CRM Dashboard</b> for open opportunities, follow-ups, overdue work, estimating and submitted proposals. || <b>Pipeline</b> でStage別に確認し、<b>CRM Dashboard</b> で進行中、要追跡、期限超過、見積中、Proposal提出済みを確認します。'
  ], 'Customer, Contact and Opportunity are linked, with one shared owner, stage and key information. || Customer、Contact、Opportunityが紐づき、担当・段階・主要情報をチームで共有できます。',
    'CRM visibility depends on scope and permissions; some roles cannot see commercial values. Clear filters and check access before creating a duplicate. || CRMの表示範囲は権限に依存し、一部役割では金額を閲覧できません。重複作成前にフィルター解除と権限確認を行います。') +
  P('5.5 Record activities, follow up and create an Inquiry || 5.5 活動記録・フォロー・Inquiryへの引継ぎ', [
    'After a meeting, call, email, site visit or new information, use <b>Activities → New activity</b>. Record type, time, owner, summary, participants, decisions and action items factually. || 会議、電話、メール、訪問、新情報の後は <b>Activities → New activity</b> で種類、日時、担当、要約、参加者、決定事項、Action itemsを事実どおり記録します。',
    'Create a <b>Next Action / Follow-up</b> in the opportunity with action, Owner, Due date, Priority and Status. Set <b>Done</b> only after completion. || Opportunityで作業内容、Owner、Due date、Priority、Statusを持つ <b>Next Action / Follow-up</b> を作成し、実完了後だけ <b>Done</b> にします。',
    'Start the day in <b>My Work</b> or CRM Dashboard, check overdue, due-soon and waiting-customer items, then update the source opportunity. || <b>My Work</b> またはCRM Dashboardから期限超過、期限間近、顧客待ちを確認し、元のOpportunityを更新します。',
    'Keep Stage aligned with facts, such as Requirement, Estimating, Proposal or Negotiation. || Requirement、Estimating、Proposal、Negotiationなど、実際の状況に合わせてStageを更新します。',
    'When ready for engineering, open the opportunity and select <b>Create Inquiry</b>. Verify Customer, Owner, Due date and Project type, then continue requirements, site visit and estimate from the linked Inquiry. || 技術引継ぎの準備後、Opportunityで <b>Create Inquiry</b> を選び、Customer、Owner、Due date、Project typeを確認して保存し、紐づくInquiryから要件・現地訪問・原価見積を進めます。'
  ], 'The latest activity is recorded, the next work has an owner and due date, and the Inquiry traces back to its opportunity. || 最新活動が記録され、次作業に担当と期限があり、Inquiryから元Opportunityを追跡できます。',
    'CRM tracks relationships and sales work; Inquiry captures technical intake and requirements. Creating an Inquiry does not approve an estimate or create a project automatically. || CRMは顧客関係と営業案件を追跡し、Inquiryは技術受付と要件を管理します。Inquiry作成だけで見積承認やProject作成は行われません。'))
...
C('reference', 'Glossary, states and handover checklist || 用語・状態・引継ぎチェック', 'Reference || 参考', 'All employees || 全従業員',
  ...
  N('Scope: current menus and user/subtab routes reviewed against source and documentation dated 19 September 2026. Screenshots captured on 6 September illustrate accessible Team Test screens; they do not certify every backend integration or approval workflow. Availability still depends on deployment and permissions. || 対象範囲は2026年9月19日のソース・資料で確認した現行メニューとユーザー・下位タブです。9月6日に撮影した実画面はアクセス可能なTeam Testの例であり、全連携・承認手順の動作保証ではありません。利用可否は導入環境と権限によります。'))

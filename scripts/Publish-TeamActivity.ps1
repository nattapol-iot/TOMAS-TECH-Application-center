...
$manifest = Get-Content -LiteralPath (Join-Path $stagePath 'activity-release.json') -Raw | ConvertFrom-Json
...
$migration = Join-Path $projectRoot 'database\migrations\035_team_activity.sql'
...
$releasePath = Join-Path $releaseBase ($stamp + '-team-activity')
...
if($LASTEXITCODE -ne 0) { throw 'Activity migration failed; runtime unchanged.' }
...
try {
 ...
 if($ready.status -ne 'ready' -or $ready.schemaVersion -lt 35) { throw 'Activity release readiness failed.' }

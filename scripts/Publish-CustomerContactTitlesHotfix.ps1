...
$releasePath = Join-Path $releaseBase ([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss', [Globalization.CultureInfo]::InvariantCulture) + '-customer-contact-titles')
...
try {
    ...
    if ($ready.status -ne 'ready' -or $ready.schemaVersion -ne 31) { throw 'Expected healthy schema 31 contact-title release.' }

...
# Validate every staged artifact against the reviewed manifest before stopping anything.
...
$releasePath = Join-Path $releaseBase ([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss') + '-customer-contact')
...
# No dependency change: retain the running release's exact installed packages.
Copy-Item -LiteralPath (Join-Path $baseline 'node_modules') -Destination $releasePath -Recurse
...
try {
    ...
    if ($ready.status -ne 'ready' -or $ready.schemaVersion -ne 28) { throw 'Expected healthy schema 28 baseline.' }

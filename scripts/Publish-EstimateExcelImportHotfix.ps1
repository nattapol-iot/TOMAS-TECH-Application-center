...
$releasePath = Join-Path $releaseBase ([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss', [Globalization.CultureInfo]::InvariantCulture) + '-estimate-excel-import')
...
try {
    ...
    if ($ready.status -ne 'ready' -or $ready.schemaVersion -lt 32) { throw 'Expected healthy schema 32 or newer Excel import release.' }

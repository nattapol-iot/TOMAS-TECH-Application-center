...
$releasePath = Join-Path $releaseBase ([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss') + '-customer-multilingual-names')
...
try {
    ...
    if ($ready.status -ne 'ready' -or $ready.schemaVersion -ne 30) { throw 'Expected healthy schema 30 customer-name release.' }

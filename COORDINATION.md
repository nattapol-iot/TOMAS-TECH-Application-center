...
| 2026-09-09 13:10 | Claude | Deep-diff of exported PPTX/PDF vs original found: (1) generatePptx() never emitted a Summary slide — added summarySlide(); (2) photo-placeholder boxes duplicated item/test name instead of showing "[No Photo]" — fixed in opPhotoSlide/elecPhotoSlide; (3) hardcoded "Inspection Report for MCP" replaced with editable `reportSubtitle` field since this form serves many equipment types/customers, not just MCP. Editing InspectionReportScreens.tsx and inspection-report-pptx.ts only. | released — typecheck clean, all generated slide XML validated well-formed; commit eb207b3 — Claude, 2026-09-09 13:10 |

| 2026-09-09 14:20 | Claude | Standardize Inspection Report for all departments (not just electrical): added per-unit `includePowerCheck` toggle that hides/skips PLC/HMI/Communication/Power Phase/Voltage/Breaker fields + the whole Power tab/slide/page when a unit doesn't need electrical measurement checks (defaults true, no behavior change unless unchecked); renamed "Electrical" tab to "Component Check". Also moved Inspection Report out of its own DOCUMENTS & SIGNING nav entry into ORGANISATION > Reports as a third tab (workspace/analytics/inspection), still gated on report.write. Edited InspectionReportScreens.tsx, inspection-report-pptx.ts, inspection-report.css, ReportScreens.tsx, ProductionApp.tsx. | released — typecheck + scoped lint clean (no new issues), verified via generated test PPTX with includePowerCheck=false (Power slide/fields correctly omitted, all slide XML valid); commit 89fbf17 — Claude, 2026-09-09 14:20 |

| 2026-09-09 15:00 | Claude | REPLACING the standalone Inspection Report system with the existing Reports/"INSPECTION" report-type engine (backend-persisted, template-reusable, real approval workflow) so Mechanical/Electrical/Software depts share one standard+dynamic form via ReportTemplateLibrary — see plan at C:\Users\TMT-E14-002\.claude\plans\polymorphic-crunching-porcupine.md. Deleting inspection-report-pptx.ts, InspectionReportScreens.tsx, inspection-report.css, app/ir-preview/*; enriching reportSections()/ReportDocumentHeader in ReportScreens.tsx + REPORT_TEMPLATE_SECTIONS_BY_TYPE/REPORT_TEMPLATE_FIELDS in backend-node/src/report-template-rules.ts; reverting the ProductionApp.tsx nav-merge from the previous entry (onOpenInspection/reportTab 3-way) since INSPECTION now lives natively in the report-creation flow. User confirmed (2026-09-09): discard localStorage test data, delete /ir-preview dev bypass. | released — typecheck + scoped lint clean across ReportScreens.tsx/ProductionApp.tsx/ReportDocumentForm.tsx/report-template-rules.ts, deleted files confirmed gone with zero remaining references repo-wide; commit 21a5703 — Claude, 2026-09-09 15:40 |

| 2026-09-09 16:30 | Claude | Added TOMAS TECH-branded PDF + PPTX export to the generic Reports engine (see plan at C:\Users\TMT-E14-002\.claude\plans\polymorphic-crunching-porcupine.md). Recovered the deleted inspection-report-pptx.ts via `git show 89fbf17:...` and adapted its generic OOXML helpers into new `report-pptx.ts` (generateReportPptx(), works for all 5 report types via Section[]/body, not just INSPECTION). Extracted `report-types.ts` (pure types/reportSections, no React/CSS deps) out of ReportScreens.tsx so the PPTX module doesn't depend on a "use client" file. Renamed inspection-report-template.ts -> report-pptx-template.ts. Added "Export PPT" button + real logo image in ReportDocumentHeader print masthead. Editing ReportScreens.tsx, report-workspace.css, new report-pptx.ts/report-types.ts, renamed report-pptx-template.ts only. | released — typecheck + scoped lint clean; verified via generated test PPTX for INSPECTION and SERVICE report types (cover colors 000000/FF0000 only, all slide XML well-formed); commit 9bba99f — Claude, 2026-09-09 16:30 |

| 2026-09-09 17:00 | Claude | User feedback: PDF/print output still didn't visually match the TOMAS TECH original document (only had a small logo bolted onto the generic masthead). Added print-only `ReportCoverPage` component in ReportScreens.tsx mirroring coverSlide() in report-pptx.ts exactly (red Rev./Confidential badges, right-aligned title block, logo, address footer), shown as page 1 via page-break-after in print CSS only (hidden on screen). Editing ReportScreens.tsx + report-workspace.css only. | released — typecheck + scoped lint clean; commit 34790e1 — Claude, 2026-09-09 17:00 |

| 2026-09-09 18:30 | Claude | Rebuilt Inspection Report as a dedicated content model + exporters, still inside ORGANISATION > Reports and still sharing the `unified_reports`/`unified_report_revisions` tables/workflow (customer/project linkage, submit/review/approve/customer-sign) — only the content editor and export format are dedicated, per explicit user request. Migration 037 adds ONE new table `unified_report_exports` (NAS-archived copy of every generated PDF/PPTX, append-only) — no other new tables; unit/measurement/photo content lives in the existing `body_json` column with a new fixed shape (`inspection-body-types.ts`) instead of the generic Section[] editor, and photos reuse the existing `unified_report_evidence_files` table/upload-download API (now supporting multiple photos per item, no more max=1 cap). New files: `inspection-body-types.ts`, `InspectionReportBody.tsx` (dedicated tabbed editor swapped in for `reportType==="INSPECTION"` only), `inspection-report-pptx.ts` (reuses report-pptx.ts's proven OOXML helpers, now exported), `inspection-report-pdf.ts` (NEW: pdf-lib-based exact-coordinate PDF generator replacing window.print() for Inspection reports specifically, sharing the same EMU-derived coordinates as the PPTX so both exports stay in lockstep). New backend routes on `unified-reports.ts`: POST/GET `/api/v1/reports/workspace/:id/exports` archiving generated bytes to NAS via existing `document-storage.ts` (`writeStoredFile`/`sendStoredFile` — no new NAS integration needed, reused as-is). Editing ReportScreens.tsx, api-client.ts, report-pptx.ts (added exports to existing helpers, no logic changes), unified-reports.ts, migration 037, inspection-report.css (recovered from git history + 3 new classes). | released — typecheck clean across frontend+backend (both tsconfigs), scoped lint clean, zero new errors; verified via generated test PPTX (16 slides, all valid XML, cover colors 000000/FF0000 only matching original) AND test PDF (16 pages via pdf-lib re-load, page size 540×780pt exactly matching the PPTX's EMU dimensions) from realistic 2-unit mixed-department test data (1 electrical unit with includePowerCheck, 1 mechanical unit without) — commit 025cd9e — Claude, 2026-09-09 18:30 |

| 2026-09-09 19:15 | Claude | User feedback: still not dynamic enough — the "dedicated" model still hardcoded electrical fields (R-S/R-T/S-T, transformer primary/secondary V/A, plcModel/hmiModel/voltage/etc, includePowerCheck as electrical-or-nothing toggle). Replaced with fully generic MeasurementSection{title,rows}/MeasurementRow{parameter,unit,specValue,actualValue,judgement,rank,remarks} and UnitAttribute{label,value} — any department names its own sections/parameters/attributes, no electrical-specific fields anywhere in the type/UI/export code. Removed includePowerCheck entirely. Editing inspection-body-types.ts, InspectionReportBody.tsx, inspection-report-pptx.ts, inspection-report-pdf.ts, inspection-report.css only. | released — typecheck+lint clean; verified with a 3-department test report (electrical/mechanical/software units, zero electrical field names leaking into non-electrical units, both PPTX 24 slides and PDF 24 pages generated correctly) — commit 5c9d38b — Claude, 2026-09-09 19:15 |

| 2026-09-09 19:20 | Claude | Investigated running migration 037 + live UI testing per user request. Found: (1) code already auto-deployed to production Mac mini via existing `deploy.yml` GitHub Actions workflow (confirmed via `gh run list` — success) but that workflow does NOT run DB migrations; (2) the real production DB is `IoTTeamCenterDev` on the team SQL Server (schemaVersion 36, documented in docs/MACMINI_HANDOFF.md), NOT the stale `IoTTeamCenterTeamTest` on localhost (schema v9, abandoned partial deployment per the same doc) that I could reach directly — user confirmed "database ต่ออยู่อีก server นึง"; (3) I have no SSH/terminal access to the Mac mini or the real DB server in this session, and no browser-automation tool to test the live authenticated site — flagged both to the user rather than guessing or acting on the wrong database. Migration 037 has NOT been applied anywhere by me. No files changed this entry. | blocked on user/ops action — needs someone with Mac mini access to run `~/iot-team-center/sqlcmd.sh -i /src/database/migrations/037_unified_report_exports.sql` — Claude, 2026-09-09 19:20 |

| 2026-09-09 17:30 | Claude | User feedback: print/export still mixed the new branded cover with the OLD generic masthead (ReportDocumentHeader) + signature summary appearing right after it. Replaced both, print-only: `ReportPrintInfoPage` (mirrors infoSlide()) and `ReportSignOffPage` (mirrors signOffSlide(), same real signatures/customerAcknowledgment data) — the live editing form's masthead/signature summary are untouched, only the print/export path changed. Also re-skinned printed table headers to the original's dark navy, and fixed a real bug in report-pptx.ts signOffSlide(): signature dates were looked up by actorId instead of stage (wrong if one person holds two roles). Editing ReportScreens.tsx, report-workspace.css, report-pptx.ts only. | released — typecheck + scoped lint clean; verified via regenerated test PPTX (sign-off dates now correctly distinct per stage, all slide XML valid); commit 6d356ef — Claude, 2026-09-09 17:30 |
...
| 2026-09-10 16:10 | Claude | COST-01 follow-up from the advisor artifact `.omx/artifacts/claude-you-are-taking-over-…-2026-09-10T15-51-34-279Z.md` finding 3: source-level guardrail so a future write path to a totals-feeding table cannot ship without `assertEstimateTotals`. Editing `tests/estimate-total-guardrails.test.mjs` only (additive test case); reads `backend-node/src/**` read-only. No source, migration, database, runtime, deployment or commit actions. | released — 3 additive test cases in the existing COST-01 file; negative controls confirmed the rule fires (a probe route writing dbo.cost_items unguarded, and a probe calling snapshotOverheadPolicy unguarded, both failed the suite by name; probe files removed). Root npm test PASS 228/228, 0 fail, 0 skip (lint + typecheck included). Backend typecheck PASS, 131 unit tests PASS, build PASS. The previously skipped root SQL integration case ran here: it created and dropped its own synthetic IoTTeamCenter_CI_* database on localhost; sys.databases shows no leftover CI database or iot_ci% login and no database created during the run. No source, migration, runtime, deployment, push or company-data change. — Claude, 2026-09-10 23:25 |
...
| --- | --- | --- | --- |
| 2026-09-08 | Claude-Template | `app/system/production/inspection-report-pptx.ts`, new `app/system/production/inspection-report-template.ts` — PPTX template extraction: replacing stub slide master/theme/layouts with real Tomas Tech template extracted from the provided PPTX. | released — full rewrite merged to main; commit 64892e8; typecheck clean — Claude, 2026-09-08 |
...
| 2026-09-14 | Claude | Same-day follow-up: user tested the new "Team" feature and got `Route POST:/api/v1/projects/41/members not found` — that error format is Fastify's, not ASP.NET Core's, which is how I found out `backend-node/` (not `backend/IoTTeamCenter.Api`) is the API this app actually calls in production (per `docs/MACMINI_HANDOFF.md`). Ported the same three endpoints into `backend-node/src/routes/projects.ts` (`demandProjectScope`, `project.write`/`project.read`, same core-member guard, same audit trail via `insertAudit`), and extended the guardrail in `production-guardrails.test.mjs` to check both backends. Only touched those two files. | released — `backend-node`'s own `npm run typecheck`/`npm test` (285/285) pass, root `npm run typecheck`/`lint` clean, guardrail passes — Claude, 2026-09-14 |

| 2026-09-14 | Claude | User-reported: creating a Report saves a real DRAFT row immediately (before any body content), and preparers had no way to discard a stray/mistaken draft other than an approver's 'void'. User confirmed the design: preparer-only 'discard' that marks the report VOID (soft, keeps audit trail — not hard delete), plus a check before creation that warns about the preparer's own existing DRAFT for the same type+source and offers to open it. Editing `backend-node/src/routes/unified-reports.ts` (new `discard` action + `GET .../existing-draft`), `backend-node/src/unified-report-service.ts` (allowedActions), `app/system/production/ReportScreens.tsx` (Discard draft button, existing-draft callout in `NewReportModal`), one new guardrail in `production-guardrails.test.mjs`. Did not touch the report state machine's other transitions, any migration, or PPTX/PDF export code. | released — `backend-node` typecheck/test (285/285) pass, root typecheck/lint clean, guardrail passes — Claude, 2026-09-14 |

| 2026-09-14 | Claude | Same-day follow-up: user asked for email notifications on Support/Report & Track Issues (new ticket, reply, assign, resolved/closed). Found `backend-node/src/email.ts` already has a working Microsoft Graph `sendMail` client (used today only for Estimate assignment) but `Email__*` was never wired into `docker-compose.dev.yml` at all — setting it in `.env` alone would have done nothing. Extended `supportEvent` in `support-service.ts` to return the exact same recipient set (name+email) the in-app `dbo.notifications` insert already computes, so email audience can never drift from the in-app one; refactored `email.ts` to share one `sendMail` helper and added `sendSupportTicketUpdate`; wired both into `support.ts` (ticket create, and comments/assign/transition — only Resolved/Closed status hops email, every other transition stays in-app-only) and threaded `email` through `registerSupportRoutes`'s signature (`app.ts` call site updated); added the missing `Email__*` passthrough to `docker-compose.dev.yml`. One new guardrail in `production-guardrails.test.mjs`. Real email still needs the human side prepared — Entra app registration with admin-consented `Mail.Send` application permission, a sender mailbox, and the resulting Tenant/Client/Secret in the macmini `.env` — none of that exists yet, so `Email__Mode` stays `Disabled` (silent no-op) until it's set up. Did not touch the .NET `backend/IoTTeamCenter.Api` (no email capability there) or `docker-compose.prod.yml` (different, apparently-secondary Linux/.NET deploy path). | released — `backend-node` typecheck/test (296/296) pass, root typecheck/lint clean, guardrail passes, `docker compose config` resolves the new vars — Claude, 2026-09-14 |

| 2026-09-15 | Claude | Same-day follow-up (2): first live test on macmini found every `support.manage` user (i.e. every Admin) was getting emailed for every unassigned ticket in every category — user asked for a config page to control this. Realized one already exists (`SupportMembers` panel in `SupportScreens.tsx`, backed by `POST /api/v1/support/members`) — the actual bug was `supportEvent`'s email-recipient query blanket-including `support.manage` the same way the in-app notification insert intentionally does. Narrowed the email query only (reporter/assignee/category `support_members` roster) and left the in-app insert's broader fallback untouched — no new table, column, or UI needed. Also fixed a stale log label ("Assignment email delivery failed" → "Email delivery failed", `app.ts`) found while walking the user through their first real deploy/log-check. Editing `backend-node/src/support-service.ts` and `backend-node/src/app.ts` only; updated the existing guardrail in `production-guardrails.test.mjs` to assert the narrower email query and the unchanged broader in-app one. | released — `backend-node` typecheck/test (304/304) pass, root typecheck/lint clean, guardrail passes — Claude, 2026-09-15 |

| 2026-09-15 | Claude | Same-day follow-up (3): user still wanted this configurable through UI specifically (per-member choice, not just membership on/off). Added `notify_email bit NOT NULL DEFAULT 1` to `dbo.support_members` (migration 051; appended to `REQUIRED_MIGRATIONS` in `migration-validation.ts` at the literal end of the array since `REQUIRED_SCHEMA_VERSION` reads `.at(-1)`, not the max — that array is already out of numeric order from earlier estimate-module entries). `POST /api/v1/support/members` now accepts `notifyEmail` (defaults true so existing members keep the pre-migration behavior); the email-recipient query in `support-service.ts` additionally requires `notify_email=1` (the in-app notification insert is untouched, no `notify_email` check there — a member can operate/award without wanting an inbox). Extended the existing `SupportMembers` panel: a "Receive email notifications" checkbox on add, and a per-row toggle in the members table that flips it without re-adding the member. New guardrail in `production-guardrails.test.mjs`. Migration itself has not been applied anywhere yet — the macmini's own migration runner applies it automatically on the next deploy (confirmed this is how 049/050 landed too, per the "[migrate] All migrations already applied." log line seen this session). | released — `backend-node` typecheck/test pass, root typecheck/lint clean, guardrail passes — Claude, 2026-09-15 |
...
| 2026-09-10 | Claude | Three fixes: (1) "window is not defined" SSR crash in supplier-quotation-pdf-parser.ts — removed top-level static pdfjs import, all pdfjs-dist now lazy-loaded inside the async function with `typeof window` guard. (2) PDF parsing quality improvement — replaced simple space-join with Y-position grouping (reconstructLines()) to rebuild table rows from positioned text items; added full Thai month names, ISO date, broader patterns. (3) Auto-migration on deploy — new backend-node/src/migrate.ts reads SQL files from /app/migrations/, checks schema_versions, runs pending migrations splitting on GO; server.ts calls it before HTTP listeners start; Dockerfile now builds with repo-root context and COPYs database/migrations/; docker-compose.dev.yml updated to context:. + dockerfile:backend-node/Dockerfile; health.ts bumped to REQUIRED_SCHEMA_VERSION=38. Commit 8119a76. | released — typecheck clean (no errors in changed files); pushed — Claude, 2026-09-10 |
— Codex Performance Pulse ACTIVE 2026-09-07: user authorized implementation of DESIGN.md employee encouragement signals. Backend lane owns structured deterministic insights in performance evidence and focused unit tests; root owns API client DTO, employee-only My KPI cards, TH/EN/JP copy, responsive CSS, frontend/guardrail tests, docs and coordinated managed release. No database/schema/migration or external AI. Preserve Team Activity, Support, customer, Reports, OCR and all unrelated source/runtime changes. No service restart until validation and a RELEASED entry.

— Codex Performance Pulse RELEASED 2026-09-07: employee-only My KPI Pulse now shows up to three factual strength/attention-context/next-step cards for Engineering and Sales, with TH/EN/JP copy, source drill-down, responsive one/two/three-column layout and LOW-data neutral fallback. Node evidence owns deterministic reason codes, facts, thresholds, priority, deduplication and Issue-attribution safeguards; no KPI/Activity score, review text, database or schema mutation. Focused frontend20/20, backend97/97, both typechecks, scoped lint and production builds pass. Full root suite207/211; four unrelated stale schema35/menu/inventory expectations fail because committed deployment now includes schema36. API release20260907-102449 PID51284 and frontend PID2744 live at http://192.168.1.125:3000; live bundle PerformanceScreen-BC6xlbBA.js and authenticated Engineering insights verified. Test-KpiWorkEvidenceLive SQL admin discovery remains blocked by remote Integrated-auth untrusted-domain; equivalent signed Team Test API read passed without data writes. Runtime intentionally remains available; source/runtime claim released.


### Documentation recovery audit — Codex, 2026-09-10

Claim: released — README.md, DESIGN.md governance additions, docs Markdown organization and new audit/register/plan documents, .omx/plans/production-recovery-2026-09-10.md. Preserved implementation changes and historical coordination entries. Independent code-reviewer and architect were read-only. No shared runtime, database, deployment, or business-data changes. Current navigation: docs/README.md. Historical claims are snapshots, not live ownership proof.

— Codex, 2026-09-10: documentation recovery complete. Canonical topical documents moved into docs/features, docs/design, docs/releases, docs/audits and docs/team; original paths remain forwarding pointers. docs/README.md is the current document index; docs/planning/PRODUCTION_BACKLOG.md contains 39 tasks across all 22 flow groups. Preserved 38 document bodies; 240 local file links across 103 Markdown files resolve; 14 DESIGN sections retained. Independent code review REQUEST CHANGES; architecture BLOCK. See docs/audits/production-readiness-2026-09-10.md for duplicate migration037, candidate/deploy divergence, readiness040 and workbook provenance findings. Node typecheck +110 unit pass; root211 pass/10 fail/1 SQL skip, lint incomplete and root typecheck29 diagnostics. Documentation completion is not Production approval. No application feature changes or deployment in this task.


Release candidate integration - Codex, 2026-09-10: ACTIVE. User authorized main-based integration. Own isolated worktree C:/Work/IoT-release-candidate-20260910 and branch codex/production-candidate-20260910. Preserve source dirty worktree and index. No runtime/deploy/database changes. Snapshot selected application/docs work through a temporary Git index; reconcile main features, migration identity, tests and release evidence.


Candidate ownership - Codex, 2026-09-10: Lead owns docs, package/tooling/CI and integration. candidate_frontend owns app/lib + auth/contact/rate tests. candidate_backend owns API feature reconciliation, handover/admin/import/overhead tests. candidate_database owns database, migrate/readiness + migration guardrail tests. All work is isolated here; no deployment, service restart, or real database writes.

Candidate integration - Codex, 2026-09-10: RELEASED local source ownership. Branch codex/production-candidate-20260910 in C:/Work/IoT-release-candidate-20260910 combines main d43eb346 with preserved source 672f1d0b. Frontend full lint/type/unit PASS (223 pass, 1 explicit SQL skip); backend type/unit/build PASS (129 pass); frontend isolated production build PASS with synthetic nondeployable config. Independent code review approves local integration; architecture WATCH; Production remains blocked by aggregate overhead overflow, non-skipped SQL/staging UAT, target schema and recovery/deployment gates. See docs/planning/CANDIDATE_INTEGRATION.md. No main push, service restart, deployment or real database writes. This entry closes only this local integration claim; older runtime claims are historical.

Codex, 2026-09-10: ACTIVE COST-01 aggregate overflow hardening on candidate1c135def. Lead owns cross-lane contract, docs and final verification; candidate_backend maps/guards all estimate write paths and tests; candidate_database owns forward migration042, migration readiness and SQL acceptance fixtures. No edits to migration040, real records, deployed runtime or services. New SQL execution requires an isolated synthetic target.

Codex, 2026-09-10: RELEASED COST-01 local aggregate hardening. Forward042 + shared transaction guard covers all mapped monetary/lifecycle/handover writers; migration040 preserved. Root npmtest PASS224/1 explicit SQLskip, backend131PASS/type/build, private SQL17 LocalDB upgrade/repeat/fresh/boundary/role/preflight rollback PASS. All owned test DBs/instances cleaned; existing MSSQLLocalDB preserved. Code review25files APPROVE, architecture29files CLEAR locally/WATCH production. See docs/planning/COST_TOTAL_GUARD.md for evidence and remaining NodeSQLlogin/SQL2022/staging/preflight-duration/UAT/recovery gates. No production data, deployed service, main push or deployment changes.

Codex, 2026-09-10: ACTIVE local-only application development on branch local/app-development-20260910. User replaced the PR/Production plan with workstation-only updates using SQL endpoint 202.151.188.68. Owns explicit no-migration/read-only API mode, local launcher/docs and focused tests. Remote identity was read-only verified as IoTTeamCenterTeamTest schema39; no migration, data write, service restart, push or deployment is authorized.

Codex, 2026-09-11: RELEASED local-only runtime foundation. API binds 127.0.0.1:5116, skips migrations, blocks mutating HTTP methods, transactions, raw sessions and classified mutating SQL; Knowledge article reads suppress their view-count update. Frontend binds 127.0.0.1:3010 and shows Local read-only/schema39. Launcher pins 202.151.188.68/IoTTeamCenterTeamTest, verifies contiguous versions1-39 plus exact identities25-39, and requires an explicit warned untrusted-certificate exception. Smoke: frontend200, live200, readiness503 schema39/required42, write403 local_read_only; remote versions remained count39/min1/max39. Root227PASS/1 explicit SQLskip, backend137PASS/type/build, frontend type/lint/build PASS. Code and architecture re-review WATCH: suitable for documented limited local read surface; dedicated SELECT-only SQL principal/trusted certificate and schema capability handling remain future improvements. No push, PR, deployment, shared service restart or database migration performed.

Codex, 2026-09-11: FIXED local read-only dashboard compatibility. The SQL classifier now permits INSERT only into connection-scoped table variables or temporary tables used to assemble reporting read models; INSERT into persistent tables and all other classified mutations remain blocked. Browser verification against the user's local runtime loaded Management Dashboard data and all 57 Inquiry records. Backend137PASS/type/build and root227PASS/1 explicit SQLskip. No database write, migration, push or deployment performed.

Codex, 2026-09-11: ACTIVE local Team Test writable mode authorized by the user for 202.151.188.68/IoTTeamCenterTeamTest. Both local launchers now require the explicit per-run `-AllowRemoteWrites` switch; omission remains read-only. Automatic migrations stay disabled because the database is schema39 while the candidate requires42; email remains disabled and document uploads remain workstation-local. Live API mutation routing reached validation instead of the read-only403 guard, and a permitted Inquiry UPDATE inside a SQL transaction succeeded then rolled back with the row version unchanged. UI reloaded without the read-only banner. No test business record, migration, push or deployment performed.

Codex, 2026-09-11: READY external Claude lane for Team Test schema39→42 preparation. Claude owns only seven new tooling/test/runbook files listed in docs/team/CLAUDE_SCHEMA_39_TO_42_HANDOFF.md and may validate only on a disposable private LocalDB instance. Claude must not edit existing migrations/backend/runtime launchers/COORDINATION/dist or connect to, back up, migrate, grant on, restart for, or mutate 202.151.188.68. Codex/operator retains remote preflight, DBA/backup, apply, restart and final verification ownership.

Claude, 2026-09-11 15:05: RELEASED bounded user-authorized Estimate/ERP QA per `.omx/context/claude-estimate-erp-qa.md` (supersedes the schema 39→42 handoff scope for this lane). Owned only new files: `docs/audits/estimate-erp-qa-20260911.md` and `.omx/artifacts/estimate-erp-qa/`. No product code, launcher, migration, COORDINATION claim table row, dist, service restart, SQL execution, commit or push. Health verified 200/ready schema43 on 127.0.0.1:5116 and frontend 200 on 127.0.0.1:3010. Sign-in is BLOCKED: no Team Test credential was supplied and the signing key was not read, so no `TEST-ERP-QA-20260911` record was created and no business data was touched. Unauthenticated guard checks, ERP mapping/reconciliation/export-guard logic (27/27 probe checks) and a real generated `.xlsx` compared against the Okaya x SSSC reference were completed instead. Four export-format defects reported, D1 (print setup does not fit the page, `fitToWidth` inert without `pageSetUpPr fitToPage`) is the one worth fixing before anyone prints an export. The reference file's 30% Profit row remains deliberately absent from the app export. Scenarios 2, 3b and 3f need an operator-issued disposable QA access code to complete.
Claude, 2026-09-11 14:05: ACTIVE estimate copy + assignment discovery lane (EC-08/EC-09/EC-10). Isolated worktree C:\Work\IoT-estimate-copy-20260911 on branch claude/estimate-copy-assignment-20260911, branched from 1b6de4be. Owns new files backend-node/src/routes/estimate-copy.ts, backend-node/src/routes/estimate-assignments-read.ts, lib/estimate-assignment-queue.ts, docs/planning/ESTIMATE_COPY_ASSIGNMENT_FIXES.md and tests/estimate-copy-contract.test.mjs, tests/estimate-assignment-queue.test.mjs, tests/estimate-assignment-visibility.test.mjs. Narrow additive edits to backend-node/src/app.ts (2 registration lines), app/system/api-client.ts, app/system/production/EstimateScreens.tsx (Copy Previous Estimate modal only), app/system/production/PlanningPricingScreens.tsx (ProductionMyWork only) and app/system/ProductionApp.tsx (one prop). No migration (043 is current, 044 stays reserved for the labor lane). No labor-master API or table change; engineering_rates is read through the existing rate rule only. No remote SQL mutation, no service restart, no push.

Claude, 2026-09-11 16:40: RELEASED estimate copy + assignment discovery lane (EC-08/EC-09/EC-10). EC-08 root causes: Copy Previous Estimate wrote one cost line per HTTP request with no transaction, so any failure left a partial copy; it never allocated a free item code against UX_cost_items_code; it copied cost items only, not man-hour/expense/other-cost/ERP; and each line took the elevated branch of upsertCategoryAssignment, silently reassigning sections owned by other engineers and forcing them to In Progress. A free-standing duplicate estimate stays impossible because dbo.estimates.inquiry_id is UNIQUE, so the new Draft still comes from the existing create-from-inquiry action. Fixed behind the same Copy button by one transactional POST /api/v1/estimates/:id/copy-from that copies all four ledgers plus ERP classifications, allocates item codes, keeps price provenance verbatim, re-rates internal man-hour from dbo.engineering_rates at today's rate, never writes to the source and never rewrites an existing assignment. EC-09/EC-10 root cause: nothing queried "estimate sections assigned to me" — GET /api/v1/estimates filters the estimate owner and My Work reads only schedule/resource tasks — so an assignment surfaced only as a side effect of work starting. Added GET /api/v1/me/estimate-assignments scoped to owner_id/support_id with completed/archived filtering on the current revision, and a My Work panel with per-state next-action guidance and a direct Open Estimate action. No migration; 044 remains reserved for the labor lane; no labor-master API or table change. Evidence: root npm test 254 pass/0 fail (lint + typecheck included), frontend build:local PASS in this separate checkout, backend typecheck/build PASS, backend npm test 170 pass/0 fail, including 13 new copy-route tests covering rollback, source-unchanged, item-code allocation, non-destructive assignment, authorization isolation and locked-target refusal. Not verified: no live authenticated UI or SQL Server execution — no Team Test credential was minted and no remote database was touched. Design and evidence: docs/planning/ESTIMATE_COPY_ASSIGNMENT_FIXES.md. Work is committed on claude/estimate-copy-assignment-20260911 in C:\Work\IoT-estimate-copy-20260911; no push, no merge, no service restart.
Claude, 2026-09-11 15:50: RELEASED bounded labor lane EC-01/02/03 per `.omx/context/claude-labor-master-next.md`. Implemented in a NEW isolated worktree `C:\Work\IoT-labor-master-20260911` on branch `claude/estimate-labor-master-20260911`, based on candidate HEAD 1b6de4be, with its own `npm ci` dependencies and its own `dist/`. The candidate worktree's `dist/` is unchanged and the user's runtime at 127.0.0.1:3010 / 127.0.0.1:5116 stayed up; nothing was restarted, pushed, merged or deployed, and no branch was moved.

Owned files: new `docs/planning/ESTIMATE_COST_IMPROVEMENT_TASKS.md` (EC-01..07 backlog), new `database/migrations/044_estimate_labor_masters.sql`, new `database/tests/estimate-labor-master-044.sql`, new `backend-node/src/labor-master.ts`, new `backend-node/src/routes/labor-rates.ts`, new `backend-node/src/routes/labor-packages.ts`, new `backend-node/tests/labor-master.test.ts` and `labor-master-routes.test.ts`, new `lib/labor-master.ts`, new `app/system/production/LaborPackagePicker.tsx`, new `tests/labor-master-ui.test.mjs`, new `docs/audits/estimate-labor-master-20260911.md`. Narrow additive edits only in shared files: `backend-node/src/app.ts` (+4 lines registering two route modules), `backend-node/src/routes/estimate-workspace-write.ts` (added `export` to five existing helpers and extracted `resolveInternalDailyRate` out of `resolveRate` with no behaviour change, so a package applies the same live rate a hand-typed line gets), `app/system/api-client.ts` (appended labor types and functions) and `app/system/production/EstimateScreens.tsx` (+10/-3: one import, one prop, two buttons, two modals inside the man-hour tab). No estimate screen redesign, no cross-department permission change, no summary category visibility change, no Excel export fix — those are recorded as EC-04/05/06 with acceptance criteria.

Migration 044 is RESERVED, not registered: `REQUIRED_MIGRATIONS` and `REQUIRED_SCHEMA_VERSION` stay at 43, so the local launchers, readiness endpoint and their tests are untouched. Registering 044 and applying it to Team Test is EC-07 and belongs to the database/leader lane. Until then the labor-package endpoints answer `503 labor_packages_unavailable` and the rate picker reports `masterFieldsAvailable: false` instead of failing.

Design decision worth knowing before touching rates: a rate master is superseded, never edited in place. Editing the amounts on an incumbent row leaves every man-hour line already priced from it holding a `daily_rate` that matches no active master rate at the line's `price_date`, so `fn_estimate_validation` raises `internal_rate_mismatch` and `POST /api/v1/estimates/:id/submit` starts failing 422 on estimates nobody touched. Supersede closes the incumbent the day before the successor starts, leaving it active over its own history. `is_active = 0` is deliberately not exposed for the same reason.

Verification: migrations 001→043 deployed fresh plus 044 on a DISPOSABLE LocalDB instance `LaborMaster0911`, then deleted — `MSSQLLocalDB` and the existing `IoTCostCI_*` / `IoTUpg3942CI_*` instances were preserved, and 202.151.188.68 was never contacted. `database/tests/estimate-labor-master-044.sql` PASS: superseding a rate left a saved line at 5000.0000 with cost 12500.0000 and the estimate engineering total at 12500.0000, the saved line still passed `fn_estimate_validation`, 2026-02-10 still resolved 5000 while 2026-03-15 resolved 5600, ERP seeding did not overwrite an existing mapping, and 14 deliberate bad writes were all rejected including the overlap trigger 51020. Re-running 044 is refused with 51441 and the guarded DDL re-applies cleanly after an interrupted attempt. Backend typecheck/build pass and 184/184 backend tests pass (candidate baseline 145). Frontend lint/typecheck pass, root suite 251/251 (candidate baseline 237), isolated `vinext build` passed. `tests/estimate-total-guardrails.test.mjs` initially failed on the new route and was obeyed rather than amended: `apply-labor-package` now calls `assertEstimateTotals` itself.

Honest gaps: no authenticated browser run — no Team Test credential was supplied, so the endpoints were exercised through route-level tests over a captured SQL layer plus the SQL fixture, not a signed-in session. Package apply provenance lives in `audit_log` because `dbo.manhour_lines` has no reference column and adding one would also need the revision-clone column list in `routes/estimates.ts`, which is another lane's file. The package library is reachable from the Estimate man-hour tab only, to avoid editing `ProductionApp.tsx`. No i18n dictionary keys were added; the new UI renders English through the `LocalizedText` fallback. Note that this file in my worktree is the committed HEAD version, so it does not contain the candidate worktree's uncommitted COORDINATION edits; the leader owns that reconciliation. Commit is local only. — Claude, 2026-09-11 15:50

Codex, 2026-09-11: ACTIVE Labor Package Master UI. Own ProductionApp.tsx, EstimateScreens.tsx, LaborPackagePicker.tsx, new LaborPackageMaster.tsx and focused tests. Add Draft/Active management, edit/publish with existing permissions and rowVersion, and saved-record navigation. Backend and database unchanged.

Codex, 2026-09-11: RELEASED Labor Package Master UI. Sidebar + Estimate modal + saved-record confirmation integrated. Root279/279, labor backend39/39, lint/typecheck passed. Browser read-only verified actual LP-SOFTWARE-DEVELOPMENT Draft and filters. No live package edit/publication. docs/labor-package-master.md records verification.

...
async function cloneRevisionLines(transaction, estimateId, currentRevision, nextRevision, actorId) {
    ...
    INSERT INTO dbo.manhour_lines(estimate_id,revision,package,activity,department,level,cost_type,provider,supplier_id,quotation_no,price_date,engineers,man_days,hours_per_day,daily_rate,owner_id,remark,created_by,updated_by)
    SELECT estimate_id,@next_revision,package,activity,department,level,cost_type,provider,supplier_id,quotation_no,price_date,engineers,man_days,hours_per_day,daily_rate,owner_id,remark,@actor,@actor FROM dbo.manhour_lines WITH(HOLDLOCK) WHERE estimate_id=@estimate_id AND revision=@current_revision AND deleted_at IS NULL;

...
-- in the same call; DDL and first references to the new column compile separately.
BEGIN TRY
BEGIN TRANSACTION;
...
EXEC sys.sp_executesql N'-- Old summary quantities were presentation-only. Adopt them only for editable,
...
END;';
EXEC sys.sp_executesql N'CREATE OR ALTER VIEW dbo.v_estimate_cost_amounts
AS
...
EXEC sys.sp_executesql N'CREATE OR ALTER VIEW dbo.v_estimate_totals
AS
WITH material AS (

...
SET NOCOUNT ON;
BEGIN TRANSACTION;
-- One request balances BEGIN/COMMIT for the Node migration runner.
EXEC(N'-- Only the owner-executed purge procedure may remove immutable history for one
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_cost_items_current_revision_only
ON dbo.cost_items
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_manhour_lines_current_revision_only
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_expense_lines_current_revision_only
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_other_cost_lines_current_revision_only
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_estimate_overhead_snapshots_immutable
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_estimate_submission_snapshots_immutable
...
EXEC(N'CREATE OR ALTER TRIGGER dbo.trg_estimate_erp_mappings_current_revision_only
...
EXEC(N'CREATE OR ALTER PROCEDURE dbo.purge_trial_document

...
### Module text rows and Summary Remark (schema 047)

`EstimateModuleEditor` edits title + descriptionRows for modules; key `summary` edits only the overall Remark. `estimate_module_details.description_rows` stores JSON text rows, copied into new revisions and submission snapshots. `package:<costType>:<name>` renames Engineering work packages across manhour/expense sources, rejecting collisions and mixed ownership for non-elevated users. `EstimateEffortCells` calls PUT `/api/v1/estimates/:id/manhour-lines/:lineId/effort` to save only Qty/Man-days/Hours, retain rates and audit changes. No changes to approval or ERP workbook rules.

...
- Cost list & ERP mapping UI: [EstimateErpSummary.tsx](../../app/system/production/EstimateErpSummary.tsx) is one collapsible, numbered cost list (sections from [estimate-cost-breakdown.ts](../../lib/estimate-cost-breakdown.ts); sections start collapsed) with the ERP category of each line editable in the last column — selection/bulk apply, text search, unmapped-only filter, discard. Suggestions from [erp-category-suggest.ts](../../lib/erp-category-suggest.ts) require review and an explicit save; OtherCostLine and Contingency always stay manual (Contingency is edited on the footer row). Export remains blocked while drafts are unsaved. Check [suggestion tests](../../tests/erp-category-suggest.test.mjs) and [workbook tests](../../tests/erp-estimate-workbook.test.mjs) for changes here.

...
const stage=path.resolve('backend-node/tmp/completion-stage-'+Date.now());
...
const changed=[];
for(const module of ['unified-report-service','routes/unified-reports','performance-framework','performance-evidence','routes/performance','historical-pr','routes/historical-pr','routes/health'])for(const ext of ['.js','.js.map','.d.ts']){const rel='dist/src/'+module+ext;fs.copyFileSync(path.join('backend-node',rel),path.join(stage,rel));changed.push(rel);}
...
fs.writeFileSync(path.join(stage,'hotfix.json'),JSON.stringify({baseline,changed,files},null,2));fs.writeFileSync('.codex-tmp/completion-stage.txt',stage);console.log(JSON.stringify({stage,changed,artifacts:files.length}));

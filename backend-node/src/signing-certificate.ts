...
export function documentClassLabel(code: string): string {
  return CLASS_LABELS[code] ?? code;
}

function markLabel(code: string): string {
...
export function buildSignatureCertificate(model: CertificateModel): Buffer {
  const markImage = (value: string | undefined, label: string) => value && /^[A-Za-z0-9+/=]+$/.test(value)
  ...
  const rows = model.signatures.map((signature) => `<tr>
      ...
      <td>${escapeHtml(signature.signerName)}${signature.signerRole ? `<br><span class="sub">${escapeHtml(signature.signerRole)}</span>` : ""}${
        signature.delegatedFromName ? `<br><span class="sub">delegated by ${escapeHtml(signature.delegatedFromName)}</span>` : ""}</td>
      <td>${escapeHtml(markLabel(signature.requiredMark))}${
        signature.stampCode ? `<br><span class="sub">stamp ${escapeHtml(signature.stampCode)} applied by ${escapeHtml(signature.signerName)}</span>` : ""}${markImage(signature.signaturePngBase64,"Signature")}${markImage(signature.stampPngBase64,"Company stamp")}</td>

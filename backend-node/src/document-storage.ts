 ...
 */
export const DOCUMENT_UPLOAD_RATE_LIMIT = { max: 6, timeWindow: "1 minute" } as const;
export const DOCUMENT_DOWNLOAD_RATE_LIMIT = { max: 12, timeWindow: "1 minute" } as const;
...
export type StoredWrite = { sizeBytes: number; sha256: string };
export type MultipartUpload = { file: SavedMultipartFile; values: MultipartFields | Record<string, unknown> };
export type MultipartForm = { file: SavedMultipartFile | null; values: MultipartFields | Record<string, unknown> };

function rootWithSeparator(rootPath: string): string {
  return rootPath.endsWith(sep) ? rootPath : `${rootPath}${sep}`;
}

export function resolveStoragePath(storage: AppConfig["documentStorage"], storageKey: string): string {
...
export function storageKey(prefix: string, extension: string, now = new Date()): string {
...
export function validateFileExtension(fileName: string, allowed: ReadonlySet<string> = DOCUMENT_EXTENSIONS): string {
...
export async function readMultipartUpload(
  ...
  if (!result.file) throw new ApiError(400, "file_required", "Exactly one multipart file field named 'file' is required.");
...
export async function readMultipartForm(
  ...
  try {
    ...
    if (status === 413) throw new ApiError(413, "file_too_large", `The file limit is ${maximumBytes} bytes.`);
    throw new ApiError(400, "invalid_multipart", "The multipart upload is invalid.");
  }
}

function rawMultipartValue(values: MultipartFields | Record<string, unknown>, name: string): unknown {
  const entry = values[name];
  if (Array.isArray(entry)) {
    if (entry.length !== 1) throw new ApiError(400, "validation_failed", `Multipart field '${name}' may be supplied once.`);
...
export function multipartText(
  ...
  if (required && !value) throw new ApiError(400, "validation_failed", `Multipart field '${name}' is required.`);
  if (value.length > maximumLength) throw new ApiError(400, "validation_failed", `${name} cannot exceed ${maximumLength} characters.`);
  return value || null;
}

export async function writeStoredFile(
  storage: AppConfig["documentStorage"],
  key: string,
  sourcePath: string,
): Promise<StoredWrite> {
  ...
  try {
    ...
    for await (const chunk of createReadStream(sourcePath)) {
      ...
      if (sizeBytes > storage.maxFileSizeBytes) throw new ApiError(413, "file_too_large", `The file limit is ${storage.maxFileSizeBytes} bytes.`);
      hash.update(chunk);
      await handle.write(chunk);
    }
    if (sizeBytes === 0) throw new ApiError(400, "empty_file", "The uploaded file is empty.");
    await handle.sync();
    await handle.close();
    handle = null;
 ...
 */
export async function writeStoredBuffer(
  storage: AppConfig["documentStorage"],
  key: string,
  contents: Buffer,
): Promise<StoredWrite> {
  if (contents.length === 0) throw new ApiError(400, "empty_file", "The generated document is empty.");
...
export async function deleteStoredFile(storage: AppConfig["documentStorage"], key: string): Promise<void> {
  try {
...
export async function verifyStoredFile(
  ...
  try {
    ...
    if (size !== expectedSize) throw new ApiError(409, "document_integrity_failed", "The stored document changed while it was being verified.");
    if (expectedSha256) {
...
export async function sendStoredFile(
  ...
  reply: FastifyReply,
  storage: AppConfig["documentStorage"],
  metadata: { storageKey: string; fileName: string; contentType: string; sizeBytes: number; sha256: string | null },
  options: { inline?: boolean } = {},
): Promise<FastifyReply> {
  if (request.method === "HEAD") throw new ApiError(405, "method_not_allowed", "HEAD is not supported for protected document downloads.");
...
export function storagePathRelativeToRoot(storage: AppConfig["documentStorage"], absolutePath: string): string {

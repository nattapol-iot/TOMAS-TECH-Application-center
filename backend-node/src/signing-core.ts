...
const NUMBER_PREFIX: Record<DocumentClass, string> = {
  DRAWING: "DWG", SPEC: "SPC", MANUAL: "MAN", MAT_APPROVE: "MTA",
  QUOTATION: "QT", PR_PO: "PRD", UAT_ACCEPT: "UAT", SERVICE_RPT: "SVR",
};

export function numberPrefix(documentClass: DocumentClass): string {
  return NUMBER_PREFIX[documentClass];
}

export function demandDocumentClass(value: unknown): DocumentClass {
  if (typeof value !== "string" || !(DOCUMENT_CLASSES as readonly string[]).includes(value)) {
    throw new ApiError(400, "unknown_document_class", `The document class must be one of ${DOCUMENT_CLASSES.join(", ")}.`);
  }
  return value as DocumentClass;
}

export function demandLocale(value: unknown): string {
...
export function normalizeRevisionLabel(value: unknown): string {
...
export function requireReason(value: unknown, code: string): string {
  const reason = typeof value === "string" ? value.trim() : "";
  if (!reason) throw new ApiError(400, code, "A reason is required and is kept in the event chain.");
  if (reason.length > 4_000) throw new ApiError(400, code, "The reason must be 4000 characters or fewer.");
  return reason;
}
...
// ---------------------------------------------------------------------------
// The signature event chain
//
//   payload_hash = SHA256( "k=v\n" for every field, keys sorted, UTF-8 )
//   hash         = SHA256( prev_hash + "|" + payload_hash )   ("" for seq 1)
//
// Deliberately simple enough to re-compute by hand from the verification page.
// What it proves: the recorded sequence has not been rewritten, and the output
// corresponds to the input each signer actually saw. What it does not prove:
// that the signer is who the account says they are — that comes from the
// identity provider and the freshness check below.
// ---------------------------------------------------------------------------

export type ChainField = [string, string | null | undefined];

export function canonicalPayload(fields: ChainField[]): string {
  return fields
...
export function chainHash(previousHash: string | null, payloadHash: string): string {
...
export function isVerifyCodeShape(value: unknown): value is string {
...
// Assurance
//
// The requirement specified the visible mark — a signature image and the company
// stamp. That is what the reader of the paper sees; it is not the assurance,
// because an image can be pasted. This platform holds no password, so the usual
// "re-enter your password to sign" is unavailable and would be theatre if faked.
// Assurance here is a *fresh* credential: the frontend forces an interactive
// Microsoft sign-in before opening the sign panel, and this checks how recently
// the token was issued. The evidence label is written into the chain so an audit
// can see how presence was established rather than having to assume it.
// ---------------------------------------------------------------------------

export type SigningAssurance = { evidence: string; authenticatedAt: Date };

/** datetimeoffset(0) keeps whole seconds only; hash and store the same value. */
function truncateToSecond(value: Date): Date {
  return new Date(Math.floor(value.getTime() / 1_000) * 1_000);
}

const DEFAULT_MAX_AUTH_AGE_SECONDS = 300;

export function evaluateSigningAssurance(request: FastifyRequest): SigningAssurance {
  ...
  if (identity.mode === "Development") return { evidence: "development", authenticatedAt: new Date() };
  if (identity.mode === "TeamTest") return { evidence: "team-test", authenticatedAt: new Date() };
  ...
  if (identity.authTime) return freshOrThrow("entra-auth_time", identity.authTime, maxAgeSeconds);
  if (requireAuthTime) {
    throw new ApiError(401, "auth_time_required",
      "This deployment requires the identity provider to assert when you last signed in. Add the auth_time optional claim to the API app registration.");
  }
  ...
  if (identity.issuedAt) return freshOrThrow("entra-token-iat", identity.issuedAt, maxAgeSeconds);
  throw new ApiError(401, "reauthentication_required",
    "The access token does not state when it was issued, so your presence cannot be evidenced. Sign in again and retry.");
}

function freshOrThrow(evidence: string, unixSeconds: number, maxAgeSeconds: number): SigningAssurance {
  ...
  if (ageSeconds < -300 || ageSeconds > maxAgeSeconds) {
    throw new ApiError(401, "reauthentication_required",
      `Signing requires a sign-in from the last ${Math.round(maxAgeSeconds / 60)} minutes. Sign in again and retry.`,
      { evidence, ageSeconds: Math.round(ageSeconds), maxAgeSeconds });
  }
...
export async function loadSignableDocument(
  ...
  forUpdate = false,
): Promise<SignableDocumentContext> {
  const statement = `
  ...
  if (!row) throw new ApiError(404, "document_not_found", "The signable document was not found.");
  return {
    id: Number(row.id),
    documentNo: String(row.doc_no),
    documentClass: String(row.doc_class) as DocumentClass,
 ...
 */
export async function demandDocumentScope(
  database: Database,
  ...
  if (isProjectElevated(actor) || document.ownerId === actor.id) return;
  ...
  if (!row?.allowed) {
    throw new ApiError(403, "record_access_denied",
      "You are not the owner of this document and not assigned to its signature flow.");
  }
}

export function demandRowVersion(actual: Buffer, expected: Buffer): void {
...
export function demandOwnerOrElevated(document: SignableDocumentContext, actor: CurrentUser): void {
  if (document.ownerId === actor.id || isProjectElevated(actor)) return;
  throw new ApiError(403, "document_owner_required",
    "Only the document owner can change its revisions or open a signature request.");
}

/** Appends one link to a request's hash chain. */
export async function appendSignEvent(
  transaction: TransactionType,
  input: {
    requestId: number;
    stepId?: number | null;
  ...
  const previous = (await head.query<{ seq: number; hash: string }>(`
  ...
  const fields: ChainField[] = [
    ...
    ["ip", input.ip ?? null],
    ...Object.entries(input.payload).map(([key, value]) => [`p.${key}`, value] as ChainField),
  ];
...
export type ChainVerification = { verified: boolean; head: string; count: number };
...
export async function verifyChain(
  ...
  if (transaction) {
    const request = new sql.Request(transaction);
    bind(request);
    rows = (await request.query<Record<string, unknown>>(statement)).recordset;
  } else {
    rows = (await database.query<Record<string, unknown>>(statement, bind)).recordset;
  }
  ...
  for (const row of rows) {
    const detail = row.detail === null || row.detail === undefined
      ? null
      : (JSON.parse(String(row.detail)) as Record<string, string | null>);
    const fields: ChainField[] = [
      ...
      ["auth_at", row.auth_at === null ? null : new Date(row.auth_at as string).toISOString()],
      ["ip", row.ip === null ? null : String(row.ip)],
      ...(detail ? Object.entries(detail).map(([key, value]) => [`p.${key}`, value] as ChainField) : []),
    ];
...
export async function activeSpecimenId(
  database: Database,
  userId: number,
  transaction?: TransactionType,
): Promise<number | null> {
  ...
  if (transaction) {
    const request = new sql.Request(transaction);
    bind(request);
    row = (await request.query<{ id: number }>(statement)).recordset[0];
  } else {
    row = (await database.query<{ id: number }>(statement, bind)).recordset[0];
  }
  return row ? Number(row.id) : null;
}

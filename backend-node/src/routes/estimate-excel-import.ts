...
export function parseExcelImport(body: Record<string, unknown>) {
  ...
  const lines = body.lines.map((raw) => {
    ...
    const moduleName = requiredText(row.module, 200, "Module");
    const key = `${moduleName.toLowerCase()}|${itemCode.toLowerCase()}`;
    ...
    return { kind, categoryCode, itemCode, module: moduleName, quantity, unitCost, supplierId, costType,

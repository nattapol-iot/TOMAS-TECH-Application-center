...
function quantity(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value) || value < 0.0001 || value > 1_000_000_000) throw new ApiError(400, "validation_failed", "Quantity must be between 0.0001 and 1000000000.");
  if (!value.toString().toLowerCase().includes("e") && (value.toString().split(".")[1]?.length ?? 0) > 4) throw new ApiError(400, "validation_failed", "Quantity cannot have more than 4 decimal places.");
  return value;
}
async function header(transaction: TransactionType, id: number): Promise<BomHeader> {
  const request = new sql.Request(transaction); request.input("id", sql.BigInt, id);
  const row = (await request.query<BomHeader>(`SELECT id,bom_no,revision,status,project_id,estimate_id FROM dbo.boms WITH (UPDLOCK,HOLDLOCK) WHERE id=@id AND deleted_at IS NULL;`)).recordset[0];
  if (!row) throw new ApiError(404, "bom_not_found", "BOM not found."); return row;
}

export function registerBomRoutes(app: FastifyInstance, config: AppConfig, database: Database, users: CurrentUserService): void {
  app.get("/api/v1/boms", async (request) => {
    ...
    const result = await database.query<Record<string, unknown> & { row_version: Buffer }>(`SELECT b.id,b.bom_no,b.revision,b.status,b.project_id,p.project_no,p.name project_name,
      ...
      r.input("project", sql.BigInt, projectId); r.input("actor", sql.BigInt, actor.id); r.input("elevated", sql.Bit, isProjectElevated(actor)); });
  ...
  app.get("/api/v1/boms/:id", async (request) => {
    await users.demandPermission(request, "procurement.read"); const actor = await users.required(request); const id = positiveLong((request.params as { id?: string }).id, "BOM id");
    const scoped = await database.query<{ project_id: number | string }>(`SELECT project_id FROM dbo.boms WHERE id=@id AND deleted_at IS NULL;`, (r) => r.input("id", sql.BigInt, id));
    const projectId = scoped.recordset[0] ? Number(scoped.recordset[0].project_id) : 0; if (!projectId) throw new ApiError(404, "bom_not_found", "BOM not found.");
  ...
  app.post("/api/v1/boms", async (request, reply) => {
    await users.demandPermission(request, "procurement.request"); const actor = await users.required(request); const body = bodyObject(request.body);
    const projectId = requiredInteger(body.projectId, "Project", 1); const today = todayIn(config.businessTimeZone);
    const created = await database.transaction(async (transaction) => { await demandProjectScope(database, actor, projectId, transaction);
      const lookup = new sql.Request(transaction); lookup.input("project", sql.BigInt, projectId);
      const estimate = (await lookup.query<{ id: number | string; revision: number; estimate_no: string; status: string }>(`SELECT e.id,e.revision,e.estimate_no,e.status
      ...
      if (!estimate) throw new ApiError(409, "estimate_missing", "This project has no linked estimate to generate a BOM from.");
      if (!["approved", "locked"].includes(estimate.status.toLowerCase())) throw new ApiError(409, "estimate_not_approved", `A BOM can only be generated from an approved or locked estimate; this one is '${estimate.status}'.`);
      const existing = new sql.Request(transaction); existing.input("project", sql.BigInt, projectId); const current = (await existing.query<{ bom_no: string }>(`SELECT TOP(1) bom_no FROM dbo.boms WITH (UPDLOCK,HOLDLOCK) WHERE project_id=@project AND deleted_at IS NULL;`)).recordset[0];
      if (current) throw new ApiError(409, "bom_exists", `${current.bom_no} already exists for this project. Create a revision instead.`);
      const number = await issueDocumentNumber(transaction, "BOM", today); const insert = new sql.Request(transaction);
      insert.input("number", sql.NVarChar(30), number); insert.input("project", sql.BigInt, projectId); insert.input("estimate", sql.BigInt, Number(estimate.id)); insert.input("actor", sql.BigInt, actor.id);
      const bom = (await insert.query<{ id: number | string; row_version: Buffer }>(`INSERT INTO dbo.boms(bom_no,revision,project_id,estimate_id,status,created_by,updated_by)
        OUTPUT inserted.id,inserted.row_version VALUES(@number,1,@project,@estimate,N'Draft',@actor,@actor);`)).recordset[0]!; const id = Number(bom.id);
      const copy = new sql.Request(transaction); copy.input("bom", sql.BigInt, id); copy.input("estimate", sql.BigInt, Number(estimate.id)); copy.input("revision", sql.Int, estimate.revision); copy.input("actor", sql.BigInt, actor.id);
  ...
  app.post("/api/v1/boms/:id/release", async (request) => { await users.demandPermission(request, "procurement.approve"); const actor = await users.required(request);
    const id = positiveLong((request.params as { id?: string }).id, "BOM id"); const body = bodyObject(request.body); const version = parseRowVersion(body.rowVersion); const comment = optionalBodyText(body.comment, 20_000, "Comment");
    return database.transaction(async (transaction) => { const current = await header(transaction, id); await demandProjectScope(database, actor, Number(current.project_id), transaction);
      if (current.status !== "Draft") throw new ApiError(409, "bom_not_draft", `Only a draft BOM can be released; this one is '${current.status}'.`);
      ...
      const row = (await update.query<{ row_version: Buffer }>(`UPDATE dbo.boms SET status=N'Released',released_at=SYSUTCDATETIME(),released_by=@actor,updated_by=@actor,updated_at=SYSUTCDATETIME()
        OUTPUT inserted.row_version WHERE id=@id AND deleted_at IS NULL AND row_version=@version;`)).recordset[0]; if (!row) throw new ApiError(409, "concurrency_conflict", "This BOM changed. Reload and try again.");
  ...
  app.post("/api/v1/boms/:id/reservations", async (request, reply) => { await users.demandPermission(request, "procurement.request"); const actor = await users.required(request);
    const id = positiveLong((request.params as { id?: string }).id, "BOM id"); const body = bodyObject(request.body); const lineId = requiredInteger(body.bomLineId, "BOM line", 1);
    const requested = quantity(body.quantity); const requiredDate = parseDateOnly(body.requiredDate, "Required date", true);
    const created = await database.transaction(async (transaction) => { const current = await header(transaction, id); const projectId = Number(current.project_id); await demandProjectScope(database, actor, projectId, transaction);
      if (current.status !== "Released") throw new ApiError(409, "bom_not_released", `Stock can only be reserved against a released BOM; this one is '${current.status}'.`);
      const lookup = new sql.Request(transaction); lookup.input("line", sql.BigInt, lineId); lookup.input("bom", sql.BigInt, id);
      const row = (await lookup.query<Record<string, number | string>>(`SELECT l.item_id,i.item_code,l.qty_required,l.customer_supplied_qty,
      ...
      if (!row) throw new ApiError(404, "bom_line_not_found", "This BOM line does not exist or is not a stock line."); const itemId = Number(row.item_id); const itemCode = String(row.item_code);
      const available = Number(row.available); const covered = Math.max(Number(row.allocated), Number(row.net_issued) + Number(row.active_reserved));
      const remaining = Math.max(0, Number(row.qty_required) - Number(row.customer_supplied_qty) - covered);
      if (requested > remaining) throw new ApiError(409, "quantity_exceeds_bom_demand", `Only ${remaining} ${itemCode} is still required by this BOM line.`, { remainingDemand: remaining, requested });
      if (requested > available) throw new ApiError(409, "insufficient_stock", `Only ${available} of ${itemCode} is available to reserve.`, { available });
      const insert = new sql.Request(transaction); insert.input("item", sql.BigInt, itemId); insert.input("project", sql.BigInt, projectId); insert.input("line", sql.BigInt, lineId);
      insert.input("qty", sql.Decimal(19, 4), requested); insert.input("required_date", sql.Date, requiredDate); insert.input("actor", sql.BigInt, actor.id);
      const reservation = (await insert.query<{ id: number | string }>(`INSERT INTO dbo.reservations(item_id,project_id,bom_line_id,qty,required_date,owner_id,status) OUTPUT inserted.id VALUES(@item,@project,@line,@qty,@required_date,@actor,N'Active');`)).recordset[0]!;
      const reservationId = Number(reservation.id); await insertMaterialAudit(transaction, actor, "Reserved stock for project", "Reservation", reservationId, itemCode,
        { available }, { reserved: requested, remaining: available - requested }, { quantity: requested, projectId, reason: "BOM allocation" });
  ...
  app.post("/api/v1/boms/:id/reservations/:reservationId/release", async (request) => { await users.demandPermission(request, "procurement.request"); const actor = await users.required(request);
    const p = request.params as { id?: string; reservationId?: string }; const id = positiveLong(p.id, "BOM id"); const reservationId = positiveLong(p.reservationId, "Reservation id");
    return database.transaction(async (transaction) => { const current = await header(transaction, id); const projectId = Number(current.project_id); await demandProjectScope(database, actor, projectId, transaction);
      const update = new sql.Request(transaction); update.input("reservation", sql.BigInt, reservationId); update.input("bom", sql.BigInt, id);
      const row = (await update.query<{ qty: number | string; item_code: string }>(`UPDATE r SET r.status=N'Released',r.updated_at=SYSUTCDATETIME() OUTPUT inserted.qty,i.item_code
        FROM dbo.reservations r INNER JOIN dbo.mat_items i ON i.id=r.item_id INNER JOIN dbo.bom_lines l ON l.id=r.bom_line_id
        WHERE r.id=@reservation AND l.bom_id=@bom AND r.status=N'Active';`)).recordset[0]; if (!row) throw new ApiError(409, "reservation_not_active", "This reservation is not active on this BOM.");
      const released = Number(row.qty); await insertMaterialAudit(transaction, actor, "Released reservation", "Reservation", reservationId, row.item_code,
        { status: "Active", quantity: released }, { status: "Released" }, { quantity: released, projectId }); return { id: reservationId, status: "Released", quantity: released }; });
  });
}

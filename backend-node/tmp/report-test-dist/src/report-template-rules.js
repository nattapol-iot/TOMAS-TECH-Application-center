 ...
 */
export const REPORT_TEMPLATE_FIELDS = {
...
export const REPORT_TEMPLATE_SECTIONS_BY_TYPE = {
...
export function sanitizeReportTemplate(reportType, value) {
    ...
    if (!sections || !object(value))
        throw new Error('Select a supported report type and an object body.');
    const output = {};
    for (const section of sections) {
            ...
            for (const field of rule.fields) {
                ...
                if (typeof text === 'string' && text.trim()) {
                    if (text.length > 20000)
                        throw new Error(`Template ${section}.${field} exceeds 20000 characters.`);
        ...
        if (rule.repeat) {
            if (!Array.isArray(source) || source.length > 500)
                throw new Error(`Template ${section} must be a list of at most 500 rows.`);
            const rows = source.map(copy).filter(row => Object.keys(row).length > 0);
            if (rows.length)
                output[section] = rows;
        }
        else {
            if (!object(source))
                throw new Error(`Template ${section} must be an object.`);
...
//# sourceMappingURL=report-template-rules.js.map
...
export class CurrentUserService {
    ...
    `, (sqlRequest) => sqlRequest.input("identity", sql.NVarChar(useEmail ? 256 : 64), identity.value));
        ...
        if (!row)
            throw new ApiError(403, "user_not_registered", "Your account is not registered for IoT Team Center.");
        const user = {
            id: Number(row.id),
        ...
        if (!user.isActive)
            throw new ApiError(403, "user_disabled", "Your IoT Team Center account is disabled.");
        request.currentUser = user;
        return user;
    }
    async demandPermission(request, permission) {
    ...
    `, (sqlRequest) => {
        ...
        if (!result.recordset[0]?.allowed)
            throw new ApiError(403, "permission_denied", `Permission '${permission}' is required.`);
    }
}

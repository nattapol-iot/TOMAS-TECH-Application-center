...
export async function budgetPicture(transaction, projectId, excludePrId) {
    const request = new sql.Request(transaction);
    request.input("project", sql.BigInt, projectId);
    request.input("exclude", sql.BigInt, excludePrId);
    ...
    const approvedBudget = Number(row.approved_budget);
    const actualConsumed = Number(row.actual_consumed);
    const openCommitment = Number(row.open_commitment);
    const reservedValue = Number(row.reserved_value);
    const siblingOpenPrValue = Number(row.sibling_open_pr);
    const amount = Number(row.this_amount);
    const forecastBefore = actualConsumed + openCommitment + reservedValue + siblingOpenPrValue;
...
export async function procurementRuleFlags(transaction, prId) {
    const request = new sql.Request(transaction);
    request.input("pr", sql.BigInt, prId);
    const result = await request.query(`SELECT pr.project_id,pr.priority,COALESCE((SELECT SUM(l.line_total) FROM dbo.mat_pr_lines l WHERE l.pr_id=pr.id),0) amount
    ...
    const flags = [];
    for (const raw of result.recordsets[1] ?? []) {
        const row = raw;
        const item = String(row.item_code);
        const estimate = Number(row.est_unit_cost);
        if (estimate > 0) {
            const variance = (Number(row.unit_price) - estimate) / estimate * 100;
            if (variance > 10)
                flags.push({ code: "price_variance", text: `${item} unit price is ${variance.toFixed(1)}% above the estimate (limit 10%)` });
        }
        if (row.is_unplanned)
            flags.push({ code: "unplanned_item", text: `${item} is not in the approved estimate` });
        if (String(row.price_source).toLowerCase() === "manual")
            flags.push({ code: "manual_price", text: `${item} uses a manual price` });
        if (Number(row.available) >= Number(row.qty) && Number(row.qty) > 0)
            flags.push({ code: "stock_available", text: `${item}: available stock covers this line — buying anyway needs a reason` });
    }
    const budget = await budgetPicture(transaction, Number(header.project_id), prId);
    if (!budget.withinBudget)
        flags.push({ code: "over_budget", text: "The requisition pushes the project forecast over its approved budget" });
    if (Number(header.amount) > 1_000_000)
        flags.push({ code: "high_value", text: "Requisition value exceeds 1,000,000 THB" });
    if (header.priority === "Emergency")
        flags.push({ code: "emergency", text: "Emergency purchase" });
    return flags;
}
//# sourceMappingURL=procurement-rules.js.map
...
import { planDates, workloadImpact, workDates } from './resource-task-math.js';
export const elevated = (actor) => ['Admin', 'Engineering Manager'].includes(actor.role);
export async function sourceAccess(tx, actor, kind, sourceId, write = false, planner = false) {
    if (!['Inquiry', 'Project'].includes(kind))
        throw new ApiError(400, 'invalid_source', 'Select Inquiry or Project.');
    const q = new sql.Request(tx);
    q.input('source', sql.BigInt, sourceId).input('actor', sql.BigInt, actor.id).input('elevated', sql.Bit, elevated(actor));
    ...
    const r = (await q.query(query)).recordset[0];
    if (!r)
        throw new ApiError(404, 'source_missing', 'Source is unavailable.');
    if (!r.can_read || planner && !r.can_plan)
        throw new ApiError(403, 'task_scope', 'Only the responsible planner or assigned team may access this work.');
    if (write && ['Closed', 'Cancelled', 'Rejected'].includes(r.status))
        throw new ApiError(409, 'source_closed', 'Closed work cannot be changed.');
    return r;
}
export async function taskRow(tx, id, lock = false) {
    const q = new sql.Request(tx);
    q.input('id', sql.BigInt, id);
    const r = (await q.query(`SELECT * FROM dbo.resource_tasks ${lock ? 'WITH(UPDLOCK,HOLDLOCK)' : ''} WHERE id=@id`)).recordset[0];
    if (!r)
        throw new ApiError(404, 'task_missing', 'Task not found.');
    return r;
}
export async function assigneeAllowed(tx, kind, sourceId, userId) {
    const q = new sql.Request(tx);
    q.input('user', sql.BigInt, userId);
    if (!(await q.query(`SELECT u.id FROM dbo.users u WHERE u.id=@user AND u.is_active=1 AND u.deleted_at IS NULL AND EXISTS(SELECT 1 FROM dbo.role_permissions rp JOIN dbo.permissions p ON p.id=rp.permission_id WHERE rp.role_id=u.role_id AND p.code=N'schedule.progress')`)).recordset.length)
        throw new ApiError(422, 'assignee_inactive', 'Select an active member with permission to acknowledge and update tasks.');
    if (kind === 'Project')
        await validatePics(tx, await readProject(tx, sourceId), [userId]);
}
export async function taskDto(tx, r, actor, db) {
    ...
    let start = dateOnly(r.plan_start), end = dateOnly(r.plan_end), effort = r.man_days === null ? null : Number(r.man_days), percent = Number(r.percent_done), status = r.execution_status, scheduleVersion = null;
    ...
    if (r.schedule_task_id) {
        ...
        if (task) {
            const resolved = resolveTasks(tasks, await readHolidays(tx)).byId.get(task.id);
            start = resolved.planStart;
            end = resolved.planFinish;
            effort = task.planManDays;
            percent = task.percentComplete;
            status = task.status;
            scheduleVersion = task.rowVersion.toString('base64');
    ...
    const q = new sql.Request(tx);
    q.input('id', sql.BigInt, r.assignee_id);
    const assignee = (await q.query('SELECT name FROM dbo.users WHERE id=@id')).recordset[0]?.name ?? '';
    ...
    return {
        ...
        percentComplete: percent, status, pendingPlan: r.pending_plan ? JSON.parse(r.pending_plan) : null,
...
export async function impact(tx, plan, exclude, kind, sourceId) {
    const q = new sql.Request(tx);
    q.input('user', sql.BigInt, plan.assigneeId);
    const holidays = await readHolidays(tx), allocations = [], replaced = [];
    const capacityRow = (await q.query('SELECT days_per_week FROM dbo.resource_capacity WHERE user_id=@user')).recordset[0];
    const records = (await q.query(`SELECT i.id,COALESCE(e.start_date,i.inquiry_date) start_date,COALESCE(e.end_date,i.due_date) end_date,e.man_days FROM dbo.inquiries i LEFT JOIN dbo.resource_effort e ON e.entity_type=N'Inquiry' AND e.entity_id=i.id WHERE i.estimate_owner_id=@user AND i.deleted_at IS NULL AND i.status NOT IN(N'Closed',N'Cancelled',N'Rejected') AND NOT EXISTS(SELECT 1 FROM dbo.resource_task_sources s WHERE s.inquiry_id=i.id)`)).recordset;
    for (const r of records) {
        const target = kind === 'Inquiry' && Number(r.id) === sourceId ? replaced : allocations;
        target.push({ key: `Inquiry-${r.id}`, start: dateOnly(r.start_date), end: dateOnly(r.end_date), manDays: r.man_days === null ? null : Number(r.man_days) });
    }
    const inquiryTasks = (await q.query(`SELECT t.* FROM dbo.resource_tasks t JOIN dbo.inquiries i ON i.id=t.inquiry_id WHERE t.assignee_id=@user AND t.state=N'Approved' AND t.execution_status<>N'Done' AND i.deleted_at IS NULL AND i.status NOT IN(N'Closed',N'Cancelled',N'Rejected')`)).recordset;
    for (const r of inquiryTasks)
        (Number(r.id) === Number(exclude?.id) ? replaced : allocations).push({ key: `Task-${r.id}`, start: dateOnly(r.plan_start), end: dateOnly(r.plan_end), manDays: Number(r.man_days) });
    const estimates = (await q.query(`WITH owners AS (SELECT estimate_id,owner_id user_id FROM dbo.estimate_assignments UNION SELECT estimate_id,support_id FROM dbo.estimate_assignments WHERE support_id IS NOT NULL UNION SELECT id,owner_id FROM dbo.estimates e WHERE NOT EXISTS(SELECT 1 FROM dbo.estimate_assignments a WHERE a.estimate_id=e.id)) SELECT e.id,COALESCE(p.start_date,e.created_date) start_date,COALESCE(p.end_date,e.due_date) end_date,p.man_days,(SELECT COUNT(*) FROM owners all_owners WHERE all_owners.estimate_id=e.id) owners FROM dbo.estimates e JOIN owners o ON o.estimate_id=e.id AND o.user_id=@user LEFT JOIN dbo.resource_effort p ON p.entity_type=N'Estimate' AND p.entity_id=e.id WHERE e.deleted_at IS NULL AND e.status NOT IN(N'Closed',N'Cancelled',N'Approved',N'Locked')`)).recordset;
    for (const r of estimates)
        allocations.push({ key: `Estimate-${r.id}`, start: dateOnly(r.start_date), end: dateOnly(r.end_date), manDays: r.man_days === null ? null : Number(r.man_days) / Number(r.owners) });
    const projects = (await q.query(`SELECT DISTINCT t.project_id FROM dbo.schedule_tasks t JOIN dbo.schedule_task_pics pic ON pic.task_id=t.id JOIN dbo.projects p ON p.id=t.project_id WHERE pic.user_id=@user AND t.deleted_at IS NULL AND p.deleted_at IS NULL AND p.status<>N'Closed'`)).recordset;
    for (const p of projects) {
        ...
        for (const task of tasks) {
            const r = resolved.byId.get(task.id);
            const owners = pics.get(task.id) ?? [];
            if (r.children.length || task.kind === 'phase' || task.status === 'Done' || !owners.some(o => o.id === plan.assigneeId))
                continue;
...
export async function saveApprovedProjectPlan(tx, r, plan, actor) {
...
//# sourceMappingURL=resource-task-service.js.map
...
export function createTeamTestAccessCode(signingKey, email) {
    return createHmac("sha256", signingKey).update(email.trim().toLowerCase(), "utf8").digest("base64url");
}
export function teamTestAccessCodeMatches(signingKey, email, supplied) {
...
function teamTestIdentity(request, signingKey) {
    ...
    if (email.length > 256 || !EMAIL_PATTERN.test(email))
        throw new ApiError(401, "invalid_test_email", "The team-test email is invalid.");
    if (!teamTestAccessCodeMatches(signingKey, email, accessCode))
        throw new ApiError(401, "invalid_credentials", "The team-test credentials are invalid.");
...
export function registerAuthentication(app, config) {
    ...
    app.addHook("preHandler", async (request) => {
        ...
        try {
            ...
            if (!scopes.includes(config.auth.requiredScope))
                throw new ApiError(403, "missing_scope", "The delegated API scope is required.");
            const objectId = typeof verified.payload.oid === "string" ? verified.payload.oid : undefined;
            if (!objectId)
                throw new ApiError(401, "missing_oid", "The Entra token does not contain an object id.");
            const unixSeconds = (claim) => typeof claim === "number" && Number.isFinite(claim) && claim > 0 ? claim : undefined;

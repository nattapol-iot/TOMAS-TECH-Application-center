...
const node = (value) => value && typeof value === "object" ? value : {};
const string = (value) => value == null ? "" : typeof value === "object" ? string(node(value)["#text"]) : String(value);
const richText = (value) => {
    const n = node(value);
    return n.r ? list(n.r).map(r => string(r.t)).join("") : string(n.t);
};
const truth = (value) => value != null && !["0", "false"].includes(string(node(value)["@_val"] || "1"));
const fail = (message) => { throw new ApiError(400, "historical_pr_invalid", message); };
const round = (v) => Math.round((v + Number.EPSILON) * 100) / 100;
export function historicalTotals(lines) {
    const sum = (status) => round(lines.filter(l => l.status === status).reduce((s, l) => s + (l.actualCost ?? l.totalPrice), 0));
...
export function parseHistoricalPr(bytes, sourceName) {
    if (!/\.xlsx$/i.test(sourceName) || bytes.length > 8 * 1024 * 1024 || bytes.length < 4)
        fail("รองรับไฟล์ .xlsx ขนาดไม่เกิน 8 MB");
    let archive;
    try {
        ...
        archive = unzipSync(bytes, { filter: entry => {
                ...
                entries++;
                if (expanded > 32 * 1024 * 1024 || entries > 1000)
                    fail("ข้อมูลภายใน Excel มีขนาดใหญ่เกินกำหนด");
        ...
        if (error instanceof ApiError)
            throw error;
        fail("อ่านไฟล์ Excel ไม่ได้ กรุณาตรวจไฟล์แล้วลองใหม่");
    }
    ...
    const xml = (path) => {
        ...
        if (/<!DOCTYPE|<!ENTITY/i.test(raw))
            fail("ไฟล์ XML มีโครงสร้างที่ไม่รองรับ");
        try {
            return parser.parse(raw);
        }
        catch {
            return fail("โครงสร้าง XML ใน Excel ไม่สมบูรณ์");
        }
    };
    ...
    const result = { sourceName, sourceHash: createHash("sha256").update(bytes).digest("hex"),
    ...
    for (const s of list(node(workbook.sheets).sheet)) {
        ...
        if (["hidden", "veryHidden"].includes(string(s["@_state"])))
            continue;
        ...
        const text = (ref) => cells[ref]?.value.trim() ?? "";
        if (text("E8") !== "Model/Part Number" || text("V8") !== "PO Number" || text("C2") !== "Project Number")
            continue;
        const number = (ref, required = false) => {
            const c = cells[ref], v = text(ref);
            if (c?.error || (c?.formula && !v))
                fail(`${sheet}!${ref}: กรุณาคำนวณสูตรและบันทึกใน Excel ใหม่`);
            if (!v || v === "-") {
                if (required)
                    fail(`${sheet}!${ref}: ไม่มีจำนวนหรือราคา`);
                return null;
            }
            const n = Number(v);
            if (!Number.isFinite(n) || n < 0 || n > 1e12)
                fail(`${sheet}!${ref}: ตัวเลขไม่ถูกต้อง`);
            return n;
        };
        if (!text("F2"))
            fail(`${sheet}: ไม่พบเลข Project`);
        if (text("F2").length > 100)
            fail(`${sheet}!F2: เลข Project ยาวเกิน 100 ตัวอักษร`);
        if (result.projectNumber && result.projectNumber.toLowerCase() !== text("F2").toLowerCase())
            fail("ไฟล์มีหลาย Project กรุณาแยกไฟล์ต่อ Project");
        result.projectNumber = text("F2");
        result.projectName = text("F3");
        result.customer = text("F4");
        if (text("H5") && text("H5") !== "-") {
            result.documentReference = text("H5");
            result.referenceFromFilename = false;
        }
        const estimate = number("T3");
        if (estimate && estimate > 0)
            result.estimate = (result.estimate ?? 0) + estimate;
        const revisions = new Map();
        for (let r = 3; r <= 7; r++) {
            const rawDate = text(`N${r}`);
            let issuedDate = null;
            if (rawDate && rawDate !== "-") {
                const serial = Number(rawDate);
                if (Number.isFinite(serial) && serial > 0 && serial < 100000)
                    issuedDate = new Date(epoch + Math.floor(serial) * 86400000).toISOString().slice(0, 10);
                else if (/^\d{4}-\d{2}-\d{2}/.test(rawDate) && !Number.isNaN(Date.parse(rawDate)))
                    issuedDate = new Date(rawDate).toISOString().slice(0, 10);
                else
                    result.warnings.push(`${sheet}!N${r}: อ่านวันที่ไม่ได้ (${rawDate})`);
            }
            if (text(`I${r}`))
                revisions.set(text(`I${r}`), { issuedBy: text(`M${r}`), issuedDate });
        }
        ...
        for (const r of rowNumbers) {
            ...
            if (quantity <= 0 || Math.abs(round(quantity * unitPrice) - totalPrice) > .011)
                fail(`${sheet}!S${r}: จำนวน × ราคาไม่ตรงยอด หรือจำนวนเป็นศูนย์`);
            ...
            if (currency !== "THB")
                fail(`${sheet}!T${r}: รุ่นนี้รองรับสกุลเงิน THB กรุณาระบุสกุลเงินให้ครบ`);
            const actualCost = number(`U${r}`);
            if (status === "Unknown")
                result.warnings.push(`${sheet}!W${r}: สถานะ PO ยังไม่ทราบ (${poStatus || "ว่าง"})`);
            if (actualCost === null && status !== "Cancelled")
                result.warnings.push(`${sheet}!U${r}: ยังไม่ระบุ Actual cost ใช้ Total price เป็นยอดอ้างอิง`);
            if (!text(`V${r}`))
                result.warnings.push(`${sheet}!V${r}: ยังไม่มีเลข PO`);
            if (!issued.issuedDate)
                result.warnings.push(`${sheet} แถว ${r}: ไม่มีวันที่ของ revision`);
    ...
    if (!result.lines.length)
        fail("ไม่พบรายการในฟอร์ม PR ที่รองรับ (หัวตาราง Model/Part Number และ PO Number แถว 8)");
    if (result.lines.length > 1000)
        fail("รองรับไม่เกิน 1,000 รายการต่อไฟล์");
    if (result.documentReference.length > 200)
        fail("ชื่ออ้างอิงเอกสารยาวเกิน 200 ตัวอักษร");
    if (result.referenceFromFilename)
        result.warnings.push("Document Ref. ว่าง ใช้ชื่อไฟล์เป็นอ้างอิง สามารถแก้ใน Preview ได้");
    if (result.estimate === null)
        result.warnings.push("Estimate ว่างหรือเป็น 0 ยังสรุปเกินงบจากไฟล์นี้ไม่ได้");

...
import { allowedTransitions, loadStatusHistory, loadTraceabilityLinks, reportSlaState, rolePermissions, rowVersion, SITE_VISIT_PERMISSIONS, VISIT_TRANSITIONS, } from "./site-visit-common.js";
...
export function visitSummary(row) {
    return {
        id: Number(row.id),
        number: row.visit_no,
        ...
        reportSlaState: reportSlaState(row.report_due_at, row.report_status, row.submitted_at),
...
export async function requiredSkillCodes(database, intakeId) {
    const result = await database.query(`DECLARE @source nvarchar(20)=CASE WHEN EXISTS(SELECT 1 FROM dbo.sales_intake_skills WHERE intake_id=@id AND source=N'Coordinator') THEN N'Coordinator' ELSE N'Sales' END; SELECT k.code FROM dbo.sales_intake_skills s INNER JOIN dbo.visit_skills k ON k.id=s.skill_id WHERE s.intake_id=@id AND s.source=@source ORDER BY k.sort_order,k.code;`, (q) => q.input("id", sql.BigInt, intakeId));
    return result.recordset.map((r) => r.code);
}
export function skillMatch(required, held) {
    const wanted = [
...
function assignment(r, required) {
    const skills = String(r.skills ?? "")
        .split(",")
        .filter(Boolean), match = skillMatch(required, skills);
    return {
        id: Number(r.id),
...
export async function loadVisitReport(database, visitId) {
    const header = (await database.query(`SELECT r.*,v.visit_no,au.name author_name,rev.name reviewed_by_name FROM dbo.site_visit_reports r INNER JOIN dbo.site_visits v ON v.id=r.visit_id INNER JOIN dbo.users au ON au.id=r.author_id LEFT JOIN dbo.users rev ON rev.id=r.reviewed_by WHERE r.visit_id=@id;`, (q) => q.input("id", sql.BigInt, visitId))).recordset[0];
    if (!header)
        return null;
    const revisions = await database.query(`SELECT rv.*,cre.name created_by_name,apr.name approved_by_name FROM dbo.site_visit_report_revisions rv INNER JOIN dbo.users cre ON cre.id=rv.created_by LEFT JOIN dbo.users apr ON apr.id=rv.approved_by WHERE rv.report_id=@id ORDER BY rv.revision DESC;`, (q) => q.input("id", sql.BigInt, Number(header.id)));
    const mapped = revisions.recordset.map((r) => ({
        id: Number(r.id),
        revision: Number(r.revision),
        status: r.status,
        ...Object.fromEntries(reportBodyFields.map((field, index) => [
            field,
    ...
    return {
        ...
        dueAt: header.due_at,
        slaState: reportSlaState(header.due_at, String(header.status), header.submitted_at),
        createdAt: header.created_at,
        updatedAt: header.updated_at,
        rowVersion: rowVersion(header.row_version),
        current: mapped.find((r) => r.revision === Number(header.current_revision)) ??
            null,
        revisions: mapped,
    };
}
export async function loadVisitDetail(database, id, actorId, role) {
    const h = (await database.query(`SELECT v.*,i.intake_no,i.subject intake_subject,i.customer_id,c.code customer_code,c.name customer_name,i.site_name,i.site_address,i.contact_name,i.contact_phone,i.contact_email,t.name_en visit_type_name,ct.name checklist_template_name,sla.name sla_policy_name,ISNULL(sla.report_due_days,3) report_due_days,cin.name checked_in_by_name,cout.name checked_out_by_name,cby.name closed_by_name,cre.name created_by_name,upd.name updated_by_name FROM dbo.site_visits v INNER JOIN dbo.sales_intakes i ON i.id=v.intake_id INNER JOIN dbo.customers c ON c.id=i.customer_id INNER JOIN dbo.visit_types t ON t.id=v.visit_type_id INNER JOIN dbo.users cre ON cre.id=v.created_by INNER JOIN dbo.users upd ON upd.id=v.updated_by LEFT JOIN dbo.visit_checklist_templates ct ON ct.id=v.checklist_template_id LEFT JOIN dbo.visit_sla_policies sla ON sla.id=v.sla_policy_id LEFT JOIN dbo.users cin ON cin.id=v.checked_in_by LEFT JOIN dbo.users cout ON cout.id=v.checked_out_by LEFT JOIN dbo.users cby ON cby.id=v.closed_by WHERE v.id=@id AND v.deleted_at IS NULL;`, (q) => q.input("id", sql.BigInt, id))).recordset[0];
    if (!h)
        return null;
    const required = await requiredSkillCodes(database, Number(h.intake_id));
    const [assignmentsResult, confirmations, scheduleHistory, checklist, findings, attachments, actions, history, links, report, permissions,] = await Promise.all([
        database.query(`SELECT a.*,u.name engineer_name,u.department,asg.name assigned_by_name,ov.name override_by_name,ISNULL((SELECT STRING_AGG(k.code,N',') FROM dbo.engineer_skills es INNER JOIN dbo.visit_skills k ON k.id=es.skill_id WHERE es.user_id=a.engineer_id),N'') skills FROM dbo.site_visit_assignments a INNER JOIN dbo.users u ON u.id=a.engineer_id INNER JOIN dbo.users asg ON asg.id=a.assigned_by LEFT JOIN dbo.users ov ON ov.id=a.override_by WHERE a.visit_id=@id ORDER BY a.is_active DESC,a.assignment_role,a.assigned_at DESC;`, (q) => q.input("id", sql.BigInt, id)),
        database.query(`SELECT c.*,u.name recorded_by_name FROM dbo.site_visit_confirmations c INNER JOIN dbo.users u ON u.id=c.recorded_by WHERE c.visit_id=@id ORDER BY c.confirmed_at DESC,c.id DESC;`, (q) => q.input("id", sql.BigInt, id)),
        database.query(`SELECT s.*,u.name changed_by_name FROM dbo.site_visit_schedule_history s INNER JOIN dbo.users u ON u.id=s.changed_by WHERE s.visit_id=@id ORDER BY s.changed_at DESC,s.id DESC;`, (q) => q.input("id", sql.BigInt, id)),
        database.query(`SELECT ci.id item_id,ci.sort_order,ci.section,ci.prompt,ci.response_type,ci.unit item_unit,ci.is_required,ci.guidance,r.id response_id,r.response_value,r.numeric_value,ISNULL(r.unit,N'') unit,ISNULL(r.is_not_applicable,0) is_not_applicable,r.note,u.name answered_by_name,r.answered_at,r.row_version FROM dbo.site_visits v INNER JOIN dbo.visit_checklist_items ci ON ci.template_id=v.checklist_template_id AND ci.is_active=1 LEFT JOIN dbo.site_visit_checklist_responses r ON r.visit_id=v.id AND r.checklist_item_id=ci.id LEFT JOIN dbo.users u ON u.id=r.answered_by WHERE v.id=@id ORDER BY ci.sort_order;`, (q) => q.input("id", sql.BigInt, id)),
        database.query(`SELECT f.*,u.name created_by_name FROM dbo.site_visit_findings f INNER JOIN dbo.users u ON u.id=f.created_by WHERE f.visit_id=@id AND f.deleted_at IS NULL ORDER BY f.kind,f.sort_order,f.id;`, (q) => q.input("id", sql.BigInt, id)),
        database.query(`SELECT a.*,u.name uploaded_by_name FROM dbo.site_visit_attachments a INNER JOIN dbo.users u ON u.id=a.uploaded_by WHERE a.visit_id=@id AND a.deleted_at IS NULL ORDER BY a.uploaded_at DESC,a.id DESC;`, (q) => q.input("id", sql.BigInt, id)),
        database.query(`SELECT a.*,u.name created_by_name FROM dbo.site_visit_action_items a INNER JOIN dbo.users u ON u.id=a.created_by WHERE a.visit_id=@id AND a.deleted_at IS NULL ORDER BY CASE a.status WHEN N'Open' THEN 0 WHEN N'In Progress' THEN 1 ELSE 2 END,a.due_date,a.id;`, (q) => q.input("id", sql.BigInt, id)),
    ...
    const assignments = assignmentsResult.recordset.map((r) => assignment(r, required)), team = skillMatch(required, assignments.filter((a) => a.isActive).flatMap((a) => a.skills));
    return {
        id: Number(h.id),
        number: h.visit_no,
        status: h.status,
        ...
        checklistTemplateId: h.checklist_template_id == null ? null : Number(h.checklist_template_id),
        ...
        checkInLatitude: h.check_in_latitude == null ? null : Number(h.check_in_latitude),
        checkInLongitude: h.check_in_longitude == null ? null : Number(h.check_in_longitude),
        ...
        reportSlaState: reportSlaState(h.report_due_at, report?.status == null ? null : String(report.status), report?.submittedAt),
        ...
        confirmations: confirmations.recordset.map((r) => ({
            ...
            comment: r.comment ?? "",
            evidenceAttachmentId: r.evidence_attachment_id == null
                ? null
        ...
        findings: findings.recordset.map((r) => ({
            ...
            title: r.title,
            detail: r.detail ?? "",
            measurementValue: r.measurement_value == null ? null : Number(r.measurement_value),
        ...
        links,
        report,
        allowedTransitions: allowedTransitions(VISIT_TRANSITIONS, String(h.status), permissions),
        canExecute: permissions.has(SITE_VISIT_PERMISSIONS.visitExecute) &&
            assignments.some((a) => a.isActive && a.engineerId === actorId && a.status === "Accepted"),
    };
}
export async function visitRowVersion(transaction, id) {
    const q = new sql.Request(transaction);
    q.input("id", sql.BigInt, id);
    const r = (await q.query(`SELECT row_version FROM dbo.site_visits WHERE id=@id;`)).recordset[0];
    return r ? rowVersion(r.row_version) : "";
}
export { reportBodyColumns, reportBodyFields };
//# sourceMappingURL=site-visit-data.js.map
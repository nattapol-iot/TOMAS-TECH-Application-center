...
export function workDates(start, end, holidays) {
    const dates = [];
    if (day(end) - day(start) > 3650 || end < start)
        return dates;
    for (let n = day(start); n <= day(end); n++) {
        const d = iso(n), weekday = new Date(n * DAY).getUTCDay();
        if (weekday !== 0 && weekday !== 6 && !holidays.has(d))
            dates.push(d);
    }
    return dates;
}
export function planDates(plan, holidays) {
    const dates = [];
    for (let n = day(plan.start); dates.length < plan.workDays && n <= day(plan.start) + 3650; n++) {
        const d = iso(n);
        if (d > '9999-12-31')
            break;
        if (workDates(d, d, holidays).length)
            dates.push(d);
    }
    if (dates.length !== plan.workDays)
        throw new ApiError(400, 'invalid_plan', 'Planning interval is too long.');
    return { start: dates[0], end: dates.at(-1), calendarDays: day(dates.at(-1)) - day(dates[0]) + 1 };
}
export function parseTaskPlan(value) {
    const b = bodyObject(value), start = parseDateOnly(b.start, 'Start');
    if (typeof b.manDays !== 'number' || !Number.isFinite(b.manDays) || b.manDays <= 0 || b.manDays > 10000 || Math.abs(b.manDays * 100 - Math.round(b.manDays * 100)) > 0.00001)
        throw new ApiError(400, 'invalid_effort', 'Effort must be positive, at most 10000 man-days and two decimals.');
...
export function workloadImpact(items, plan, capacity, holidays, replaced = []) {
    ...
    for (let n = monday; n <= day(range.end); n += 7) {
        ...
        const pct = (value) => available === null || available === 0 ? null : Math.round(value / available * 1000) / 10;
...
//# sourceMappingURL=resource-task-math.js.map
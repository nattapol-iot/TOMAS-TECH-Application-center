...
export async function activeEngineerIds(t, visitId) {
    const q = new sql.Request(t);
    q.input("id", sql.BigInt, visitId);
    const r = await q.query(`SELECT engineer_id FROM dbo.site_visit_assignments WHERE visit_id=@id AND is_active=1;`);
    return r.recordset.map((x) => Number(x.engineer_id));
}
export async function demandAssignedEngineer(t, visitId, actorId) {
    const q = new sql.Request(t);
    q.input("id", sql.BigInt, visitId);
    q.input("actor", sql.BigInt, actorId);
    const r = (await q.query(`SELECT CAST(CASE WHEN EXISTS(SELECT 1 FROM dbo.site_visit_assignments WHERE visit_id=@id AND engineer_id=@actor AND is_active=1 AND status=N'Accepted') THEN 1 ELSE 0 END AS bit) valid;`)).recordset[0];
    if (!r?.valid)
        throw new ApiError(403, "not_assigned", "Only an engineer who accepted this visit can record work on it.");
}
export async function checkAvailability(t, engineerId, start, end, travelBefore, travelAfter, excludeVisitId, allowConflict) {
    const q = new sql.Request(t);
...
export async function visitVersion(t, id) {
    const q = new sql.Request(t);
    q.input("id", sql.BigInt, id);
...
export async function reportVersion(t, id) {
    const q = new sql.Request(t);
    q.input("id", sql.BigInt, id);
    const r = (await q.query(`SELECT row_version FROM dbo.site_visit_reports WHERE id=@id;`)).recordset[0];
    if (!r)
        throw new ApiError(404, "report_not_found", "Site visit report not found.");
    return rowVersion(r.row_version);
}
export async function lockReport(t, visitId, expected) {
    const q = new sql.Request(t);
    q.input("visit", sql.BigInt, visitId);
    const r = (await q.query(`SELECT r.id,r.report_no,r.status,r.current_revision,r.author_id,r.visit_id,v.visit_no,r.row_version FROM dbo.site_visit_reports r WITH(UPDLOCK,HOLDLOCK) INNER JOIN dbo.site_visits v ON v.id=r.visit_id WHERE r.visit_id=@visit AND v.deleted_at IS NULL;`)).recordset[0];
    if (!r)
        throw new ApiError(404, "report_not_found", "Site visit report not found.");
    if (expected !== undefined)
        requireRowVersion(expected, r.row_version);
...
export async function ensureReport(t, visitId, visitNumber, actorId, dueAt, today) {
    const find = new sql.Request(t);
    find.input("visit", sql.BigInt, visitId);
    const existing = (await find.query(`SELECT report_no FROM dbo.site_visit_reports WHERE visit_id=@visit;`)).recordset[0];
    if (existing)
    ...
    const reportId = Number((await q.query(`INSERT INTO dbo.site_visit_reports(report_no,visit_id,status,current_revision,author_id,due_at,updated_by) OUTPUT inserted.id VALUES(@number,@visit,N'Draft',0,@actor,@due,@actor);`)).recordset[0].id);

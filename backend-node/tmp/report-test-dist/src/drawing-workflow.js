...
export function drawingApprovers(ownerId, leaderId, managerId) {
    if (!leaderId || !managerId)
        throw new ApiError(409, "drawing_approvers_missing", "Assign a project Leader and Manager before submitting a drawing.");
...
export async function demandDrawingTask(transaction, projectId, taskId, memberId) {
    const task = await readTask(transaction, taskId);
    if (task.projectId !== projectId)
        throw new ApiError(400, "drawing_task_project_mismatch", "The design task belongs to another project.");
    const project = await readProject(transaction, projectId, true);
    if (project.status === "Closed")
        throw new ApiError(409, "project_closed", "A closed project cannot release new drawings.");
    await demandAssignedLeaf(transaction, task, memberId);
}
export async function loadDrawingApprovers(transaction, projectId, ownerId) {
  ...
  `)).recordset[0];
    ...
    for (const id of Object.values(result)) {
      ...
      WHERE u.id=@id AND u.is_active=1 AND u.deleted_at IS NULL;`)).recordset[0];
        if (!eligible)
            throw new ApiError(409, "drawing_signer_ineligible", "A drawing signer is inactive or lacks signing.sign permission.");
    }
    return result;
}
//# sourceMappingURL=drawing-workflow.js.map
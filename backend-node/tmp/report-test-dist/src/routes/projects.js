...
export function registerProjectRoutes(app, config, database, users) {
    ...
    `, (sqlRequest) => {
            ...
            sqlRequest.input("elevated", sql.Bit, isProjectElevated(actor));
    ...
    app.post("/api/v1/projects", async (request, reply) => {
        ...
        const created = await database.transaction(async (transaction) => {
            ...
            people.input("manager_id", sql.BigInt, input.managerId);
            people.input("lead_id", sql.BigInt, input.leadEngineerId);
            const roles = (await people.query(`
      ...
      `)).recordset[0];
            ...
            lookup.input("id", sql.BigInt, input.estimateId);
            const estimate = (await lookup.query(`
      ...
      `)).recordset[0];
            if (!estimate)
                throw new ApiError(422, "estimate_not_approved", "An approved estimate is required to create a project.");
            const inquiryLookup = new sql.Request(transaction);
            inquiryLookup.input("inquiry_id", sql.BigInt, Number(estimate.inquiry_id));
            const inquiry = (await inquiryLookup.query(`
        SELECT end_user_customer_id FROM dbo.inquiries WITH (HOLDLOCK) WHERE id=@inquiry_id AND deleted_at IS NULL;
      `)).recordset[0];
            if (!inquiry)
                throw new ApiError(422, "invalid_reference", "The source inquiry is no longer available.");
            ...
            insert.input("name", sql.NVarChar(300), estimate.project_name);
            insert.input("customer_id", sql.BigInt, Number(estimate.customer_id));
            insert.input("project_type", sql.NVarChar(100), estimate.project_type);
            insert.input("end_user_id", sql.BigInt, endUserId);
            insert.input("manager_id", sql.BigInt, input.managerId);
            insert.input("lead_id", sql.BigInt, input.leadEngineerId);
            insert.input("inquiry_id", sql.BigInt, Number(estimate.inquiry_id));
            insert.input("estimate_id", sql.BigInt, input.estimateId);
            insert.input("po_no", sql.NVarChar(100), input.purchaseOrderNumber);
            insert.input("po_date", sql.Date, input.purchaseOrderDate);
            insert.input("start_date", sql.Date, input.startDate);
            insert.input("target_delivery", sql.Date, input.targetDelivery);
            insert.input("site", sql.NVarChar(300), input.site);
            insert.input("remark", sql.NVarChar(sql.MAX), input.remark);
      ...
      `)).recordset[0];
            ...
            members.input("project_id", sql.BigInt, projectId);
            members.input("manager_id", sql.BigInt, input.managerId);
            members.input("lead_id", sql.BigInt, input.leadEngineerId);
            members.input("actor", sql.BigInt, actor.id);
            await members.query(`
      ...
      `);
            const folders = new sql.Request(transaction);
            folders.input("project_id", sql.BigInt, projectId);
            folders.input("actor", sql.BigInt, actor.id);
            ...
            await insertAudit(transaction, actor.id, "Project", projectId, number, "Created from approved estimate", estimate.estimate_no, { ...input, ...endUser, endUserInheritedFromInquiry: requestedEndUserId === undefined });

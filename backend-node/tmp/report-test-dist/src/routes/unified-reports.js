...
import { CUSTOMER_CONSENT, TEAM_CONSENT, REPORT_TYPES, REPORT_SELECT, draftInput, eligibleReportSigners, reportSignerCapabilities, parseCustomerEvidence, readReport, reportAccess, reportDto, reportElevated, reportHash, reportSnapshot, reportSource, reportVersion, reportPermissions, signReport, validateReportSource, validateCustomerPng, validateReportForSubmission } from '../unified-report-service.js';
const sections = { INSTALLATION: ['hardware', 'software', 'commissioning'], UAT: ['scenarios', 'steps', 'punchlist', 'summary'], SERVICE: ['hardware', 'software', 'issues', 'verification'], INSPECTION: ['checkpoints', 'correctiveActions'], POC: ['hypothesis', 'criteria', 'baseline', 'trial', 'result', 'limitations'] };
export function registerUnifiedReportRoutes(app, db, users) {
    const actorFor = async (request, permission = 'report.read') => { await users.demandPermission(request, permission); return users.required(request); };
    const requireSourcePermission = async (request, r) => users.demandPermission(request, r.project_id ? 'project.read' : 'inquiry.read');
    const detail = async (tx, id, request) => reportDto(tx, await readReport(tx, id), await users.required(request));
    ...
    app.get('/api/v1/reports/workspace/people', async (request) => {
        const actor = await actorFor(request), b = request.query, kind = requiredText(b.sourceKind, 20, 'Source'), sourceId = positiveLong(b.sourceId, 'Source');
        if (!['PROJECT', 'INQUIRY'].includes(kind))
            throw new ApiError(400, 'report_source', 'Select Inquiry or Project.');
        await users.demandPermission(request, kind === 'PROJECT' ? 'project.read' : 'inquiry.read');
        return db.transaction(async (tx) => {
            await reportSource(tx, actor, kind, sourceId);
            const rows = (await new sql.Request(tx).query('SELECT u.id,u.name,r.code role FROM dbo.users u JOIN dbo.roles r ON r.id=u.role_id WHERE u.is_active=1 AND u.deleted_at IS NULL ORDER BY u.name')).recordset;
            const items = [];
            for (const row of rows) {
                const p = await reportPermissions(tx, Number(row.id));
                if (!p.has('report.review') && !p.has('report.approve'))
                    continue;
                try {
                    await reportSource(tx, { id: Number(row.id), role: row.role }, kind, sourceId);
                }
                catch (error) {
    ...
    app.get('/api/v1/reports/workspace/sources', async (request) => {
        const actor = await actorFor(request), q = request.query, page = clampedInteger(q.page, 1, 1, 100000), pageSize = clampedInteger(q.pageSize, 50, 1, 100), search = String(q.search ?? '').slice(0, 200);
        const can = (permission) => db.query(`SELECT COUNT(*) allowed FROM dbo.users u JOIN dbo.role_permissions rp ON rp.role_id=u.role_id JOIN dbo.permissions p ON p.id=rp.permission_id WHERE u.id=@actor AND p.code=@permission`, s => s.input('actor', sql.BigInt, actor.id).input('permission', sql.NVarChar(100), permission));
        const [iq, pj] = await Promise.all([can('inquiry.read'), can('project.read')]);
        const result = await db.query(`WITH sources AS (
   ...
   SELECT *,COUNT(*) OVER() total FROM sources WHERE reference LIKE N'%'+@search+N'%' OR title LIKE N'%'+@search+N'%' ORDER BY sourceKind,reference OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`, s => s.input('actor', sql.BigInt, actor.id).input('elevated', sql.Bit, reportElevated(actor)).input('iq', sql.Bit, Number(iq.recordset[0]?.allowed) > 0).input('pj', sql.Bit, Number(pj.recordset[0]?.allowed) > 0).input('search', sql.NVarChar(200), search).input('offset', sql.Int, (page - 1) * pageSize).input('limit', sql.Int, pageSize));
    ...
    app.get('/api/v1/reports/workspace', async (request) => {
        const actor = await actorFor(request), b = request.query, page = clampedInteger(b.page, 1, 1, 100000), pageSize = clampedInteger(b.pageSize, 50, 1, 100);
        return db.transaction(async (tx) => {
            const q = new sql.Request(tx);
            q.input('actor', sql.BigInt, actor.id).input('elevated', sql.Bit, reportElevated(actor)).input('search', sql.NVarChar(200), String(b.search ?? '').slice(0, 200)).input('type', sql.NVarChar(20), String(b.reportType ?? '')).input('status', sql.NVarChar(30), String(b.status ?? '')).input('offset', sql.Int, (page - 1) * pageSize).input('limit', sql.Int, pageSize);
   ...
   OR (h.inquiry_id IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.inquiries i WHERE i.id=h.inquiry_id AND i.deleted_at IS NULL AND (@elevated=1 OR i.created_by=@actor OR i.estimate_owner_id=@actor OR r.prepared_by=@actor OR r.reviewer_id=@actor OR r.approver_id=@actor))))`;
            const rows = (await q.query(`${REPORT_SELECT}${predicate} ORDER BY r.updated_at DESC,h.id DESC OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY;SELECT COUNT(*) total FROM dbo.unified_reports h JOIN dbo.unified_report_revisions r ON r.report_id=h.id ${predicate};`));
            const items = [];
            for (const row of rows.recordsets[0])
                items.push(await reportDto(tx, row, actor));
            return { items, page, pageSize, total: Number(rows.recordsets[1][0]?.total ?? 0) };
        }, sql.ISOLATION_LEVEL.READ_COMMITTED);
    });
    app.get('/api/v1/reports/workspace/:id', async (request) => {
        const actor = await actorFor(request), id = positiveLong(request.params.id, 'Report'), query = request.query;
    ...
    app.post('/api/v1/reports/workspace', async (request, reply) => {
        ...
        if (b.templateId == null && b.templateVersion != null)
            throw new ApiError(400, 'report_template_version', 'Template id and version must be supplied together.');
        ...
        const result = await db.transaction(async (tx) => {
            await reportSource(tx, actor, kind, sourceId, true);
            const source = { project_id: kind === 'PROJECT' ? sourceId : null, inquiry_id: kind === 'INQUIRY' ? sourceId : null };
            ...
            if (template) {
                input.bodyJson = template.bodyJson;
                if (b.locale == null)
                    input.locale = template.locale;
            }
            const q = new sql.Request(tx);
            q.input('project', sql.BigInt, source.project_id).input('inquiry', sql.BigInt, source.inquiry_id).input('task', sql.BigInt, scheduleId);
            if (scheduleId && (!source.project_id || !(await q.query('SELECT id FROM dbo.schedule_tasks WHERE id=@task AND project_id=@project AND deleted_at IS NULL')).recordset.length))
                throw new ApiError(400, 'report_task_source', 'Schedule task must belong to this project.');
            ...
            const id = Number((await q.query('INSERT dbo.unified_reports(report_no,report_type,inquiry_id,project_id,schedule_task_id,created_by,template_id,template_version,template_name) OUTPUT inserted.id VALUES(@number,@type,@inquiry,@project,@task,@actor,@template,@templateVersion,@templateName)')).recordset[0].id);
            await insertRevision(tx, id, 0, actor.id, input);
            await insertAudit(tx, actor.id, 'Report', id, number, 'Created', null, { type, sourceKind: kind, sourceId });
            return detail(tx, id, request);
        });
        return reply.status(201).send(result);
    });
    app.put('/api/v1/reports/workspace/:id', async (request) => {
        const actor = await actorFor(request, 'report.write'), id = positiveLong(request.params.id, 'Report'), b = bodyObject(request.body), input = draftInput(b);
        return db.transaction(async (tx) => {
            const r = await readReport(tx, id);
            await requireSourcePermission(request, r);
            await reportAccess(tx, r, actor, true);
            reportVersion(r, b.rowVersion);
            if (r.state !== 'DRAFT' || Number(r.prepared_by) !== actor.id)
                throw new ApiError(409, 'report_draft_owner', 'Only the preparer may edit a draft.');
            await eligibleReportSigners(tx, r, actor.id, input.reviewerId, input.approverId);
            const q = bindDraft(tx, input);
            q.input('revision', sql.BigInt, r.revision_id);
            ...
            await insertAudit(tx, actor.id, 'Report', id, r.report_no, 'Draft updated', null, { revision: r.revision });
            return detail(tx, id, request);
        });
    });
    app.post('/api/v1/reports/workspace/:id/:action', async (request) => {
        const { id: rawId, action } = request.params, id = positiveLong(rawId, 'Report'), b = bodyObject(request.body);
        if (!['submit', 'review', 'approve', 'return', 'revise', 'void', 'customer-link', 'revoke-customer-link'].includes(action))
            throw new ApiError(400, 'report_action', 'Unknown report action.');
        ...
        return db.transaction(async (tx) => {
            ...
            reportVersion(r, b.rowVersion);
            const q = new sql.Request(tx);
            q.input('revision', sql.BigInt, r.revision_id).input('id', sql.BigInt, id).input('note', sql.NVarChar(2000), note);
            let next = r.state;
            if (action === 'submit') {
                if (r.state !== 'DRAFT' || Number(r.prepared_by) !== actor.id)
                    throw new ApiError(409, 'report_transition', 'Only the preparer may submit a draft.');
                ...
                await signReport(tx, db, request, actor, r, 'PREPARE', snapshot, b.consent);
                next = 'SUBMITTED';
                ...
                if (r.state !== 'SUBMITTED' || Number(r.reviewer_id) !== actor.id || Number(r.prepared_by) === actor.id)
                    throw new ApiError(409, 'report_transition', 'Only the assigned reviewer may review this submission.');
                if (b.sign !== false)
                    await signReport(tx, db, request, actor, r, 'REVIEW', r.snapshot_json, b.consent);
                next = 'REVIEWED';
                await q.query('UPDATE dbo.unified_report_revisions SET reviewed_at=SYSUTCDATETIME() WHERE id=@revision');
            }
            else if (action === 'approve') {
                if (r.state !== (r.reviewer_id === null ? 'SUBMITTED' : 'REVIEWED') || Number(r.approver_id) !== actor.id || Number(r.prepared_by) === actor.id)
                    throw new ApiError(409, 'report_transition', 'Only the assigned approver may approve after the required review.');
                await signReport(tx, db, request, actor, r, 'APPROVE', r.snapshot_json, b.consent);
                next = 'APPROVED';
                await q.query('UPDATE dbo.unified_report_revisions SET approved_at=SYSUTCDATETIME() WHERE id=@revision');
            }
                ...
                if (!note || !reviewer && !approver)
                    throw new ApiError(409, 'report_transition', 'Assigned reviewer or approver must provide a reason to request changes.');
                await users.demandPermission(request, reviewer ? 'report.review' : 'report.approve');
                next = 'CHANGES_REQUESTED';
            }
            else if (action === 'revise') {
                if (!['CHANGES_REQUESTED', 'APPROVED', 'COMPLETED', 'VOID'].includes(r.state) || Number(r.prepared_by) !== actor.id || !note)
                    throw new ApiError(409, 'report_revision', 'The preparer must provide a reason to create a new revision. Revoke an active customer link first.');
                ...
                await insertAudit(tx, actor.id, 'Report', id, r.report_no, 'New revision', { revision: r.revision }, { revision: r.current_revision + 1, note });
                return detail(tx, id, request);
            }
            else if (action === 'void') {
                if (['COMPLETED', 'VOID'].includes(r.state) || Number(r.approver_id) !== actor.id || !note)
                    throw new ApiError(409, 'report_void', 'Assigned approver must provide a reason; completed evidence cannot be voided.');
                next = 'VOID';
                ...
                if (r.state !== 'APPROVED' || ![Number(r.prepared_by), Number(r.approver_id)].includes(actor.id))
                    throw new ApiError(409, 'report_customer_link', 'Only the preparer or approver may issue a link for an approved revision.');
                ...
                if (r.state !== 'AWAITING_CUSTOMER' || ![Number(r.prepared_by), Number(r.approver_id)].includes(actor.id))
                    throw new ApiError(409, 'report_customer_link', 'Only the preparer or approver may revoke an unused customer link.');
                await q.query('UPDATE dbo.unified_report_customer_links SET revoked_at=SYSUTCDATETIME() WHERE revision_id=@revision AND consumed_at IS NULL AND revoked_at IS NULL');
                next = 'APPROVED';
            }
            q.input('state', sql.NVarChar(30), next);
            await q.query('UPDATE dbo.unified_report_revisions SET state=@state,decision_note=@note,updated_at=SYSUTCDATETIME() WHERE id=@revision');
            await insertAudit(tx, actor.id, 'Report', id, r.report_no, action, { revision: r.revision, status: r.state }, { status: next, note });
            return detail(tx, id, request);
        });
    });
    // Suppress automatic request logging: the path contains a one-use bearer token.
    const publicOptions = { config: { public: true, rateLimit: { max: 20, timeWindow: '1 minute' } }, logLevel: 'silent' };
    app.get('/api/v1/report-acknowledgments/:token', publicOptions, async (request) => db.transaction(async (tx) => {
        const { r } = await customerRevision(tx, request.params.token);
    ...
    app.post('/api/v1/report-acknowledgments/:token', { ...publicOptions, bodyLimit: 400000 }, async (request) => {
        const input = parseCustomerEvidence(request.body), b = bodyObject(request.body);
        await validateCustomerPng(input.image);
        return db.transaction(async (tx) => {
            const { r, linkId } = await customerRevision(tx, request.params.token);
            if (b.snapshotSha256 !== r.snapshot_sha256)
                throw new ApiError(409, 'report_snapshot_changed', 'Review this exact report before acknowledging it.');
...
async function insertRevision(tx, id, revision, author, input) {
    const q = bindDraft(tx, input);
    q.input('report', sql.BigInt, id).input('rev', sql.Int, revision).input('actor', sql.BigInt, author);
...
async function customerRevision(tx, token) {
    ...
    if (!/^[A-Za-z0-9_-]{43}$/.test(token))
        throw missing();
    const q = new sql.Request(tx);
    q.input('hash', sql.Char(64), reportHash(token));
    // Header-before-link lock order matches internal lifecycle routes.
    const found = (await q.query(`SELECT l.id,r.report_id,r.revision FROM dbo.unified_report_customer_links l WITH(READCOMMITTEDLOCK) JOIN dbo.unified_report_revisions r WITH(READCOMMITTEDLOCK) ON r.id=l.revision_id WHERE l.token_hash=@hash`)).recordset[0];
    if (!found)
        throw missing();
    ...
    if (!valid || r.state !== 'AWAITING_CUSTOMER' || r.revision !== r.current_revision || !r.snapshot_json || reportHash(r.snapshot_json) !== r.snapshot_sha256)
        throw missing();
    return { r, linkId: Number(found.id) };
}
//# sourceMappingURL=unified-reports.js.map
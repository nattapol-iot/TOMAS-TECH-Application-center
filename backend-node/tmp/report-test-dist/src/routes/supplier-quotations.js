...
import { contentTypeFor, deleteStoredFile, DOCUMENT_EXTENSIONS, multipartText, readMultipartUpload, sendStoredFile, storageKey, SUPPLIER_QUOTATION_EXTENSIONS, uploadedFileName, validateFileExtension, writeStoredFile, } from "../document-storage.js";
...
function multipartPositiveId(values, name, optional = false) {
...
function quotation(row) {
    return {
        ...
        receivedDate: dateOnly(row.received_date), validUntil: dateOnly(row.valid_until),
...
export function registerSupplierQuotationRoutes(app, config, database, users) {
    ...
    `, (bind) => {
            ...
            bind.input("supplier", sql.BigInt, supplierId);
            bind.input("status", sql.NVarChar(30), status);
            bind.input("search", sql.NVarChar(200), search);
            bind.input("offset", sql.Int, (page - 1) * pageSize);
    ...
    app.post("/api/v1/supplier-quotations", async (request, reply) => {
        ...
        if (validUntil < receivedDate)
            throw new ApiError(400, "validation_failed", "Valid until cannot be before received date.");
        const currency = multipartText(values, "currency", 3, true).toUpperCase();
        if (!["THB", "JPY", "USD", "EUR"].includes(currency))
            throw new ApiError(400, "validation_failed", "Currency is invalid.");
    ...
    `, (bind) => { bind.input("supplier", sql.BigInt, supplierId); bind.input("inquiry", sql.BigInt, inquiryId); });
        if (!references.recordset[0])
            throw new ApiError(422, "invalid_reference", "Supplier or inquiry was not found.");
        ...
        try {
            const created = await database.transaction(async (transaction) => {
                ...
                validate.input("supplier", sql.BigInt, supplierId);
                validate.input("inquiry", sql.BigInt, inquiryId);
                const supplier = (await validate.query(`
        ...
        `)).recordset[0];
                ...
                insert.input("reference", sql.NVarChar(200), supplierReference);
                insert.input("supplier", sql.BigInt, supplierId);
                insert.input("received", sql.Date, receivedDate);
                insert.input("valid", sql.Date, validUntil);
                insert.input("inquiry", sql.BigInt, inquiryId);
                insert.input("currency", sql.Char(3), currency);
                insert.input("amount", sql.Decimal(19, 4), amount);
                insert.input("file", sql.NVarChar(500), fileName);
        ...
        `)).recordset[0];
                ...
                audit.input("actor", sql.BigInt, actor.id);
                audit.input("id", sql.BigInt, Number(row.id));
                audit.input("number", sql.NVarChar(50), number);
    ...
    app.get("/api/v1/supplier-quotations/:id/content", async (request, reply) => {
        await users.demandPermission(request, "estimate.read");
        const id = positiveLong(request.params.id, "Supplier quotation id");
        const result = await database.query(`SELECT file_name,content_type,storage_key,size_bytes,sha256 FROM dbo.supplier_quotations WHERE id=@id;`, (bind) => bind.input("id", sql.BigInt, id));
        const row = result.recordset[0];
        if (!row)
            throw new ApiError(404, "quotation_not_found", "Supplier quotation not found.");
...
//# sourceMappingURL=supplier-quotations.js.map
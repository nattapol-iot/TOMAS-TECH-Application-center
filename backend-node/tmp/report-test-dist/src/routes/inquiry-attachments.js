...
export function registerInquiryAttachmentRoutes(app, config, database, users) {
    app.post("/api/v1/inquiries/:id/attachments", { config: { rateLimit: DOCUMENT_UPLOAD_RATE_LIMIT } }, async (request, reply) => {
        await users.demandPermission(request, "inquiry.write");
        const actor = await users.required(request);
        const id = positiveLong(request.params.id, "Inquiry id");
        const inquiry = await database.query(`SELECT inquiry_no FROM dbo.inquiries WHERE id=@id AND deleted_at IS NULL;`, (bind) => bind.input("id", sql.BigInt, id));
        ...
        try {
            const created = await database.transaction(async (transaction) => {
                ...
                const current = (await verify.query(`SELECT inquiry_no FROM dbo.inquiries WITH (UPDLOCK,HOLDLOCK) WHERE id=@id AND deleted_at IS NULL;`)).recordset[0];
                if (!current)
                    throw new ApiError(404, "inquiry_not_found", "Inquiry not found.");
    ...
    app.get("/api/v1/inquiries/:id/attachments/:attachmentId/content", { config: { rateLimit: DOCUMENT_DOWNLOAD_RATE_LIMIT } }, async (request, reply) => {
    ...
    `, (bind) => { bind.input("attachment", sql.BigInt, attachmentId); bind.input("inquiry", sql.BigInt, id); });
        ...
        if (!row)
            throw new ApiError(404, "attachment_not_found", "Inquiry attachment not found.");
...
//# sourceMappingURL=inquiry-attachments.js.map
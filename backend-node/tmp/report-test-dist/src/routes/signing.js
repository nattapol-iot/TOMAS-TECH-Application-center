...
import { loadSigningPdf, parsePlacement, validatePage, paintMarks, appendPdfCertificate } from "../signing-pdf.js";
import { DatabaseCommitOutcomeUnknownError } from "../db.js";
import { deleteStoredFile, DOCUMENT_DOWNLOAD_RATE_LIMIT, sendStoredFile, storageKey, writeStoredBuffer, resolveStoragePath, } from "../document-storage.js";
...
import { buildSignatureCertificate } from "../signing-certificate.js";
import { activeSpecimenId, appendSignEvent, clientIp, clientUserAgent, demandDocumentClass, demandDocumentScope, demandLocale, demandOwnerOrElevated, demandRowVersion, evaluateSigningAssurance, isSha256Hex, isVerifyCodeShape, loadSignableDocument, newVerifyCode, normalizeRevisionLabel, numberPrefix, requireReason, resolveStampAuthority, setDocumentState, verifyChain, } from "../signing-core.js";
 ...
 */
async function loadProjectDocument(database, projectDocumentId, transaction) {
    if (projectDocumentId <= 0)
        throw new ApiError(400, "project_document_required", "A stored document must be named.");
    const statement = `
  ...
  `;
    ...
    if (!row)
        throw new ApiError(404, "project_document_not_found", "The stored document was not found.");
    const sha256 = text(row.sha256);
    if (!isSha256Hex(sha256)) {
        throw new ApiError(409, "content_hash_missing", "This stored document has no recorded SHA-256, so it cannot be frozen for signature. Re-upload it.");
    }
    return {
        id: Number(row.id),
...
async function freezeRevision(transaction, documentId, revisionLabel, source, actorId) {
...
async function liveRequestId(transaction, documentId) {
    const request = new sql.Request(transaction);
...
async function loadActiveTemplate(transaction, documentClass) {
    const request = new sql.Request(transaction);
...
function appliesToAmount(step, amount) {
    if (step.minAmount === null && step.maxAmount === null)
        return true;
    const value = amount ?? Number.MAX_SAFE_INTEGER;
    if (step.minAmount !== null && value < step.minAmount)
        return false;
    if (step.maxAmount !== null && value > step.maxAmount)
        return false;
    return true;
}
async function activateSteps(transaction, requestId, ordered) {
    const request = new sql.Request(transaction);
...
async function mandatoryStepsRemaining(transaction, requestId) {
    const request = new sql.Request(transaction);
    request.input("request_id", sql.BigInt, requestId);
    const row = (await request.query(`
...
async function loadStep(transaction, stepId) {
    const request = new sql.Request(transaction);
  ...
  `)).recordset[0];
    if (!row)
        throw new ApiError(404, "step_not_found", "The signature step was not found.");
...
function demandStepIsMine(step, actor) {
    ...
    if (!mine)
        throw new ApiError(403, "step_not_assigned", "This signature step is not waiting for you.");
}
async function insertMark(transaction, input) {
    const request = new sql.Request(transaction);
    ...
    for (const key of ["x", "y", "width", "height"])
        request.input(key, sql.Decimal(9, 6), input.placement?.[key] ?? null);
    await request.query(`
...
async function closeStep(transaction, stepId, state, decision, reason, actorId) {
    const request = new sql.Request(transaction);
    request.input("step_id", sql.BigInt, stepId);
...
async function setRequestState(transaction, requestId, state) {
    const request = new sql.Request(transaction);
    request.input("request_id", sql.BigInt, requestId);
...
async function closeRequest(transaction, requestId, state, reason) {
    const request = new sql.Request(transaction);
    request.input("request_id", sql.BigInt, requestId);
    request.input("state", sql.NVarChar(30), state);
    request.input("reason", sql.NVarChar(sql.MAX), reason);
...
async function frozenSource(transaction, config, fileId) {
    const query = new sql.Request(transaction);
    query.input("file", sql.BigInt, fileId);
    const file = (await query.query("SELECT storage_key,content_type,sha256 FROM dbo.document_files WHERE id=@file;")).recordset[0];
    if (!file)
        throw new ApiError(404, "file_not_found", "Frozen revision not found.");
    const bytes = await readFile(resolveStoragePath(config.documentStorage, file.storage_key));
    return { ...file, bytes };
}
async function placedArtwork(transaction, config, requestId) {
    const query = new sql.Request(transaction);
    query.input("request", sql.BigInt, requestId);
...
async function produceOutput(transaction, database, config, requestId, document, actor) {
  ...
  `)).recordset[0];
    ...
    const signatures = (await signatureRequest.query(`
  ...
  `)).recordset.map((row) => ({
    ...
    const marksQuery = new sql.Request(transaction);
    marksQuery.input("request_id", sql.BigInt, requestId);
    const artwork = (await marksQuery.query(`
    SELECT s.step_no,
  ...
  `)).recordset;
    for (const art of artwork) {
        ...
        if (!signature)
            continue;
        if (art.signature_key)
            signature.signaturePngBase64 = (await readFile(resolveStoragePath(config.documentStorage, art.signature_key))).toString("base64");
        if (art.stamp_key)
            signature.stampPngBase64 = (await readFile(resolveStoragePath(config.documentStorage, art.stamp_key))).toString("base64");
    }
...
export function registerSigningRoutes(app, config, database, users) {
    // -------------------------------------------------------------------------
    ...
    `, (bind) => {
        ...
        const signedRecently = (await database.query(`
    ...
    app.post("/api/v1/signing/documents", async (request, reply) => {
        ...
        const created = await database.transaction(async (transaction) => {
            ...
            if (taskId !== null)
                await demandDrawingTask(transaction, source.projectId, taskId, actor.id);
      ...
      `)).recordset[0];
            ...
            await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, documentId, documentNo, "Created", null, { documentClass, revisionLabel, sha256: source.sha256, projectId: source.projectId, taskId });
    ...
    app.get("/api/v1/signing/documents/:documentId", async (request) => {
        await users.demandPermission(request, "signing.read");
        const actor = await users.required(request);
        const documentId = positiveLong(request.params.documentId, "Document id");
    ...
    app.post("/api/v1/signing/documents/:documentId/revisions", async (request) => {
        ...
        return database.transaction(async (transaction) => {
            ...
            if (document.documentClass === "DRAWING") {
                if (actor.id !== document.ownerId)
                    throw new ApiError(403, "drawing_member_required", "Only the drawing Member may upload its revision.");
        ...
        `);
            ...
            const fileId = await freezeRevision(transaction, documentId, revisionLabel, source, actor.id);
            await setDocumentState(transaction, documentId, "DRAFT", fileId);
            await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, documentId, document.documentNo, "Revision frozen", null, { revisionLabel, sha256: source.sha256, supersededRequestId: superseded });
    ...
    app.post("/api/v1/signing/documents/:documentId/request", async (request) => {
        ...
        return database.transaction(async (transaction) => {
            ...
            if (drawing) {
                if (actor.id !== document.ownerId)
                    throw new ApiError(403, "drawing_member_required", "Only the drawing Member may submit this drawing.");
                if (!document.scheduleTaskId || !document.projectId)
                    throw new ApiError(409, "drawing_task_required", "Import this drawing from an assigned project task first.");
                await demandDrawingTask(transaction, document.projectId, document.scheduleTaskId, actor.id);
            }
            ...
            if (drawingStampId) {
                ...
                const roleQuery = new sql.Request(transaction);
                roleQuery.input("id", sql.BigInt, approver.id);
                approver.role = (await roleQuery.query(`SELECT r.code FROM dbo.users u JOIN dbo.roles r ON r.id=u.role_id WHERE u.id=@id;`)).recordset[0].code;
                if (!await resolveStampAuthority(transaction, drawingStampId, "DRAWING", approver))
                    throw new ApiError(409, "drawing_stamp_authority_missing", "The project Manager does not hold authority for this stamp.");
            }
            const steps = template.steps.filter((step) => appliesToAmount(step, document.amount)).sort((a, b) => a.stepNo - b.stepNo);
            if (steps.length === 0)
                throw new ApiError(409, "no_applicable_steps", "No step of the active flow applies to this document.");
            ...
            const requestId = Number((await insertRequest.query(`
        DECLARE @created TABLE (id bigint NOT NULL);
      ...
      `)).recordset[0].id);
            ...
            for (const step of steps) {
                stepNo += 1;
                const assigneeUserId = drawingPeople ? drawingPeople[step.blockCode] ?? null : step.assigneeKind === "OWNER"
                    ? document.ownerId
                ...
                if (step.assigneeKind === "PROJECT_MANAGER" && assigneeUserId === null) {
                    throw new ApiError(409, "project_manager_unknown", "This flow assigns a step to the project manager, but the document is not attached to a project.");
                }
                ...
                if (assigneeUserId !== null && step.requiredMark !== "PAPER"
                    ...
                    throw new ApiError(409, "specimen_required", "A signer on this flow has not created their signature yet.", { stepNo, blockCode: step.blockCode, userId: assigneeUserId });
                }
        ...
        `);
            ...
            const frozen = (await fileRequest.query(`
      ...
      `)).recordset[0];
            ...
            await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, documentId, document.documentNo, "Signature requested", null, { requestId, templateVersion: template.version, steps: stepNo });
    ...
    app.get("/api/v1/signing/documents/:documentId/preview", { config: { rateLimit: DOCUMENT_DOWNLOAD_RATE_LIMIT } }, async (request, reply) => {
        await users.demandPermission(request, "signing.read");
        const actor = await users.required(request);
        const id = positiveLong(request.params.documentId, "Document id");
        const query = request.query;
        ...
        const result = await database.transaction(async (transaction) => {
            ...
            check.input("file", sql.BigInt, fileId);
            check.input("document", sql.BigInt, id);
            const file = (await check.query(`SELECT (SELECT TOP(1) id FROM dbo.sign_requests WHERE document_file_id=f.id ORDER BY id DESC) request_id FROM dbo.document_files f WHERE f.id=@file AND f.document_id=@document;`)).recordset[0];
            if (!file)
                throw new ApiError(404, "file_not_found", "Revision not found in this document.");
            ...
            if (stepId) {
                ...
                if (step.documentId !== id || step.fileSha256.toLowerCase() !== source.sha256.toLowerCase() || step.requestId !== Number(file.request_id))
                    throw new ApiError(409, "revision_changed", "Reload the current signing revision.");
                const specimenId = await activeSpecimenId(database, actor.id, transaction);
                if (specimenId !== null) {
                    const imageQuery = new sql.Request(transaction);
                    imageQuery.input("id", sql.BigInt, specimenId);
                    imageQuery.input("initial", sql.Bit, step.requiredMark === "INITIAL");
                    const image = (await imageQuery.query("SELECT COALESCE(CASE WHEN @initial=1 THEN initials_image_key END,image_key) image_key FROM dbo.signature_specimens WHERE id=@id;")).recordset[0];
                ...
                if (step.companyStampId && step.requiredMark === "SIGNATURE_STAMP") {
                    ...
                    if (authority === null)
                        throw new ApiError(403, "stamp_authority_missing", "You cannot preview/apply this stamp without current authority.");
                    const stampQuery = new sql.Request(transaction);
                    stampQuery.input("id", sql.BigInt, step.companyStampId);
                    const stamp = (await stampQuery.query("SELECT image_key FROM dbo.company_stamps WHERE id=@id;")).recordset[0];
    ...
    app.post("/api/v1/signing/steps/:stepId/sign", async (request) => {
        ...
        const stepId = positiveLong(request.params.stepId, "Step id");
        const body = bodyObject(request.body);
        const expected = parseRowVersion(body.rowVersion);
        ...
        try {
            const result = await database.transaction(async (transaction) => {
                ...
                if (placement || stampPlacement) {
                    if (!placement || body.sourceSha256 !== step.fileSha256 || Number(body.previewSpecimenId) !== specimenId)
                        throw new ApiError(409, "preview_changed", "Reload the preview: the frozen file or your signature has changed.");
                    const fileQuery = new sql.Request(transaction);
                    fileQuery.input("request", sql.BigInt, step.requestId);
                    const fileId = Number((await fileQuery.query("SELECT document_file_id FROM dbo.sign_requests WHERE id=@request;")).recordset[0].document_file_id);
                    const source = await frozenSource(transaction, config, fileId);
                    const pdf = await loadSigningPdf(source.bytes, source.content_type, source.sha256);
                    validatePage(pdf, placement);
                    if (stampPlacement)
                        validatePage(pdf, stampPlacement);
                    if (step.requiredMark === "SIGNATURE_STAMP" && !stampPlacement)
                        throw new ApiError(400, "stamp_placement_required", "Place the company stamp as well as your signature.");
                    if (step.requiredMark !== "SIGNATURE_STAMP" && stampPlacement)
                        throw new ApiError(400, "unexpected_stamp", "This step does not permit a company stamp.");
                }
                let stampAuthorityId = null;
                if (step.requiredMark === "SIGNATURE_STAMP") {
                    ...
                    if (stampAuthorityId === null) {
                        throw new ApiError(403, "stamp_authority_missing", "You do not hold a live authority to apply this company stamp to this document class.");
                    }
                }
                ...
                await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, step.documentId, document.documentNo, "Signed", null, { stepId, blockCode: step.blockCode, requiredMark: step.requiredMark, stampAuthorityId, evidence: assurance.evidence, complete });
            ...
            if (writtenKey) {
                if (error instanceof DatabaseCommitOutcomeUnknownError) {
                    request.log.fatal({ err: error.originalError, storageKey: writtenKey }, "Signature certificate commit outcome unknown; preserving storage file");
                }
                else {
     ...
     */
    app.post("/api/v1/signing/steps/:stepId/return", (request) => decideStep(request, "signing.sign", "RETURNED", "VOIDED", "VOIDED", "DRAFT", "STEP_RETURNED", true));
    app.post("/api/v1/signing/steps/:stepId/reject", (request) => decideStep(request, "signing.reject", "REJECTED", "REJECTED", "REJECTED", "REJECTED", "STEP_REJECTED", false));
    async function decideStep(request, permission, decision, stepState, requestState, documentState, action, requireAssignee) {
      ...
      `);
            ...
            await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, step.documentId, document.documentNo, decision, null, { stepId, blockCode: step.blockCode, reason });
    ...
    app.post("/api/v1/signing/steps/:stepId/delegate", async (request) => {
        ...
        if (toUserId === actor.id)
            throw new ApiError(400, "delegate_self", "You cannot delegate a step to yourself.");
        return database.transaction(async (transaction) => {
            const step = await loadStep(transaction, stepId);
            ...
            const candidate = (await check.query(`
        SELECT u.name,
      ...
      `)).recordset[0];
            if (!candidate)
                throw new ApiError(404, "delegate_not_found", "The delegate is not an active user.");
            if (!candidate.can_sign)
                throw new ApiError(409, "delegate_cannot_sign", `${String(candidate.name)} does not hold the signing permission.`);
            if (!candidate.has_specimen)
                throw new ApiError(409, "delegate_specimen_required", `${String(candidate.name)} has not created a signature yet.`);
      ...
      `);
            ...
            await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, step.documentId, document.documentNo, "Delegated", null, { stepId, blockCode: step.blockCode, toUserId, reason });
    ...
    app.post("/api/v1/signing/steps/:stepId/paper", async (request) => {
        ...
        try {
            return await database.transaction(async (transaction) => {
                ...
                await insertAudit(transaction, actor.id, DOCUMENT_ENTITY, step.documentId, document.documentNo, "Paper signature attached", null, { stepId, blockCode: step.blockCode, scanDocumentId: scan.id, complete });
                return { stepId, state: "SIGNED", requestComplete: complete };
            }, sql.ISOLATION_LEVEL.READ_COMMITTED);
        }
        catch (error) {
    ...
    // -------------------------------------------------------------------------
    app.get("/api/v1/signing/requests/:requestId/output", { config: { rateLimit: DOCUMENT_DOWNLOAD_RATE_LIMIT } }, async (request, reply) => {
        await users.demandPermission(request, "signing.read");
        const actor = await users.required(request);
        const requestId = positiveLong(request.params.requestId, "Request id");
      ...
      `, (bind) => bind.input("request_id", sql.BigInt, requestId))).recordset[0];
        if (!row)
            throw new ApiError(404, "output_not_found", "This signature request has not produced a signed output.");
    ...
    `, (bind) => bind.input("code", sql.NVarChar(24), code))).recordset[0];
        if (!row)
            throw new ApiError(404, "verify_code_not_found", "No signed document carries that verification code.");
...
// Shared read models
// ---------------------------------------------------------------------------
async function listDocuments(database, actor, options) {
  ...
  `, bind)).recordset.map((row) => ({
    ...
    if (options.withTotal) {
        const counted = (await database.query(`SELECT COUNT_BIG(*) AS total FROM dbo.signable_documents d ${scope};`, bind)).recordset[0];
        total = Number(counted.total);
    }
    return { items: rows, page, pageSize, total };
}
async function listRequests(database, documentId) {
  ...
  `, (bind) => bind.input("document_id", sql.BigInt, documentId))).recordset;
    ...
    for (const request of requests) {
        details.push({
            ...
            rowVersion: request.row_version.toString("base64"),
        });
    }
    return details;
}
async function listSteps(database, requestId) {
    const marks = new Map();
    for (const mark of (await database.query(`
...
async function listEvents(database, requestId) {
    return (await database.query(`
...
async function loadOutput(database, requestId) {
    const row = (await database.query(`

...
const categories = { "01": "Hardware", "02": "Software", "03": "Electrical", "04": "Mechanical", "05": "Robot", "06": "Engineering", "07": "Outsource", "08": "Transportation", "09": "Accommodation", "10": "Other Cost" };
function decimal(v, minimum, maximum, scale = 4) {
    if (typeof v !== "number" || !Number.isFinite(v) || v < minimum || v > maximum || Math.abs(v * 10 ** scale - Math.round(v * 10 ** scale)) > .001)
        throw new ApiError(400, "invalid_import_number", "Invalid quantity, rate or total in import.");
    return v;
}
export function parseExcelImport(body) {
    ...
    if (!Array.isArray(body.lines) || !body.lines.length || body.lines.length > 1000)
        throw new ApiError(400, "invalid_import_lines", "Import requires 1–1000 rows.");
    const keys = new Set();
    const lines = body.lines.map((raw) => {
        const row = bodyObject(raw);
        const kind = requiredText(row.kind, 20, "Line kind");
        if (!["cost", "manhour", "reference"].includes(kind))
            throw new ApiError(400, "invalid_import_kind", "Invalid import kind.");
        const categoryCode = requiredText(row.categoryCode, 2, "Category");
        if (!categories[categoryCode])
            throw new ApiError(400, "invalid_category", "Invalid category.");
        ...
        if (keys.has(key))
            throw new ApiError(400, "duplicate_import_line", "Duplicate item in the same module.");
        keys.add(key);
        ...
        if (kind === "reference" && unitCost !== 0)
            throw new ApiError(400, "invalid_reference_cost", "Customer supplied reference must have zero cost.");
    ...
    if (Math.abs(total - sourceTotal) > .011)
        throw new ApiError(400, "import_total_mismatch", "Import total does not match source total.");
...
export function registerEstimateExcelImportRoutes(app, database, users) {
    app.get("/api/v1/estimates/:id/excel-imports", async (request) => {
        await users.demandPermission(request, "estimate.read");
        const id = positiveLong(request.params.id, "Estimate");
        const result = await database.query(`SELECT TOP(20) after_json FROM dbo.audit_log WHERE entity_type=N'Estimate' AND entity_id=@id AND action=N'Excel imported' ORDER BY id DESC;`, r => r.input("id", sql.BigInt, id));
    ...
    app.post("/api/v1/estimates/:id/excel-import", async (request, reply) => {
        ...
        const input = parseExcelImport(body);
        ...
        const result = await database.transaction(async (transaction) => {
            ...
            const e = (await lock.query(`SELECT estimate_no,owner_id,revision,status,row_version,due_date FROM dbo.estimates WITH(UPDLOCK,HOLDLOCK) WHERE id=@id AND deleted_at IS NULL;`)).recordset[0];
            if (!e)
                throw new ApiError(404, "estimate_not_found", "Estimate not found.");
            if (Number(e.owner_id) !== actor.id && !["Admin", "Engineering Manager"].includes(actor.role))
                throw new ApiError(403, "import_forbidden", "Only the estimate owner or engineering manager can import a complete workbook.");
            const old = query();
            old.input("id", sql.BigInt, id);
            old.input("revision", sql.Int, e.revision);
            old.input("hash", sql.NVarChar(64), input.sourceHash);
            const imported = (await old.query(`SELECT TOP(1) after_json FROM dbo.audit_log WHERE entity_type=N'Estimate' AND entity_id=@id AND action=N'Excel imported' AND JSON_VALUE(after_json,'$.sourceHash')=@hash AND TRY_CONVERT(int,JSON_VALUE(after_json,'$.revision'))=@revision;`)).recordset[0];
            if (imported)
                return { ...JSON.parse(imported.after_json), alreadyImported: true };
            if (!e.row_version.equals(version))
                throw new ApiError(409, "concurrency_conflict", "Estimate changed. Refresh the preview and retry.");
            if (!["Draft", "Engineering Input", "Revision Required"].includes(e.status))
                throw new ApiError(409, "estimate_locked", "Create a revision before importing into a locked estimate.");
            if (!(await query().query(`SELECT version FROM dbo.schema_versions WHERE version=32;`)).recordset.length)
                throw new ApiError(503, "import_not_ready", "Excel import database update is not installed yet.");
            const owner = query();
            owner.input("owner", sql.BigInt, e.owner_id);
            if (!(await owner.query(`SELECT u.id FROM dbo.users u JOIN dbo.roles r ON r.id=u.role_id WHERE u.id=@owner AND u.is_active=1 AND u.deleted_at IS NULL AND r.code IN(N'Engineer',N'Engineering Manager',N'Admin');`)).recordset.length)
                throw new ApiError(400, "invalid_owner", "Estimate owner must be an active engineer or manager.");
            const created = [];
            for (const l of input.lines) {
                if (l.kind === "reference")
                    continue;
                const cmd = query();
                cmd.input("estimate", sql.BigInt, id);
                cmd.input("revision", sql.Int, e.revision);
                cmd.input("actor", sql.BigInt, actor.id);
                cmd.input("owner", sql.BigInt, e.owner_id);
                cmd.input("module", sql.NVarChar(200), l.module);
                cmd.input("code", sql.NVarChar(100), l.itemCode);
                cmd.input("description", sql.NVarChar(500), l.description);
                cmd.input("qty", sql.Decimal(19, 4), l.quantity);
                cmd.input("unit", sql.NVarChar(50), l.unit);
                cmd.input("rate", sql.Decimal(19, 4), l.unitCost);
                cmd.input("date", sql.Date, input.sourceDate);
                cmd.input("supplier", sql.BigInt, l.supplierId);
                ...
                cmd.input("category", sql.NVarChar(100), categories[l.categoryCode]);
                cmd.input("category_code", sql.Char(2), l.categoryCode);
                cmd.input("brand", sql.NVarChar(100), l.brand);
                cmd.input("model", sql.NVarChar(200), l.model);
                cmd.input("reference", sql.NVarChar(200), input.sourceName.slice(0, 200));
                cmd.input("hours", sql.Decimal(9, 2), input.hoursPerDay);
                cmd.input("department", sql.NVarChar(100), l.department);
                cmd.input("cost_type", sql.NVarChar(30), l.costType);
                ...
                if (!valid)
                    throw new ApiError(400, "invalid_supplier", "Selected supplier is no longer active.");
                ...
                if (duplicates.length)
                    throw new ApiError(409, "duplicate_import_line", `Existing item: ${l.itemCode}. Import into an empty revision or remove duplicates first.`);
                let row;
                if (l.kind === "manhour") {
                    if ((await cmd.query(`SELECT id FROM dbo.manhour_lines WHERE estimate_id=@estimate AND revision=@revision AND deleted_at IS NULL AND package=@module AND activity=@description;`)).recordset.length)
                        throw new ApiError(409, "duplicate_import_line", `Existing activity: ${l.description}.`);
            ...
            for (const section of new Set(input.lines.filter(l => l.kind !== "reference").map(l => l.kind === "manhour" ? "06" : l.categoryCode))) {
                const a = query();
                a.input("id", sql.BigInt, id);
                a.input("section", sql.NVarChar(100), section);
                a.input("owner", sql.BigInt, e.owner_id);
                a.input("due", sql.Date, e.due_date);
...
//# sourceMappingURL=estimate-excel-import.js.map
...
function amount(value, label, minimum) {
...
function parseLines(value) {
    ...
    if (value.length > 200)
        throw new ApiError(400, "validation_failed", "A receipt cannot carry more than 200 lines.");
    const seen = new Set();
    return value.map((raw) => {
        const body = bodyObject(raw);
        const purchaseOrderLineId = requiredInteger(body.purchaseOrderLineId, "Purchase order line", 1);
        if (seen.has(purchaseOrderLineId))
            throw new ApiError(400, "validation_failed", "A purchase order line can appear only once on a receipt.");
        seen.add(purchaseOrderLineId);
        const receivedQuantity = amount(body.receivedQuantity, "Received quantity", 0.0001);
        const acceptedQuantity = amount(body.acceptedQuantity, "Accepted quantity", 0);
        const damagedQuantity = amount(body.damagedQuantity, "Damaged quantity", 0);
        const rejectedQuantity = amount(body.rejectedQuantity, "Rejected quantity", 0);
        if (Math.abs(acceptedQuantity + damagedQuantity + rejectedQuantity - receivedQuantity) > 0.0000001)
            throw new ApiError(400, "validation_failed", "Accepted, damaged and rejected quantity must add up to the received quantity.");
...
async function insertLine(transaction, grnId, poId, line) {
    const lookup = new sql.Request(transaction);
    lookup.input("line", sql.BigInt, line.purchaseOrderLineId);
    lookup.input("po", sql.BigInt, poId);
    const row = (await lookup.query(`SELECT pol.item_id,COALESCE(i.item_code,prl.item_code) item_code,pol.qty,COALESCE(i.location,N'') default_location,
    ...
    WHERE pol.id=@line AND pol.po_id=@po;`)).recordset[0];
    if (!row)
        throw new ApiError(404, "po_line_not_found", `PO line ${line.purchaseOrderLineId} does not belong to this purchase order.`);
    const ordered = Number(row.qty), previous = Number(row.received), itemCode = String(row.item_code);
    if (previous + line.receivedQuantity > ordered) {
        if (!line.allowOverReceipt)
            throw new ApiError(409, "over_receipt", `${itemCode}: ${ordered} ordered and ${previous} already received, so ${line.receivedQuantity} exceeds the order. Approve it with allowOverReceipt and a remark.`, { ordered, previouslyReceived: previous, requested: line.receivedQuantity });
        if (!line.remark)
            throw new ApiError(400, "reason_required", `${itemCode}: an over-receipt needs a remark.`);
    }
    const location = line.location ?? String(row.default_location);
    if (!location)
        throw new ApiError(400, "validation_failed", `${itemCode}: a storage location is required.`);
    const insert = new sql.Request(transaction);
    insert.input("grn", sql.BigInt, grnId);
    insert.input("po", sql.BigInt, poId);
    insert.input("line", sql.BigInt, line.purchaseOrderLineId);
    insert.input("item", sql.BigInt, row.item_id === null ? null : Number(row.item_id));
    insert.input("received", sql.Decimal(19, 4), line.receivedQuantity);
    insert.input("accepted", sql.Decimal(19, 4), line.acceptedQuantity);
    insert.input("damaged", sql.Decimal(19, 4), line.damagedQuantity);
    insert.input("rejected", sql.Decimal(19, 4), line.rejectedQuantity);
    insert.input("lot", sql.NVarChar(100), line.lotNumber);
    insert.input("serial", sql.NVarChar(200), line.serialNumber);
    insert.input("location", sql.NVarChar(100), location);
    insert.input("qc", sql.NVarChar(30), line.qcStatus);
    insert.input("project", sql.BigInt, line.projectAllocationId);
    insert.input("over", sql.Bit, line.allowOverReceipt);
...
async function refreshPo(transaction, poId, actorId) {
    const request = new sql.Request(transaction);
    request.input("po", sql.BigInt, poId);
    request.input("actor", sql.BigInt, actorId);
    return String((await request.query(`DECLARE @outstanding decimal(19,4)=(SELECT COALESCE(SUM(CASE WHEN pol.qty>COALESCE(rc.received_qty,0) THEN pol.qty-COALESCE(rc.received_qty,0) ELSE 0 END),0)
...
export function registerGoodsReceiptRoutes(app, config, database, users) {
    app.get("/api/v1/purchase-orders", async (request) => {
        await users.demandPermission(request, "procurement.read");
        const actor = await users.required(request);
        const q = request.query;
        const projectId = optionalPositiveLong(q.projectId, "Project id"), openOnly = booleanQuery(q.openOnly, false);
        const result = await database.query(`SELECT po.id,po.po_no,po.pr_id,pr.pr_no,po.project_id,p.project_no,po.supplier_id,s.name supplier_name,
    ...
    app.get("/api/v1/purchase-orders/:id", async (request) => {
        await users.demandPermission(request, "procurement.read");
        const actor = await users.required(request);
        const id = positiveLong(request.params.id, "Purchase order id");
        const result = await database.query(`SELECT po.id,po.po_no,po.project_id,p.project_no,po.supplier_id,s.name supplier_name,po.order_date,po.expected_date,po.status,po.row_version,pr.pr_no
      ...
      OUTER APPLY(SELECT SUM(gl.received_qty) received_qty FROM dbo.grn_lines gl INNER JOIN dbo.grns g ON g.id=gl.grn_id AND g.status=N'Confirmed' WHERE gl.po_line_id=pol.id) rc WHERE pol.po_id=@id ORDER BY pol.id;`, (r) => r.input("id", sql.BigInt, id));
        ...
        return { purchaseOrder: { id: Number(h.id), number: h.po_no, projectId: Number(h.project_id), projectNumber: h.project_no, supplierId: Number(h.supplier_id), supplierName: h.supplier_name,
            ...
            lines: (result.recordsets[1] ?? []).map((raw) => {
                const row = raw;
                const ordered = Number(row.qty), received = Number(row.received_qty);
                return { id: Number(row.id), itemId: row.item_id === null ? null : Number(row.item_id),
    ...
    app.get("/api/v1/goods-receipts", async (request) => {
        await users.demandPermission(request, "inventory.read");
        const poId = optionalPositiveLong(request.query.poId, "Purchase order id");
        const result = await database.query(`SELECT g.id,g.grn_no,g.po_id,po.po_no,g.supplier_id,s.name supplier_name,g.delivery_note,g.received_date,g.status,g.confirmed_by,cu.name confirmed_by_name,g.confirmed_at,g.created_by,ou.name created_by_name,
    ...
    app.get("/api/v1/goods-receipts/:id", async (request) => {
        await users.demandPermission(request, "inventory.read");
        const id = positiveLong(request.params.id, "Goods receipt id");
        const result = await database.query(`SELECT g.id,g.grn_no,g.po_id,po.po_no,g.supplier_id,s.name supplier_name,g.delivery_note,g.received_date,g.status,g.created_by,ou.name received_by_name,g.confirmed_by,cu.name confirmed_by_name,g.confirmed_at,g.row_version
      ...
      OUTER APPLY(SELECT SUM(x.received_qty) received_qty FROM dbo.grn_lines x INNER JOIN dbo.grns xg ON xg.id=x.grn_id AND xg.status=N'Confirmed' WHERE x.po_line_id=gl.po_line_id AND x.grn_id<>gl.grn_id) prev WHERE gl.grn_id=@id ORDER BY gl.id;`, (r) => r.input("id", sql.BigInt, id));
        const h = result.recordsets[0]?.[0];
        if (!h)
            throw new ApiError(404, "grn_not_found", "Goods receipt not found.");
        return { goodsReceipt: { id: Number(h.id), number: h.grn_no, purchaseOrderId: Number(h.po_id), purchaseOrderNumber: h.po_no, supplierId: Number(h.supplier_id), supplierName: h.supplier_name,
            ...
            lines: (result.recordsets[1] ?? []).map((raw) => {
                const row = raw;
                const ordered = Number(row.qty), received = Number(row.received_qty), previous = Number(row.previous_received);
                return { id: Number(row.id), purchaseOrderLineId: Number(row.po_line_id), itemId: row.item_id === null ? null : Number(row.item_id), itemCode: row.item_code,
    ...
    app.post("/api/v1/goods-receipts", async (request, reply) => {
        await users.demandPermission(request, "inventory.receive");
        const actor = await users.required(request);
        const body = bodyObject(request.body);
        const poId = requiredInteger(body.purchaseOrderId, "Purchase order", 1);
        const deliveryNote = optionalBodyText(body.deliveryNote, 200, "Delivery note") ?? "";
        const receivedDate = parseDateOnly(body.receivedDate, "Received date", true) ?? todayIn(config.businessTimeZone);
        const lines = parseLines(body.lines);
        const created = await database.transaction(async (transaction) => {
            const lookup = new sql.Request(transaction);
            lookup.input("po", sql.BigInt, poId);
            const po = (await lookup.query(`SELECT supplier_id,status FROM dbo.mat_pos WITH (UPDLOCK,HOLDLOCK) WHERE id=@po AND deleted_at IS NULL;`)).recordset[0];
            if (!po)
                throw new ApiError(404, "po_not_found", "Purchase order not found.");
            if (po.status !== "Ordered" && po.status !== "Partially Received")
                throw new ApiError(409, "po_not_receivable", `Goods cannot be received against a purchase order that is '${po.status}'.`);
            const number = await issueDocumentNumber(transaction, "GRN", todayIn(config.businessTimeZone));
            const insert = new sql.Request(transaction);
            insert.input("number", sql.NVarChar(30), number);
            insert.input("po", sql.BigInt, poId);
            insert.input("supplier", sql.BigInt, Number(po.supplier_id));
            insert.input("note", sql.NVarChar(200), deliveryNote);
            insert.input("date", sql.Date, receivedDate);
            insert.input("actor", sql.BigInt, actor.id);
            const grn = (await insert.query(`INSERT INTO dbo.grns(grn_no,po_id,supplier_id,delivery_note,received_date,status,created_by) OUTPUT inserted.id,inserted.row_version VALUES(@number,@po,@supplier,@note,@date,N'Draft',@actor);`)).recordset[0];
            const id = Number(grn.id);
            for (const line of lines)
                await insertLine(transaction, id, poId, line);
            await insertMaterialAudit(transaction, actor, "Recorded goods receipt", "GRN", id, number, null, { purchaseOrderId: poId, lines: lines.length, status: "Draft" }, { quantity: lines.reduce((sum, line) => sum + line.receivedQuantity, 0), reason: deliveryNote || null });
            return { id, number, status: "Draft", lineCount: lines.length, rowVersion: grn.row_version.toString("base64") };
        });
        return reply.status(201).header("Location", `/api/v1/goods-receipts/${created.id}`).send(created);
    });
    app.post("/api/v1/goods-receipts/:id/confirm", async (request) => {
        await users.demandPermission(request, "inventory.receive");
        const actor = await users.required(request);
        const id = positiveLong(request.params.id, "Goods receipt id");
        const body = bodyObject(request.body);
        ...
        return database.transaction(async (transaction) => {
            ...
            const grn = (await lookup.query(`SELECT grn_no,po_id,status,received_date FROM dbo.grns WITH (UPDLOCK,HOLDLOCK) WHERE id=@id;`)).recordset[0];
            if (!grn)
                throw new ApiError(404, "grn_not_found", "Goods receipt not found.");
            if (grn.status !== "Draft")
                throw new ApiError(409, "grn_not_draft", `Only a draft receipt can be confirmed; this one is '${grn.status}'.`);
            const poId = Number(grn.po_id);
            const projectRequest = new sql.Request(transaction);
            projectRequest.input("po", sql.BigInt, poId);
            const projectId = Number((await projectRequest.query(`SELECT project_id FROM dbo.mat_pos WITH (UPDLOCK,HOLDLOCK) WHERE id=@po AND deleted_at IS NULL;`)).recordset[0].project_id);
            const confirmable = new sql.Request(transaction);
            confirmable.input("grn", sql.BigInt, id);
            const checks = (await confirmable.query(`SELECT gl.po_line_id,COALESCE(i.item_code,prl.item_code) item_code,pol.qty ordered,gl.received_qty requested,
        ...
        gl.allow_over_receipt,gl.remark FROM dbo.grn_lines gl WITH (UPDLOCK,HOLDLOCK) INNER JOIN dbo.mat_po_lines pol WITH (UPDLOCK,HOLDLOCK) ON pol.id=gl.po_line_id INNER JOIN dbo.mat_pr_lines prl ON prl.id=pol.pr_line_id LEFT JOIN dbo.mat_items i ON i.id=gl.item_id WHERE gl.grn_id=@grn ORDER BY gl.po_line_id;`)).recordset;
            for (const row of checks)
                if (Number(row.previous_received) + Number(row.requested) > Number(row.ordered)) {
                    const details = { purchaseOrderLineId: Number(row.po_line_id), ordered: Number(row.ordered), previouslyReceived: Number(row.previous_received), requested: Number(row.requested), resultingReceived: Number(row.previous_received) + Number(row.requested) };
                    if (!row.allow_over_receipt)
                        throw new ApiError(409, "over_receipt", `${row.item_code}: only ${Math.max(0, Number(row.ordered) - Number(row.previous_received))} remains on the order; another confirmed receipt makes this draft exceed the ordered quantity.`, details);
                    if (!row.remark)
                        throw new ApiError(409, "over_receipt_reason_required", `${row.item_code}: the explicitly allowed over-receipt still needs a remark before it can be confirmed.`, details);
                }
            const linesRequest = new sql.Request(transaction);
            linesRequest.input("id", sql.BigInt, id);
            const movements = (await linesRequest.query(`SELECT gl.id,gl.item_id,gl.accepted_qty,gl.damaged_qty+gl.rejected_qty held,gl.location,pol.unit_price FROM dbo.grn_lines gl INNER JOIN dbo.mat_po_lines pol ON pol.id=gl.po_line_id WHERE gl.grn_id=@id ORDER BY gl.id;`)).recordset;
            if (!movements.length)
                throw new ApiError(409, "grn_empty", "This receipt has no lines to confirm.");
            const occurred = grn.received_date instanceof Date ? grn.received_date.toISOString().slice(0, 10) : String(grn.received_date).slice(0, 10);
            for (const move of movements)
                if (move.item_id !== null) {
                    const accepted = Number(move.accepted_qty), held = Number(move.held);
                    const itemId = Number(move.item_id), location = String(move.location), cost = Number(move.unit_price);
                    if (accepted > 0)
                        await appendStockLedger(transaction, `grn:${id}:line:${move.id}:accepted`, "GRN_RECEIPT", itemId, accepted, "stock", location, grn.grn_no, null, cost, actor.id, "Accepted on delivery", occurred);
                    if (held > 0)
                        await appendStockLedger(transaction, `grn:${id}:line:${move.id}:quarantine`, "GRN_QUARANTINE", itemId, held, "quarantine", location, grn.grn_no, null, cost, actor.id, "Damaged or rejected on delivery — held for inspection", occurred);
                }
            const confirm = new sql.Request(transaction);
            confirm.input("actor", sql.BigInt, actor.id);
            confirm.input("id", sql.BigInt, id);
            confirm.input("version", sql.VarBinary(8), version);
            const confirmed = (await confirm.query(`UPDATE dbo.grns SET status=N'Confirmed',confirmed_by=@actor,confirmed_at=SYSUTCDATETIME() OUTPUT inserted.row_version WHERE id=@id AND status=N'Draft' AND row_version=@version;`)).recordset[0];
            if (!confirmed)
                throw new ApiError(409, "concurrency_conflict", "This receipt changed. Reload and try again.");
            const poStatus = await refreshPo(transaction, poId, actor.id);
            const accepted = movements.reduce((sum, x) => sum + Number(x.accepted_qty), 0), quarantined = movements.reduce((sum, x) => sum + Number(x.held), 0);
            await insertMaterialAudit(transaction, actor, "Confirmed goods receipt", "GRN", id, grn.grn_no, { status: "Draft" }, { status: "Confirmed", accepted, quarantined, purchaseOrderStatus: poStatus }, { quantity: accepted + quarantined, projectId, reason: comment });
            const stakeholders = new sql.Request(transaction);
            stakeholders.input("project", sql.BigInt, projectId);
            stakeholders.input("po", sql.BigInt, poId);
            stakeholders.input("actor", sql.BigInt, actor.id);
            const recipientRows = await stakeholders.query(`SELECT DISTINCT recipient_id FROM(SELECT p.manager_id recipient_id FROM dbo.projects p WHERE p.id=@project UNION SELECT p.lead_engineer_id FROM dbo.projects p WHERE p.id=@project UNION SELECT pm.user_id FROM dbo.project_members pm WHERE pm.project_id=@project UNION SELECT pr.requested_by FROM dbo.mat_pos po INNER JOIN dbo.mat_prs pr ON pr.id=po.pr_id WHERE po.id=@po) recipients WHERE recipient_id IS NOT NULL AND recipient_id<>@actor;`);
...
//# sourceMappingURL=goods-receipts.js.map
...
function shift(value, years, days = 0) {
...
function range(query, timeZone) {
    ...
    if (to < from)
        throw new ApiError(400, "validation_failed", "The report end date cannot be before its start date.");
    if (to > shift(from, 10))
        throw new ApiError(400, "validation_failed", "A report range cannot exceed ten years.");
    return { from, to };
}
export function registerReportRoutes(app, config, database, users) {
    app.get("/api/v1/reports/project-cost", async (request) => {
        ...
        const projectId = positiveLong(request.query.projectId, "Project id");
    ...
    `, (sqlRequest) => sqlRequest.input("project_id", sql.BigInt, projectId));
        ...
        const material = Number(row.material_total);
        const actual = Number(row.material_consumed);
        const openPo = Number(row.open_value);
        const reserved = Number(row.reserved_value);
        const openPr = Number(row.open_pr_value);
    ...
    app.get("/api/v1/reports/inventory-value", async (request) => {
        ...
        if (slowMovingDays < 1 || slowMovingDays > 3650)
            throw new ApiError(400, "validation_failed", "Slow-moving days must be between 1 and 3650.");
        if (asOf < shift(today, -20) || asOf > shift(today, 0, 1))
            throw new ApiError(400, "validation_failed", "Inventory valuation date is outside the supported range.");
        const search = optionalText(query.search, 200, "Search");
        const slowOnly = booleanQuery(query.slowMovingOnly, false);
        const result = await database.query(`
    ...
    `, (sqlRequest) => {
            sqlRequest.input("as_of", sql.Date, asOf);
            sqlRequest.input("search", sql.NVarChar(200), search);
            sqlRequest.input("slow_only", sql.Bit, slowOnly);
            sqlRequest.input("slow_days", sql.Int, slowMovingDays);
        });
        const items = result.recordset.map((row) => {
            const usable = Number(row.usable);
            const quarantine = Number(row.quarantine);
    ...
    app.get("/api/v1/reports/supplier-performance", async (request) => {
        ...
        const actor = await users.required(request);
        const query = request.query;
        const dates = range(query, config.businessTimeZone);
    ...
    `, (sqlRequest) => {
            sqlRequest.input("from", sql.Date, dates.from);
            sqlRequest.input("to", sql.Date, dates.to);
    ...
    app.get("/api/v1/reports/pr-cycle-time", async (request) => {
        ...
        const query = request.query;
        const dates = range(query, config.businessTimeZone);
        const projectId = optionalPositiveLong(query.projectId, "Project id");
        if (projectId !== null)
            await demandProjectScope(database, actor, projectId);
    ...
    `, (sqlRequest) => {
            ...
            sqlRequest.input("actor", sql.BigInt, actor.id);
            sqlRequest.input("elevated", sql.Bit, isProjectElevated(actor));
            sqlRequest.input("project_id", sql.BigInt, projectId);
        });
        const stages = result.recordsets[0];
        const lifecycle = result.recordsets[1][0];
        const statuses = result.recordsets[2];
        const decimal = (value) => value === null ? null : Number(value);

...
import { bodyObject, clampedInteger, optionalBodyText, optionalText, parseDateOnly, parseRowVersion, positiveLong, requiredInteger, requiredText, } from "../http.js";
const categoryNames = {
...
function categoryCode(value, label) {
    const code = requiredText(value, 2, label);
    if (!categoryNames[code])
        throw validation(`${label} is not an allowed discipline.`);
    return code;
}
...
function parseLine(value, index) {
    if (typeof value !== "object" || value === null)
        throw validation(`Line ${index + 1} is not an object.`);
    const line = value;
    return {
...
function parseTemplate(request) {
    ...
    if (!Array.isArray(rawLines) || rawLines.length === 0)
        throw validation("A template needs at least one line.");
    if (rawLines.length > MAXIMUM_LINES)
        throw validation(`A template cannot hold more than ${MAXIMUM_LINES} lines.`);
    const status = optionalBodyText(body.status, 20, "Status") ?? "Active";
    if (!templateStatuses.includes(status))
        throw validation("Status is not allowed.");
    if (status === "Retired")
        throw validation("Retire a template through the retire action, not by editing it.");
    return {
...
async function assertSuppliersUsable(transaction, lines) {
    const supplierIds = [...new Set(lines.map((line) => line.supplierId).filter((id) => id !== null))];
  ...
  `)).recordset[0];
    if (Number(row.missing) > 0)
        throw validation("A supplier on one of the lines is inactive or does not exist.");
}
async function replaceLines(transaction, templateId, lines) {
...
export function registerModuleTemplateRoutes(app, database, users) {
    app.get("/api/v1/module-templates", async (request) => {
        ...
        if (status && !templateStatuses.includes(status))
            throw validation("Status is not allowed.");
    ...
    app.get("/api/v1/module-templates/:id", async (request) => {
        await users.demandPermission(request, "estimate.read");
        const id = positiveLong(request.params.id, "Template id");
        const header = await database.query(`${templateProjection} WHERE t.id = @id;`, (sqlRequest) => {
    ...
    app.put("/api/v1/module-templates/:id", async (request) => {
        ...
        return database.transaction(async (transaction) => {
            ...
            const current = (await lock.query(`
      ...
      `)).recordset[0];
            ...
            if (current.status === "Retired")
                throw new ApiError(409, "module_template_retired", "A retired template cannot be edited.");
            await assertSuppliersUsable(transaction, input.lines);
      ...
      `)).recordset[0];
            ...
            await insertAudit(transaction, actor.id, "ModuleTemplate", id, input.code, "Updated", { name: current.name, status: current.status, revision: current.revision }, { name: input.name, status: input.status, revision: updated.revision, lines: input.lines.length });
    ...
    app.post("/api/v1/module-templates/:id/retire", async (request) => {
      ...
      `)).recordset[0];
        ...
        updated_by=@actor,updated_at=SYSUTCDATETIME() WHERE id=@id;`);
            await insertAudit(transaction, actor.id, "ModuleTemplate", id, current.code, "Retired", { status: current.status }, { status: "Retired" });
            return { id, status: "Retired" };
        });
    });
    ...
    app.post("/api/v1/module-templates/from-estimate", async (request, reply) => {
      ...
      `)).recordset;
            if (!source.length)
                throw new ApiError(404, "module_not_found", "That module has no cost lines in the current revision.");
            if (source.length > MAXIMUM_LINES)
                throw validation(`A template cannot hold more than ${MAXIMUM_LINES} lines.`);
...
//# sourceMappingURL=module-templates.js.map
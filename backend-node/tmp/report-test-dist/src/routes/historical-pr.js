...
import { parseHistoricalPr } from "../historical-pr.js";
const normalize = (v) => v.trim().toLowerCase();
const base = "/api/v1/historical-pr";
const publicRow = (r) => ({ id: Number(r.id), projectId: r.project_id == null ? null : Number(r.project_id), documentReference: r.document_reference,
    revision: r.revision, isCurrent: r.is_current, sourceName: r.source_name, createdAt: r.created_at,
    rowVersion: r.row_version.toString("base64"), workbook: JSON.parse(r.payload), links: JSON.parse(r.links) });
export function registerHistoricalPrRoutes(app, database, users) {
    async function scope(actor, row, transaction) {
        if (row.project_id != null)
            await demandProjectScope(database, actor, Number(row.project_id), transaction);
        else if (Number(row.created_by) !== actor.id && !isProjectElevated(actor))
            throw new ApiError(403, "project_scope_forbidden", "ไม่มีสิทธิ์อ่าน PR ย้อนหลังนี้");
    }
    async function ready() {
        const r = await database.query("SELECT CASE WHEN OBJECT_ID(N'dbo.historical_pr_imports') IS NULL THEN 0 ELSE 1 END ready;");
        if (!r.recordset[0]?.ready)
            throw new ApiError(503, "historical_pr_not_ready", "กำลังเตรียมระบบ Import PR ย้อนหลัง กรุณาลองใหม่ภายหลัง");
    }
    async function read(id, transaction) {
        ...
        const r = transaction ? await new sql.Request(transaction).input("id", sql.BigInt, id).query(q)
            : await database.query(q, c => c.input("id", sql.BigInt, id));
        if (!r.recordset[0])
            throw new ApiError(404, "historical_pr_not_found", "ไม่พบ PR ย้อนหลัง");
    ...
    async function parseRequest(request) {
        ...
        if (bytes.toString("base64") !== encoded)
            throw new ApiError(400, "invalid_file", "ข้อมูลไฟล์ไม่สมบูรณ์");
        const workbook = parseHistoricalPr(bytes, sourceName);
        const projects = await database.query("SELECT id,project_no,name FROM dbo.projects WHERE project_no=@number AND deleted_at IS NULL;", c => c.input("number", sql.NVarChar(100), workbook.projectNumber));
        if (projects.recordset.length > 1)
            throw new ApiError(409, "ambiguous_project", "พบ Project มากกว่าหนึ่งรายการ กรุณาตรวจเลข Project");
        const project = projects.recordset[0] ?? { id: null, project_no: workbook.projectNumber, name: workbook.projectName };
        if (project.id != null)
            await demandProjectScope(database, actor, Number(project.id));
        else if (!["Admin", "Purchasing", "Engineering Manager"].includes(actor.role))
            throw new ApiError(409, "project_not_found", "ไม่พบ Project ให้ Admin หรือ Purchasing นำเข้าประวัติรอเชื่อม Project ก่อน");
    ...
    app.get(base, async (request) => {
      ...
      AND (@search=N'' OR h.document_reference LIKE N'%'+@search+N'%' OR h.source_project_number LIKE N'%'+@search+N'%' OR p.name LIKE N'%'+@search+N'%' OR h.payload LIKE N'%'+@search+N'%') ORDER BY h.id DESC;`, c => {
            c.input("actor", sql.BigInt, actor.id);
            c.input("elevated", sql.Bit, isProjectElevated(actor));
            c.input("search", sql.NVarChar(200), search);
        });
        return result.recordset.map(r => {
            const doc = publicRow(r);
            return { id: doc.id, projectId: doc.projectId, projectNumber: r.project_no, projectName: r.name,
    ...
    app.post(`${base}/preview`, { bodyLimit: 12 * 1024 * 1024 }, async (request) => {
        ...
        const existing = await database.query("SELECT id,revision,source_hash FROM dbo.historical_pr_imports WHERE source_project_number=@project AND document_reference=@reference AND is_current=1;", c => {
            c.input("project", sql.NVarChar(100), workbook.projectNumber);
            c.input("reference", sql.NVarChar(200), documentReference);
        });
        const duplicate = await database.query("SELECT id FROM dbo.historical_pr_imports WHERE source_project_number=@project AND source_hash=@hash;", c => {
            c.input("project", sql.NVarChar(100), workbook.projectNumber);
            c.input("hash", sql.Char(64), workbook.sourceHash);
        });
        ...
        if (existing.recordset[0])
            await scope(actor, await read(Number(existing.recordset[0].id)));
        if (duplicate.recordset[0])
            await scope(actor, await read(Number(duplicate.recordset[0].id)));
    ...
    app.post(base, { bodyLimit: 12 * 1024 * 1024 }, async (request, reply) => {
        ...
        const result = await database.transaction(async (transaction) => {
            if (project.id != null)
                await demandProjectScope(database, actor, Number(project.id), transaction);
            ...
            const duplicate = (await q.query("SELECT id FROM dbo.historical_pr_imports WHERE source_project_number=@sourceProject AND source_hash=@hash;")).recordset[0];
            if (duplicate) {
            ...
            const old = (await q.query("SELECT id,revision,created_by,project_id FROM dbo.historical_pr_imports WITH(UPDLOCK,HOLDLOCK) WHERE source_project_number=@sourceProject AND document_reference=@reference AND is_current=1;")).recordset[0];
            if (old)
                await scope(actor, old, transaction);
            if ((old ? Number(old.id) : null) !== expectedId)
                throw new ApiError(409, "preview_changed", "เอกสารเปลี่ยนหลัง Preview กรุณาตรวจไฟล์อีกครั้ง");
            q.input("actor", sql.BigInt, actor.id);
            if (old)
                await q.query("UPDATE dbo.historical_pr_imports SET is_current=0,updated_by=@actor,updated_at=SYSUTCDATETIME() WHERE source_project_number=@sourceProject AND document_reference=@reference AND is_current=1;");
            q.input("revision", sql.Int, (old?.revision ?? 0) + 1);
            q.input("source", sql.NVarChar(500), workbook.sourceName);
            q.input("bytes", sql.VarBinary(sql.MAX), bytes);
            q.input("payload", sql.NVarChar(sql.MAX), JSON.stringify(workbook));
            // New source versions start with no inferred mappings. Old version and links stay inspectable.
            const created = (await q.query(`INSERT dbo.historical_pr_imports(project_id,source_project_number,document_reference,revision,source_name,source_hash,source_bytes,payload,created_by,updated_by)
        OUTPUT inserted.id VALUES(@project,@sourceProject,@reference,@revision,@source,@hash,@bytes,@payload,@actor,@actor);`)).recordset[0];
            await insertAudit(transaction, actor.id, "HistoricalPR", Number(created.id), `HPR-${created.id}`, "Historical PR imported", old ? { previousId: old.id } : null, { projectId: project.id, documentReference, sourceName: workbook.sourceName, sourceHash: workbook.sourceHash, lines: workbook.lines.length, totals: workbook.totals });
    ...
    app.get(`${base}/:id`, async (request) => {
        ...
        const actor = await users.required(request);
        await ready();
        const row = await read(positiveLong(request.params.id, "PR ย้อนหลัง"));
      ...
      JOIN dbo.cost_items c ON c.estimate_id=e.id AND c.revision=e.revision AND c.deleted_at IS NULL WHERE p.id=@project;`, c => c.input("project", sql.BigInt, row.project_id));
        const versions = await database.query("SELECT id,revision,is_current,created_at,project_id,created_by FROM dbo.historical_pr_imports WHERE source_project_number=@project AND document_reference=@reference ORDER BY revision DESC;", c => {
            c.input("project", sql.NVarChar(100), row.source_project_number);
            c.input("reference", sql.NVarChar(200), row.document_reference);
        });
        const visibleVersions = [];
        for (const version of versions.recordset) {
            try {
                await scope(actor, version);
    ...
    app.get(`${base}/:id/source`, async (request, reply) => {
        ...
        const row = await read(positiveLong(request.params.id, "PR ย้อนหลัง"));
        await scope(actor, row);
        const data = await database.query("SELECT source_bytes FROM dbo.historical_pr_imports WHERE id=@id;", c => c.input("id", sql.BigInt, row.id));
    ...
    app.put(`${base}/:id/links`, async (request) => {
        ...
        const actor = await users.required(request);
        await ready();
        const id = positiveLong(request.params.id, "PR ย้อนหลัง"), body = bodyObject(request.body), version = parseRowVersion(body.rowVersion);
        const rawLinks = bodyObject(body.links);
        return database.transaction(async (transaction) => {
            const row = await read(id, transaction);
            await scope(actor, row, transaction);
            if (!row.is_current)
                throw new ApiError(409, "historical_pr_old_version", "แก้การจับคู่ได้เฉพาะไฟล์รุ่นล่าสุด");
            const workbook = JSON.parse(row.payload);
            const valid = new Map(workbook.lines.map(l => [l.key, l]));
            const links = {};
            for (const [key, value] of Object.entries(rawLinks)) {
                const line = valid.get(key);
                if (!line)
                    throw new ApiError(400, "invalid_line", "ไม่พบรายการที่จะจับคู่");
            ...
            JOIN dbo.cost_items c ON c.estimate_id=e.id AND c.revision=e.revision AND c.deleted_at IS NULL WHERE p.id=@project AND c.id=@line;`);
                    if (!check.recordset.length)
                        throw new ApiError(400, "invalid_estimate_line", "Estimate line ต้องอยู่ใน revision ปัจจุบันของ Project นี้");
                }
            ...
            const updated = await new sql.Request(transaction).input("id", sql.BigInt, id).input("links", sql.NVarChar(sql.MAX), JSON.stringify(links))
                .input("actor", sql.BigInt, actor.id).input("version", sql.VarBinary(8), version).query(`
          UPDATE dbo.historical_pr_imports SET links=@links,updated_by=@actor,updated_at=SYSUTCDATETIME() OUTPUT inserted.row_version WHERE id=@id AND is_current=1 AND row_version=@version;`);
            if (!updated.recordset[0])
                throw new ApiError(409, "concurrency_conflict", "ข้อมูลเปลี่ยนแล้ว กรุณาเปิดรายการใหม่ก่อนบันทึก");
    ...
    app.post(`${base}/:id/link-project`, async (request) => {
        ...
        const actor = await users.required(request);
        await ready();
        const id = positiveLong(request.params.id, "PR ย้อนหลัง"), version = parseRowVersion(bodyObject(request.body).rowVersion);
        return database.transaction(async (transaction) => {
            const row = await read(id, transaction);
            await scope(actor, row, transaction);
            ...
            const p = (await q.query("SELECT id FROM dbo.projects WHERE project_no=@number AND deleted_at IS NULL;")).recordset[0];
            if (!p)
                throw new ApiError(409, "project_not_found", `ยังไม่พบ Project ${row.source_project_number} ในระบบ`);
            await demandProjectScope(database, actor, Number(p.id), transaction);
            q.input("project", sql.BigInt, p.id);
            ...
            if (!updated.recordset.length)
                throw new ApiError(409, "concurrency_conflict", "รายการเปลี่ยนแล้วหรือเชื่อม Project แล้ว กรุณาเปิดใหม่");
...
//# sourceMappingURL=historical-pr.js.map
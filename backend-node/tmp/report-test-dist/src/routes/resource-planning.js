...
import { bodyObject, dateOnly, parseDateOnly, parseRowVersion, positiveLong, } from "../http.js";
...
const map = (r) => ({
    ...
    start: dateOnly((r.start_date ?? null)),
    end: dateOnly((r.end_date ?? null)),
...
function decimal(value, max) {
    if (typeof value !== "number" ||
        ...
        throw new ApiError(400, "validation_failed", "Enter a non-negative number with at most two decimals.");
    return value;
}
export function registerResourcePlanningRoutes(app, db, users) {
    ...
    `, (q) => q.input("role", sql.NVarChar(50), actor.role));
        return {
            capacities: result.recordsets[0].map(map),
            efforts: result.recordsets[1].map(map),
            holidays: result.recordsets[2].map((r) => dateOnly(r.holiday_date)),
        };
    });
    app.put("/api/v1/resource-planning/:kind/:id", async (request) => {
        await users.demandPermission(request, "schedule.plan");
        const actor = await users.required(request), params = request.params;
        const id = positiveLong(params.id, "Record id"), body = bodyObject(request.body);
        ...
        if (!capacity)
            await users.demandPermission(request, params.kind === "Inquiry" ? "inquiry.write" : "estimate.write");
        const expected = body.rowVersion == null ? null : parseRowVersion(body.rowVersion);
        const amount = decimal(capacity ? body.daysPerWeek : body.manDays, capacity ? 5 : 100000);
        const start = capacity ? null : parseDateOnly(body.start, "Start date"), end = capacity ? null : parseDateOnly(body.end, "End date");
        if (!capacity &&
            (!start ||
                !end ||
                end < start ||
                Date.parse(end) - Date.parse(start) > 3650 * 86400000))
            throw new ApiError(400, "validation_failed", "Valid start/end dates spanning at most ten years are required.");
        return db.transaction(async (tx) => {
            ...
            const source = capacity
                ...
                : params.kind === "Inquiry"
                    ? "inquiries"
                    : "estimates";
            const ref = (await q.query(`SELECT id FROM dbo.${source} WITH(UPDLOCK,HOLDLOCK) WHERE id=@id AND deleted_at IS NULL${capacity ? " AND is_active=1" : ""};`)).recordset[0];
            if (!ref)
            ...
            const current = (await q.query(`SELECT * FROM dbo.${table} WITH(UPDLOCK,HOLDLOCK) WHERE ${predicate};`)).recordset[0];
            if (current
                ? !expected || !current.row_version.equals(expected)
                : expected !== null)
                throw new ApiError(409, "concurrency_conflict", "Planning data changed. Reload before saving.");
            ...
            const saved = (await q.query(statement)).recordset[0];
            await insertAudit(tx, actor.id, "ResourcePlan", id, params.kind, "Planning updated", current ? map(current) : null, map(saved));
            return map(saved);

...
async function softDelete(transaction, table, estimateId, revision, lineId, expected, actorId) {
    ...
    const row = (await request.query(`DECLARE @removed TABLE(row_version binary(8)); UPDATE ${table} SET deleted_at=SYSUTCDATETIME(),updated_by=@actor${timestamps}
...
export function registerEstimateWorkspaceWriteRoutes(app, config, database, users, email) {
    ...
    app.put("/api/v1/estimates/:id/manhour-lines/:lineId", async (request) => {
        ...
        return database.transaction(async (transaction) => {
            ...
            const rateDate = preserveImported && before.price_date ? new Date(before.price_date).toISOString().slice(0, 10) : today;
            const update = new sql.Request(transaction);
            bindManhour(update, id, estimate.revision, input, rate, rateDate, actor.id);
            update.input("line", sql.BigInt, lineId);
            update.input("version", sql.VarBinary(8), input.lineVersion);
            const row = (await update.query(`DECLARE @updated TABLE(row_version binary(8)); UPDATE dbo.manhour_lines SET package=@package,activity=@activity,department=@department,level=@level,

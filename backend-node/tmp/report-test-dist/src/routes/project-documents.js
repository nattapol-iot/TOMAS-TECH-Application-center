...
async function projectFolder(transaction, database, actor, projectId, folderCode) {
  ...
  `)).recordset[0];
    ...
    if (!row.folder_name)
        throw new ApiError(422, "invalid_folder", "The selected project folder does not exist.");
    return { projectNumber: row.project_no, folderName: row.folder_name };
}
export function registerProjectDocumentRoutes(app, config, database, users) {
    app.get("/api/v1/projects/:projectId/drawing-tasks", async (request) => {
        ...
        return (await database.query(`SELECT t.id,t.name,p.lead_engineer_id leaderId,p.manager_id managerId,
    ...
    app.post("/api/v1/projects/:projectId/documents", { config: { rateLimit: DOCUMENT_UPLOAD_RATE_LIMIT } }, async (request, reply) => {
        const actor = await users.required(request);
        const projectId = positiveLong(request.params.projectId, "Project id");
        await demandProjectScope(database, actor, projectId);
        ...
        if (taskId)
            await database.transaction(tx => demandDrawingTask(tx, projectId, taskId, actor.id), sql.ISOLATION_LEVEL.READ_COMMITTED);
        ...
        if (taskId && (folderCode !== "02" || documentType !== "Drawing"))
            throw new ApiError(400, "drawing_folder_required", "Task import is restricted to the Drawing folder and document type.");
        const remark = multipartText(upload.values, "remark", 20_000);
        if (!/^\d{2}$/.test(folderCode))
            throw new ApiError(400, "invalid_folder", "Folder code must be two digits.");
    ...
    `, (bind) => { bind.input("project", sql.BigInt, projectId); bind.input("folder", sql.Char(2), folderCode); });
        if (!preflight.recordset[0])
            throw new ApiError(404, "project_not_found", "Project not found.");
        if (!preflight.recordset[0].folder_name)
            throw new ApiError(422, "invalid_folder", "The selected project folder does not exist.");
        ...
        try {
            const created = await database.transaction(async (transaction) => {
                ...
                if (taskId)
                    await demandDrawingTask(transaction, projectId, taskId, actor.id);
    ...
    app.get("/api/v1/projects/:projectId/documents/:documentId/content", { config: { rateLimit: DOCUMENT_DOWNLOAD_RATE_LIMIT } }, async (request, reply) => {
        await users.demandPermission(request, "project.read");
        const actor = await users.required(request);
    ...
    `, (bind) => { bind.input("document", sql.BigInt, documentId); bind.input("project", sql.BigInt, projectId); });
        ...
        if (!row)
            throw new ApiError(404, "document_not_found", "Project document not found.");
...
//# sourceMappingURL=project-documents.js.map
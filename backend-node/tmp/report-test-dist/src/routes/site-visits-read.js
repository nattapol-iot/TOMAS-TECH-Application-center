...
import { booleanQuery, clampedInteger, optionalPositiveLong, optionalText, parseDateOnly, positiveLong, } from "../http.js";
import { reportSlaState, SITE_VISIT_PERMISSIONS, } from "../site-visit-common.js";
import { loadVisitDetail, requiredSkillCodes, skillMatch, visitSummary, visitSummaryColumns, visitSummaryFrom, } from "../site-visit-data.js";
...
export function registerSiteVisitReadRoutes(app, _config, database, users) {
    app.get("/api/v1/site-visits/", async (request) => {
        await users.demandPermission(request, SITE_VISIT_PERMISSIONS.visitRead);
        const qv = request.query, page = clampedInteger(qv.page, 1, 1, 1_000_000), pageSize = clampedInteger(qv.pageSize, 25, 1, 100), search = optionalText(qv.search, 200, "Search"), status = optionalText(qv.status, 50, "Status"), customer = optionalPositiveLong(qv.customerId, "Customer"), engineer = optionalPositiveLong(qv.engineerId, "Engineer"), type = optionalPositiveLong(qv.visitTypeId, "Visit type"), from = qv.from ? parseDateOnly(qv.from, "From") : null, to = qv.to ? parseDateOnly(qv.to, "To") : null, unassigned = booleanQuery(qv.unassigned, false), overdue = booleanQuery(qv.reportOverdue, false), archived = booleanQuery(qv.includeArchived, false);
        const result = await database.query(`SELECT ${visitSummaryColumns},COUNT_BIG(*) OVER() total ${visitSummaryFrom} WHERE v.deleted_at IS NULL AND(@archived=1 OR v.archived_at IS NULL) AND(@status IS NULL OR v.status=@status) AND(@customer IS NULL OR i.customer_id=@customer) AND(@type IS NULL OR v.visit_type_id=@type) AND(@engineer IS NULL OR EXISTS(SELECT 1 FROM dbo.site_visit_assignments a WHERE a.visit_id=v.id AND a.is_active=1 AND a.engineer_id=@engineer)) AND(@unassigned=0 OR NOT EXISTS(SELECT 1 FROM dbo.site_visit_assignments a WHERE a.visit_id=v.id AND a.is_active=1)) AND(@overdue=0 OR(v.report_due_at IS NOT NULL AND v.report_due_at<SYSUTCDATETIME() AND(r.status IS NULL OR r.status IN(N'Draft',N'Revision Requested')))) AND(@from IS NULL OR v.scheduled_start>=@from) AND(@to IS NULL OR v.scheduled_start<DATEADD(day,1,CONVERT(datetime2,@to))) AND(@search IS NULL OR v.visit_no LIKE N'%'+@search+N'%' OR i.intake_no LIKE N'%'+@search+N'%' OR i.subject LIKE N'%'+@search+N'%' OR c.name LIKE N'%'+@search+N'%' OR i.site_name LIKE N'%'+@search+N'%') ORDER BY CASE WHEN v.scheduled_start IS NULL THEN 1 ELSE 0 END,v.scheduled_start DESC,v.id DESC OFFSET @offset ROWS FETCH NEXT @take ROWS ONLY;`, (q) => {
    ...
    app.get("/api/v1/site-visits/my-assignments", async (request) => {
        ...
        return result.recordset.map((r) => ({
            ...
            reportSlaState: reportSlaState(r.report_due_at, r.report_status, r.submitted_at),
    ...
    app.get("/api/v1/site-visits/dashboard/engineering", async (request) => {
        await users.demandPermission(request, SITE_VISIT_PERMISSIONS.visitRead);
        const counts = (await database.query(`SELECT(SELECT COUNT_BIG(*) FROM dbo.site_visits v WHERE v.deleted_at IS NULL AND v.status NOT IN(N'Cancelled',N'Closed') AND NOT EXISTS(SELECT 1 FROM dbo.site_visit_assignments a WHERE a.visit_id=v.id AND a.is_active=1)) awaiting,(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL AND status IN(N'Pending Engineer Confirmation',N'Pending Customer Confirmation')) confirmation,(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL AND scheduled_start>=CONVERT(datetimeoffset,CONVERT(date,SYSUTCDATETIME())) AND scheduled_start<DATEADD(day,1,CONVERT(datetimeoffset,CONVERT(date,SYSUTCDATETIME()))) AND status NOT IN(N'Cancelled',N'Closed')) today,(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL AND scheduled_start>=CONVERT(datetimeoffset,CONVERT(date,SYSUTCDATETIME())) AND scheduled_start<DATEADD(day,7,CONVERT(datetimeoffset,CONVERT(date,SYSUTCDATETIME()))) AND status NOT IN(N'Cancelled',N'Closed')) week,(SELECT COUNT_BIG(*) FROM dbo.site_visit_assignments WHERE is_active=1 AND conflict_override=1) conflicts,(SELECT COUNT_BIG(*) FROM dbo.site_visits v LEFT JOIN dbo.site_visit_reports r ON r.visit_id=v.id WHERE v.deleted_at IS NULL AND v.report_due_at<SYSUTCDATETIME() AND(r.status IS NULL OR r.status IN(N'Draft',N'Revision Requested'))) overdue;`)).recordset[0];
        const [workload, demand, attention] = await Promise.all([
            database.query(`SELECT TOP(20)u.name label,COUNT_BIG(*) value FROM dbo.site_visit_assignments a INNER JOIN dbo.users u ON u.id=a.engineer_id INNER JOIN dbo.site_visits v ON v.id=a.visit_id WHERE a.is_active=1 AND v.deleted_at IS NULL AND v.status NOT IN(N'Cancelled',N'Closed') GROUP BY u.name ORDER BY COUNT_BIG(*) DESC,u.name;`),
            database.query(`SELECT TOP(20)k.name_en label,COUNT_BIG(*) value FROM dbo.sales_intake_skills s INNER JOIN dbo.visit_skills k ON k.id=s.skill_id INNER JOIN dbo.sales_intakes i ON i.id=s.intake_id WHERE i.deleted_at IS NULL AND i.status NOT IN(N'Cancelled',N'Closed') GROUP BY k.name_en ORDER BY COUNT_BIG(*) DESC,k.name_en;`),
            database.query(`SELECT TOP(12)${visitSummaryColumns} ${visitSummaryFrom} WHERE v.deleted_at IS NULL AND v.archived_at IS NULL AND(v.status IN(N'Tentative',N'Reschedule Requested',N'Pending Engineer Confirmation') OR(v.report_due_at<SYSUTCDATETIME() AND(r.status IS NULL OR r.status IN(N'Draft',N'Revision Requested')))) ORDER BY CASE WHEN v.report_due_at<SYSUTCDATETIME() THEN 0 ELSE 1 END,v.scheduled_start,v.id;`),
        ]);
        return {
    ...
    app.get("/api/v1/site-visits/dashboard/management", async (request) => {
        ...
        await users.demandPermission(request, SITE_VISIT_PERMISSIONS.visitRead);
        const c = (await database.query(`DECLARE @i bigint=(SELECT COUNT_BIG(*) FROM dbo.sales_intakes WHERE deleted_at IS NULL),@v bigint=(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL); SELECT @i intakes,@v visits,ISNULL((SELECT AVG(CONVERT(decimal(18,2),DATEDIFF(hour,i.created_at,v.customer_confirmed_at))/24.0) FROM dbo.site_visits v INNER JOIN dbo.sales_intakes i ON i.id=v.intake_id WHERE v.deleted_at IS NULL AND v.customer_confirmed_at IS NOT NULL),0) lead,CASE WHEN @v=0 THEN 0 ELSE CONVERT(decimal(9,2),(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL AND status IN(N'Completed',N'Closed'))*100.0/@v) END completion,ISNULL((SELECT CONVERT(decimal(9,2),SUM(CASE WHEN submitted_at<=due_at THEN 1.0 ELSE 0 END)*100.0/NULLIF(COUNT_BIG(*),0)) FROM dbo.site_visit_reports WHERE due_at IS NOT NULL),0) sla,CASE WHEN @v=0 THEN 0 ELSE CONVERT(decimal(9,2),(SELECT COUNT_BIG(DISTINCT source_id) FROM dbo.site_visit_links WHERE source_type=N'SiteVisit' AND target_type=N'Estimate')*100.0/@v) END estimate_rate,CASE WHEN @v=0 THEN 0 ELSE CONVERT(decimal(9,2),(SELECT COUNT_BIG(DISTINCT source_id) FROM dbo.site_visit_links WHERE source_type=N'SiteVisit' AND target_type=N'Project')*100.0/@v) END project_rate,CASE WHEN @v=0 THEN 0 ELSE CONVERT(decimal(9,2),(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL AND status=N'Cancelled')*100.0/@v) END cancelled,CASE WHEN @v=0 THEN 0 ELSE CONVERT(decimal(9,2),(SELECT COUNT_BIG(DISTINCT visit_id) FROM dbo.site_visit_schedule_history WHERE previous_start IS NOT NULL)*100.0/@v) END rescheduled,CASE WHEN @v=0 THEN 0 ELSE CONVERT(decimal(9,2),(SELECT COUNT_BIG(*) FROM dbo.site_visits WHERE deleted_at IS NULL AND status=N'Customer No-show')*100.0/@v) END no_show;`)).recordset[0];
        const queries = [
        ...
        const groups = await Promise.all(queries.map((x) => database.query(x)));
        return {
    ...
    app.get("/api/v1/site-visits/:id", async (request) => {
        await users.demandPermission(request, SITE_VISIT_PERMISSIONS.visitRead);
        const actor = await users.required(request), id = positiveLong(request.params.id, "Visit id"), detail = await loadVisitDetail(database, id, actor.id, actor.role);
        if (!detail)
    ...
    app.get("/api/v1/site-visits/:id/brief", async (request) => {
        ...
        const questions = [
            ...
            ...intake.reviews
                ...
                .slice(0, 1)
                .flatMap((x) => String(x.missingInformation)
                .split(";")
                .map((s) => s.trim())
                .filter(Boolean)),
        ];
    ...
    app.get("/api/v1/site-visits/:id/candidates", async (request) => {
        ...
        for (const r of engineers.recordset) {
            ...
            if (visit.scheduled_start && visit.scheduled_end) {
                const checked = await database.withSession(async (executor) => {
                    ...
                    check.input("travel_minutes_before", sql.Int, Number(visit.travel_minutes_before));
                    check.input("travel_minutes_after", sql.Int, Number(visit.travel_minutes_after));
        ...
        return result.sort((a, b) => b.skillMatchPercent - a.skillMatchPercent ||
            a.conflictCount - b.conflictCount ||
            a.openAssignments - b.openAssignments ||
            String(a.name).localeCompare(String(b.name)));
    });
}
//# sourceMappingURL=site-visits-read.js.map
...
function sectionCode(section) {
    const value = section.trim().slice(0, 2);
    return /^\d{2}$/.test(value) && Number(value) >= 1 && Number(value) <= 10 ? value : null;
}
function expenseSection(expenseType) {
    if (expenseType === "Travel" || expenseType === "Transportation")
        return "08";
    if (expenseType === "Accommodation" || expenseType === "Per Diem")
        return "09";
    if (expenseType === "Equipment Rental" || expenseType === "Other")
        return "10";
    return null;
}
export function registerEstimateWorkspaceReadRoute(app, config, database, users) {
    ...
    `, (sqlRequest) => {
            ...
            sqlRequest.input("role", sql.NVarChar(50), actor.role);
            sqlRequest.input("stale_before", sql.Date, shiftDays(today, -180));
        });
        const headerRow = result.recordsets[0][0];
        if (!headerRow)
        ...
        const header = {
            ...
            revision: number(headerRow.revision), createdDate: dateOnly(headerRow.created_date), dueDate: dateOnly(headerRow.due_date),
            status: headerRow.status, progress: number(headerRow.progress), contingencyRate: number(headerRow.contingency_rate),
            lockedAt: headerRow.locked_at, lockedBy: nullableNumber(headerRow.locked_by), lockedByName: headerRow.locked_by_name,
            createdAt: headerRow.created_at, updatedAt: headerRow.updated_at, rowVersion: headerRow.row_version.toString("base64"),
        ...
        const permissionRow = result.recordsets[1][0] ?? { can_write: false, can_approve: false };
        const assignmentRows = result.recordsets[2];
        ...
        for (const row of assignmentRows)
            if (number(row.owner_id) === actor.id || nullableNumber(row.support_id) === actor.id) {
                const code = sectionCode(String(row.section));
                if (code)
                    assignedSections.add(code);
            }
        const assignments = assignmentRows.map((row) => ({ id: number(row.id), section: row.section, ownerId: number(row.owner_id),
            ...
            dueDate: dateOnly(row.due_date), status: row.status, progress: number(row.progress), comment: row.comment,
            rowVersion: row.row_version.toString("base64"), canEdit: permissionRow.can_write && editable
                && (elevated || number(row.owner_id) === actor.id || nullableNumber(row.support_id) === actor.id) }));
        const costItems = result.recordsets[3].map((row) => ({ id: number(row.id),
            ...
            referenceNumber: row.reference_no, referenceProject: row.reference_project, priceDate: row.price_date ? dateOnly(row.price_date) : null,
            remark: row.remark, ownerId: number(row.owner_id), ownerName: row.owner_name, status: row.status, updatedAt: row.updated_at,
            rowVersion: row.row_version.toString("base64"), canEdit: permissionRow.can_write && editable && (elevated || assignedSections.has(String(row.category_code))) }));
        const manhourLines = result.recordsets[4].map((row) => ({ id: number(row.id), package: row.package,
            ...
            priceDate: row.price_date ? dateOnly(row.price_date) : null, engineers: number(row.engineers), manDays: number(row.man_days),
        ...
        const expenseLines = result.recordsets[5].map((row) => ({ id: number(row.id), package: row.package,
            ...
            rowVersion: row.row_version.toString("base64"), canEdit: permissionRow.can_write && editable && (elevated
        ...
        const otherCostLines = result.recordsets[6].map((row) => ({ id: number(row.id), category: row.category,
            ...
            remark: row.remark, rowVersion: row.row_version.toString("base64"), canEdit: canEditOther }));
        const revisionHistory = result.recordsets[7].map((row) => ({ id: number(row.id), revision: number(row.revision),
        ...
        const validationIssues = result.recordsets[8].map((row) => ({ code: row.code, message: row.message,
...
//# sourceMappingURL=estimate-workspace-read.js.map
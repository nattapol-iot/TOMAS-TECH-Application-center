const AREA_CODES = ["DELIVERY", "QUALITY", "TECHNICAL", "TEAMWORK"];
const SALES_AREA_CODES = ["PIPELINE", "CUSTOMER", "FORECAST", "COMMERCIAL", "HANDOVER"];
...
export function buildPerformanceEvidence(input) {
    ...
    const allTasks = [
        ...input.scheduleTasks.map((task) => ({
            kind: "schedule", id: task.id, sourceId: task.project_id, sourceType: "PROJECT",
        ...
        ...input.resourceTasks.map((task) => ({
            kind: "resource", id: task.id, sourceId: Number(task.project_id ?? task.inquiry_id),
            sourceType: (task.project_id ? "PROJECT" : "INQUIRY"),
...
//# sourceMappingURL=performance-evidence.js.map
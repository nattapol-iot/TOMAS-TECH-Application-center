...
test("placement rejects non-finite, off-page, zero-sized and unknown pages", async () => {
    ...
    for (const invalid of [{ ...valid, x: NaN }, { ...valid, width: 0 }, { ...valid, x: .9 }, { ...valid, y: -.1 }, { ...valid, page: 1.5 }, { ...valid, height: Infinity }, { ...valid, page: 201 }])
        assert.throws(() => parsePlacement(invalid));
    const pdf = await PDFDocument.create();
    pdf.addPage([400, 600]);
    assert.throws(() => validatePage(pdf, { ...valid, page: 2 }));
});
test("display coordinates map to PDF CropBox for every right-angle rotation", () => {
    const box = { x: 30, y: 40, width: 400, height: 600 };
    const rect = { x: 20, y: 50, width: 100, height: 30 };
...
//# sourceMappingURL=signing-pdf.test.js.map
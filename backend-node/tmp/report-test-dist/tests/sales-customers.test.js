...
test("company contact roles preserve Thai, English and Japanese and require an identified person", () => {
    for (const parse of [salesCustomerInput, customerInput]) {
        ...
        assert.equal(input.name, "Test Co., Ltd.");
        assert.equal(input.nameTh, "บริษัท ทดสอบ");
        assert.equal(input.nameJa, "テスト株式会社");
        assert.equal(input.contact, "Somchai Jaidee");
        assert.equal(input.contactNameTh, "สมชาย ใจดี");
        assert.equal(input.contactNameJa, "ソムチャイ");
        assert.equal(input.department, "技術部");
        assert.equal(input.position, "Sales Engineer");
        for (const field of ["department", "position"]) {
            for (const value of [123, {}, "x".repeat(201)])
                assert.throws(() => parse({ code: "ROLE-TEST", name: "Test", contact: "Person", [field]: value }), ApiError);
...
test("sales company validation generates bounded unique codes and requires contact identity", () => {
    ...
    for (const body of [{ name: "" }, { name: "Client", code: "bad code" }, { name: "Client", phone: "123" }, { name: "Client", email: "a@example.com" }, { name: "Client", contact: "Jane", email: "Jane <a@example.com>" }, { name: "Client", site: "x".repeat(301) }]) {
        assert.throws(() => salesCustomerInput(body), (error) => error instanceof ApiError && error.statusCode === 400);
    }
});
test("sales contact validation handles optional fields and rejects malformed values", () => {
    ...
    for (const body of [{ name: "" }, { name: "Jane", email: "bad" }, { name: "Jane", phone: 123 }, { name: "Jane", department: "x".repeat(201) }])
        assert.throws(() => salesContactInput(body));
});
test("permission fallback accepts only explicit permission-denied, never auth or database failures", async () => {
    const calls = [];
    const users = { async demandPermission(_request, permission) { calls.push(permission); if (permission !== "master.write")
            throw new ApiError(403, "permission_denied", "Denied"); } };
    await demandSalesCustomerPermission(users, {}, true);
    ...
    for (const error of [new ApiError(401, "unauthenticated", "Login required"), new Error("Database unavailable")]) {
        let count = 0;
        const failing = { async demandPermission() { count++; throw error; } };
        await assert.rejects(demandSalesCustomerPermission(failing, {}, true), (received) => received === error);
        assert.equal(count, 1);
    }
});
test("HTTP create endpoints reject read-only users before any write or actor lookup", async () => {
    let touchedDatabase = false;
    const database = { async transaction() { touchedDatabase = true; throw new Error("Unexpected database access"); } };
    const users = { async demandPermission() { throw new ApiError(403, "permission_denied", "Denied"); }, async required() { throw new Error("Unexpected actor lookup"); } };
    const app = Fastify();
...
test("Sales intake.write can reach validation without master.write; malformed site and contact writes stop before transaction", async () => {
    let touchedDatabase = false;
    const granted = [];
    const database = { async transaction() { touchedDatabase = true; throw new Error("Unexpected transaction"); } };
    const users = { async demandPermission(_request, permission) { granted.push(permission); assert.equal(permission, "intake.write"); }, async required() { return { id: 1 }; } };
    const app = Fastify();
    registerErrorHandler(app);
    registerSalesCustomerRoutes(app, database, users);
    try {
...
test("contact titles stay separate, optional, bounded and require a named person", () => {
    for (const parse of [salesCustomerInput, customerInput]) {
        ...
        assert.equal(result.contact, "Somchai");
        assert.equal(result.contactTitleTh, "นาย");
        assert.equal(result.contactTitleEn, "Mr.");
        assert.equal(result.contactTitleJa, "様");
        for (const key of ["contactTitleTh", "contactTitleEn", "contactTitleJa"]) {
            for (const value of [123, {}, "x".repeat(51)])
                assert.throws(() => parse({ code: "TITLE", name: "Company", contact: "Somchai", [key]: value }), ApiError);
            assert.throws(() => parse({ code: "TITLE", name: "Company", [key]: "Dr." }), ApiError);
        }
    }
    const contact = salesContactInput({ name: "山田", titleJa: "様" });
    assert.equal(contact.name, "山田");
    assert.equal(contact.titleJa, "様");
});
//# sourceMappingURL=sales-customers.test.js.map
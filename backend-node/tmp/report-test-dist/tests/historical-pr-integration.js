...
const run = (query, db = name) => execFileSync("sqlcmd", [...args, "-d", db, "-Q", query], { stdio: "pipe" });
let database;
...
try {
    ...
    const config = { environment: "development", host: "127.0.0.1", port: 0, allowedHosts: ["localhost"], corsOrigins: [], businessTimeZone: "Asia/Bangkok", auth: { mode: "Development" },
    ...
    registerHistoricalPrRoutes(app, database, new CurrentUserService(database));
    async function api(method, url, payload, expected = 200, actor = "admin") {
    ...
    const bytes = historicalFixture();
    const payload = { sourceName: "TEST-PR.xlsx", fileBase64: bytes.toString("base64"), documentReference: "TEST-REF", expectedCurrentId: null };
    const preview = await api("POST", "/api/v1/historical-pr/preview", payload);
    assert.equal(preview.workbook.lines.length, 3);
    assert.equal(preview.existingId, null);
    ...
    const duplicate = await api("POST", "/api/v1/historical-pr", { ...payload, sourceName: "renamed.xlsx" });
    assert.equal(duplicate.id, created.id);
    assert.equal(duplicate.alreadyImported, true);
    const detail = await api("GET", `/api/v1/historical-pr/${created.id}`);
    assert.equal(detail.workbook.lines[0].status, "Cancelled");
    await api("GET", `/api/v1/historical-pr/${created.id}`, undefined, 403, "outsider");
    const source = await app.inject({ method: "GET", url: `/api/v1/historical-pr/${created.id}/source`, headers: { "x-dev-user-id": "admin" } });
    assert.equal(source.statusCode, 200);
    assert.deepEqual(source.rawPayload, bytes);
    checks++;
    ...
    const old = await api("GET", `/api/v1/historical-pr/${created.id}`);
    assert.equal(old.isCurrent, false);
    assert.equal(old.links["001_PR!9"].replacementKey, "001_PR!10");
    const latest = await api("GET", `/api/v1/historical-pr/${updated.id}`);
    ...
    const unlinkedDetail = await api("GET", `/api/v1/historical-pr/${unlinked.id}`);
    assert.equal(unlinkedDetail.projectId, null);
    ...
    assert.deepEqual(memberDetail.versions.map((v) => v.id), [nextRestricted.id]);
    checks++;
    ...
    await assert.rejects(database.query("UPDATE dbo.historical_pr_imports SET payload=N'{}';"));
    checks++;
...
//# sourceMappingURL=historical-pr-integration.js.map
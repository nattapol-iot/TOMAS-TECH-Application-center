...
function sqlFile(path) { execFileSync("sqlcmd", [...sqlArgs, "-d", name, "-i", path, "-v", `DatabaseName=${name}`], { cwd: repo, stdio: "inherit" }); }
...
try {
    ...
    config.database.applicationRoleName = "drawing_ci_role";
    config.database.applicationRolePassword = rolePassword;
    ...
    const project = Number((await database.query("SELECT id FROM dbo.projects;")).recordset[0].id);
    const task = Number((await database.query("SELECT id FROM dbo.schedule_tasks;")).recordset[0].id);
    async function api(actor, url, body, method = body ? "POST" : "GET", expected = 200) {
    ...
    for (const actor of ["member", "leader", "manager"])
        await api(actor, "/api/v1/me/signature", { source: "UPLOADED", imageBase64: stampImage, initialsBase64: stampImage, initialsText: "TEST" }, "PUT");
    ...
    await api("manager", `/api/v1/master/company-stamps/${stamp.id}/image`, { imageBase64: stampImage, rowVersion: stamps.find((s) => s.id === stamp.id).rowVersion }, "PUT");
    ...
    async function upload(actor, expected = 201) {
    ...
    assert.deepEqual(workspace.liveRequest.steps.map((s) => s.assigneeUserId), [actors["drawing-member"], actors["drawing-leader"], actors["drawing-manager"]]);
    ...
    for (const actor of ["member", "leader", "manager"]) {
        workspace = await detail();
        const pending = workspace.liveRequest.steps.find((s) => s.state === "PENDING");
    ...
    const output = (await database.query("SELECT TOP(1) storage_key FROM dbo.signed_documents ORDER BY id DESC;")).recordset[0];
    ...
    assert.equal((await database.query("SELECT COUNT(*) count FROM dbo.signature_marks WHERE kind=N'STAMP';")).recordset[0].count, 1);
    ...
    for (const actor of ["member", "leader"]) {
        next = await nextDetail();
        const pending = next.liveRequest.steps.find((s) => s.state === "PENDING");
        ...
        assert.ok(preview.specimen.imageBase64);
    ...
    const signedPdf = (await database.query("SELECT TOP(1) storage_key,content_type FROM dbo.signed_documents ORDER BY id DESC;")).recordset[0];
    ...
    const parsed = await PDFDocument.load(readFileSync(signedPath));
    ...
    assert.match(String(download.headers["content-type"]), /application\/pdf/);
    const denied = await app.inject({ method: "GET", url: `/api/v1/signing/requests/${next.liveRequest.id}/output` });
    assert.equal(denied.statusCode, 403); // Development's default identity is unregistered.
    ...
    if (/^IoTTeamCenter_DrawingCI_[a-f0-9]{32}$/.test(name))
...
//# sourceMappingURL=drawing-integration.js.map
...
export function historicalFixture(status = "Cancelled", project = "TEST-PR", pendingCost = 4800) {
    const cell = (r, v, attrs = "") => typeof v === "number" ? `<c r="${r}" ${attrs}><v>${v}</v></c>` : `<c r="${r}" t="inlineStr" ${attrs}><is><t>${v}</t></is></c>`;
    const row = (r, cells) => `<row r="${r}">${cells}</row>`;
    const item = (r, price, po, state, struck = false) => row(r, cell(`B${r}`, "△1") + cell(`E${r}`, `PART-${r}`, struck ? 's="1"' : "") + cell(`F${r}`, "Description") + cell(`L${r}`, 1) + cell(`M${r}`, "Pcs.") + cell(`O${r}`, "SUPPLIER") + cell(`R${r}`, price) + `<c r="S${r}"><f>R${r}*L${r}</f><v>${price}</v></c>` + cell(`T${r}`, "THB") + cell(`U${r}`, price) + cell(`V${r}`, po) + cell(`W${r}`, state));
...
test("cancelled PO keeps demand; issued strike never removes item; totals separate pending", () => {
    ...
    assert.equal(p.lines[0].prIssued, true);
    assert.equal(p.lines[0].status, "Cancelled");
    ...
    assert.equal(p.lines[0].issuedBy, "Original engineer");
    assert.equal(p.lines[0].issuedDate, "2026-05-13");
    assert.equal(p.referenceFromFilename, true);
...
test("struck approved rows remain approved and unknown status never becomes approved", () => {
    ...
    assert.equal(p.totals.unknown, 100);
    assert.equal(p.totals.active, 5000);
});
...
//# sourceMappingURL=historical-pr.test.js.map
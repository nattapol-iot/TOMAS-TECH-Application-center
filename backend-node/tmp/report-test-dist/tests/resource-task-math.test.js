...
test('task duration skips weekends and company holidays while effort stays independent', () => {
    const dates = planDates(plan, new Set(['2026-09-07']));
    assert.deepEqual(dates, { start: '2026-09-04', end: '2026-09-09', calendarDays: 6 });
    const impact = workloadImpact([], plan, 5, new Set(['2026-09-07']));
    assert.equal(impact.weeks.reduce((sum, w) => sum + w.added, 0), 3);
});
...
test('invalid fractional duration, negative effort and malformed date are rejected', () => {
    for (const invalid of [{ ...plan, workDays: 1.5 }, { ...plan, manDays: -1 }, { ...plan, start: 'not-a-date' }, { ...plan, manDays: NaN }])
        assert.throws(() => parseTaskPlan(invalid));
});
test('replacement compares actual before and after rather than dropping the old plan', () => {
    ...
    assert.equal(result.weeks[0].before, 5);
    assert.equal(result.weeks[0].removed, 5);
    assert.equal(result.weeks[0].after, 3);
});
//# sourceMappingURL=resource-task-math.test.js.map
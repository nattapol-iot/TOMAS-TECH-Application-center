...
try{
 ...
 const mgr=await overview('manager');assert.ok(mgr.assessments.length>=3);assert.ok(mgr.assessments.every((r:{frameworkCode:string})=>r.frameworkCode==='SALES'));
 const evidence=await api('sales',`/api/v1/performance/evidence/${ids.sales!.employee}?cycleId=${cycle}`);assert.equal(evidence.frameworkCode,'SALES');assert.ok(evidence.areas.every((a:{suggestedScore:number|null})=>a.suggestedScore===null));
 ...
 const partial=(await overview('sales')).assessments[0];assert.ok(partial.selfScores.every((s:number|null)=>s===null));
 ...
 let visible=(await overview('sales')).assessments[0];assert.ok(visible.managerScores.every((s:number|null)=>s===null));assert.ok(visible.managerEvidence.every((s:string)=>s===''));assert.equal(visible.managerSummary,'');
 ...
 visible=(await overview('sales')).assessments[0];assert.ok(visible.managerScores.every((s:number|null)=>s===4));assert.match(visible.managerSummary,/Private manager rationale/);
 ...
 const privateOwn=(await overview('manager')).assessments.find((r:{userId:number})=>r.userId===ids.manager!.id);assert.equal(privateOwn.managerSummary,'');assert.ok(privateOwn.managerScores.every((s:number|null)=>s===null));

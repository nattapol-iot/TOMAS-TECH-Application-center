...
import type { buildApp as BuildApp } from '../src/app.js';
...
const { buildApp } = await import(process.env.IOT_CONTACT_API_MODULE
  ? pathToFileURL(resolve(process.env.IOT_CONTACT_API_MODULE)).href : '../src/app.js') as { buildApp: typeof BuildApp };
...
try {
  ...
  assert.equal(customer.department, 'เทคโนโลยี / 技術部'); assert.equal(customer.position, 'Senior Engineer');
  assert.equal(customer.name, 'TEST ONLY Alpha'); assert.equal(customer.nameTh, 'บริษัท ทดสอบ อัลฟา'); assert.equal(customer.nameJa, 'テスト・アルファ株式会社');
  ...
  assert.equal(directory.contacts[0].nameTh, 'ผู้ติดต่อทดสอบ'); assert.equal(directory.contacts[0].nameJa, 'テスト担当者');
  ...
  assert.equal(customerRow.position, customer.position); assert.equal(customerRow.department, customer.department);
  assert.equal(customerRow.nameTh, customer.nameTh); assert.equal(customerRow.nameEn, customer.nameEn); assert.equal(customerRow.nameJa, customer.nameJa);
  assert.equal(customerRow.contactNameTh, customer.contactNameTh); assert.equal(customerRow.contactNameJa, customer.contactNameJa);
  ...
  assert.equal(edited.name, masterBody.nameEn); assert.equal(edited.nameTh, masterBody.nameTh); assert.equal(edited.nameJa, masterBody.nameJa);
  assert.equal(edited.contactNameTh, masterBody.contactNameTh); assert.equal(edited.contactNameEn, updatedContact); assert.equal(edited.contactNameJa, masterBody.contactNameJa);
  ...
  assert.equal(directory.primaryContact.titleTh,'ดร.'); assert.equal(directory.primaryContact.titleEn,'Dr.'); assert.equal(directory.primaryContact.titleJa,'先生');
  ...
  assert.equal(directory.primaryContact.name, updatedContact); assert.equal(directory.primaryContact.nameTh, masterBody.contactNameTh); assert.equal(directory.primaryContact.nameJa, masterBody.contactNameJa);
  assert.equal(directory.contacts.find((c: { id: number }) => c.id === added.id).position, 'Engineer');
  assert.equal(directory.contacts.find((c: { id: number }) => c.id === added.id).department, 'QA');
  const legacyBody: Record<string, unknown> = { ...masterBody, name: masterBody.nameEn, contact: updatedContact };
  for (const key of ['contactTitleTh', 'contactTitleEn', 'contactTitleJa', 'department', 'position', 'nameTh', 'nameEn', 'nameJa', 'contactNameTh', 'contactNameEn', 'contactNameJa']) delete legacyBody[key];
  const legacyEdit = await api('admin', `/api/v1/master/customers/${customer.id}`, { ...legacyBody, rowVersion: edited.rowVersion }, 200, 'PUT');
  assert.equal(legacyEdit.contactTitleTh,'ดร.'); assert.equal(legacyEdit.contactTitleEn,'Dr.'); assert.equal(legacyEdit.contactTitleJa,'先生');
  assert.equal(legacyEdit.department, '品質保証'); assert.equal(legacyEdit.position, 'QA Manager');
  assert.equal(legacyEdit.nameTh, masterBody.nameTh); assert.equal(legacyEdit.nameEn, masterBody.nameEn); assert.equal(legacyEdit.nameJa, masterBody.nameJa);
  assert.equal(legacyEdit.contactNameTh, masterBody.contactNameTh); assert.equal(legacyEdit.contactNameEn, updatedContact); assert.equal(legacyEdit.contactNameJa, masterBody.contactNameJa);
  ...
  for (const key of ['nameTh', 'nameEn', 'nameJa', 'contactNameTh', 'contactNameEn', 'contactNameJa'] as const) assert.equal(clearedNames[key], '');
  directory = await api('sales', `${base}/${customer.id}/contacts`);
  assert.equal(directory.primaryContact.name, updatedContact); assert.equal(directory.primaryContact.nameTh, ''); assert.equal(directory.primaryContact.nameEn, ''); assert.equal(directory.primaryContact.nameJa, '');
  ...
  assert.equal(cleared.contactTitleTh,''); assert.equal(cleared.contactTitleEn,''); assert.equal(cleared.contactTitleJa,'');
  ...
  assert.equal(cleared.department, ''); assert.equal(cleared.position, '');
  ...
  const engineer = Number((await database.query<{ id: number }>(`SELECT id FROM dbo.users WHERE entra_object_id=N'sales-ci-reader'`)).recordset[0]!.id);
  ...
  const saved = (await database.query<{ customer_id: number; contact: string }>(`SELECT customer_id,contact FROM dbo.inquiries WHERE id=${Number(inquiry.id)}`)).recordset[0]!;
  ...
  assert.deepEqual(customerRace.map(r => r.statusCode).sort(), [201, 409]); checks += 2;
  ...
  assert.deepEqual(contactRace.map(r => r.statusCode).sort(), [201, 409]); checks += 2;
  ...
  const audit = await database.query<{ total: number }>(`SELECT COUNT(*) AS total FROM dbo.audit_log WHERE entity_type IN(N'Customer',N'CustomerSite',N'CustomerSiteContact')`);
  ...
  const sqlError = error as { stdout?: Buffer; stderr?: Buffer };
  if (sqlError.stdout) console.error(sqlError.stdout.toString());
  if (sqlError.stderr) console.error(sqlError.stderr.toString());
  throw error;
} finally {

...
const name=`IoTTeamCenter_OverheadCI_${randomUUID().replaceAll("-","")}`;
assert.match(name,/^IoTTeamCenter_OverheadCI_[a-f0-9]{32}$/);
const sqlArgs=["-S","localhost","-E","-C","-I","-b"];
const run=(statement:string)=>execFileSync("sqlcmd",[...sqlArgs,"-d",name,"-Q",statement],{cwd:repo,encoding:"utf8"});
const storageRoot=resolve(process.cwd(),"tmp",name);
...
try {
  ...
  execFileSync("sqlcmd",[...sqlArgs,"-d",name,"-i","database/scripts/010_application_login.sql","-v",`DatabaseName=${name}`,"AppLogin=overhead_ci_role"],{cwd:repo,stdio:"pipe"});
  const config:AppConfig={environment:"development",host:"127.0.0.1",port:0,allowedHosts:["localhost"],corsOrigins:["http://localhost:3000"],
    businessTimeZone:"Asia/Bangkok",auth:{mode:"Development"},database:{connectionString:`Server=localhost;Database=${name};Integrated Security=true;TrustServerCertificate=true`,trustServerCertificate:true,applicationRoleName:"overhead_ci_role",applicationRolePassword:rolePassword},
    documentStorage:{mode:"Local",rootPath:storageRoot,maxFileSizeBytes:10_000_000},email:{mode:"Disabled"}};
  ...
  async function estimate(inquiryId:number){
    let item=await api("engineer","POST","/api/v1/estimates",{inquiryId,ownerId:users["overhead-engineer"],contingencyRate:5,dueDate:due},201);

...
test('report source semantics separate delivery reports and pre-sales POC',()=>{
 for(const type of ['INSTALLATION','UAT','SERVICE']) {assert.doesNotThrow(()=>validateReportSource(type,'PROJECT'));assert.throws(()=>validateReportSource(type,'INQUIRY'));}
 assert.doesNotThrow(()=>validateReportSource('POC','INQUIRY'));assert.throws(()=>validateReportSource('POC','PROJECT'));
 for(const kind of ['PROJECT','INQUIRY'])assert.doesNotThrow(()=>validateReportSource('INSPECTION',kind));
...
test('customer acknowledgment requires identity, explicit consent and the correct evidence mode',()=>{
 ...
 for(const change of [{consent:false},{consent:'true'},{name:''},{company:''},{title:''},{mode:'Signature'},{date:'2026-02-30'},{mode:'DRAWN_SIGNATURE',signatureDataUrl:'data:image/svg+xml;base64,PHN2Zz4='}])assert.throws(()=>parseCustomerEvidence({...b,...change}));
});
test('report JSON is bounded and optional reviewer does not invent report facts',()=>{
 const body={scenarios:[],findings:''};assert.deepEqual(JSON.parse(parseReportBody(body)),body);
 assert.throws(()=>parseReportBody([]));assert.throws(()=>parseReportBody({text:'x'.repeat(200001)}));
 const input=draftInput({title:'Test',reportDate:'2026-09-06',body:{},approverId:1});assert.equal(input.reviewerId,null);assert.equal(input.bodyJson,'{}');
});
...
const completeBodies:Record<string,Record<string,unknown>>={
...
test('submission requires common facts and type-specific evidence while draft parsing stays permissive',()=>{
 ...
 for(const [type,body] of Object.entries(completeBodies)) {
  ...
  assert.throws(()=>validateReportForSubmission(type,'{}'),(error:unknown)=>error instanceof ApiError&&error.statusCode===422&&error.code==='report_incomplete');
  for(const change of [{overview:{objective:'',summary:'TEST'}},{context:{start:'2026-09-06T09:00',end:''}},{context:{start:'2026-09-06T10:00',end:'2026-09-06T09:00'}}])assert.throws(()=>validateReportForSubmission(type,JSON.stringify({...body,...change})));
 }
});
test('complete row never hides partially completed UAT/Inspection/POC rows',()=>{
 for(const [type,key] of [['UAT','scenarios'],['INSPECTION','checkpoints'],['POC','trials']] as const) {
  const body=completeBodies[type]!,validRows=body[key] as Record<string,unknown>[];
  assert.doesNotThrow(()=>validateReportForSubmission(type,JSON.stringify({...body,[key]:[...validRows,{}, {remark:'  '}]})));
  assert.throws(()=>validateReportForSubmission(type,JSON.stringify({...body,[key]:[...validRows,{remark:'unfinished'}]})),(error:unknown)=>error instanceof ApiError&&JSON.stringify(error.details).includes(`${key}[1]`));
  for(const field of Object.keys(validRows[0]!))assert.throws(()=>validateReportForSubmission(type,JSON.stringify({...body,[key]:[{...validRows[0],[field]:''}]})));
 }
});
test('structured results accept only recorded PASS, FAIL or PARTIAL and every row must be complete',()=>{
 for(const [type,key] of [['UAT','scenarios'],['INSPECTION','checkpoints']] as const) {
  const body=completeBodies[type]!,row=(body[key] as object[])[0];
  for(const result of ['PASS','FAIL','PARTIAL'])assert.doesNotThrow(()=>validateReportForSubmission(type,JSON.stringify({...body,[key]:[{...row,result}]})));
  for(const result of ['N/A','pending','',null])assert.throws(()=>validateReportForSubmission(type,JSON.stringify({...body,[key]:[{...row,result}]})));
 }
});
...
test('report draft keeps explicitly selected language and rejects UI-language aliases',()=>{
 for(const locale of ['th','en','ja'])assert.equal(draftInput({title:'Localized report',reportDate:'2026-09-06',body:{summary:'Customer original 日本 ไทย'},approverId:2,locale}).locale,locale);
 for(const locale of ['jp','JP','TH'])assert.throws(()=>draftInput({title:'Invalid',reportDate:'2026-09-06',body:{},approverId:2,locale}));
});

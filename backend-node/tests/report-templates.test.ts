...
import { canManageTemplate, safeTemplateBody, templateMetadata, type ReportTemplateRow } from '../src/report-template-service.js';
...
test('all report types retain only reusable definitions and strip source/result/evidence data',()=>{
 ...
 const keys:Record<string,string[]>= {INSTALLATION:['overview','hardware','software','commissioning','deliverables'],UAT:['overview','scenarios','deliverables'],SERVICE:['overview','hardware','software','deliverables'],INSPECTION:['overview','checkpoints','deliverables'],POC:['overview','trials','deliverables']};
 for(const type of Object.keys(keys)) {
  const safe=sanitizeReportTemplate(type,body);assert.deepEqual(Object.keys(safe),keys[type]);
...
test('allowlist does not copy inherited fields or unknown report types',()=>{
 ...
 assert.throws(()=>safeTemplateBody('__proto__',{}));assert.throws(()=>safeTemplateBody('UAT',{scenarios:'not rows'}));
...
test('template metadata accepts empty descriptions and 200-character names',()=>{
 ...
 assert.equal(templateMetadata({name:' Template '}).description,'');assert.throws(()=>templateMetadata({name:'x',description:5}));
});
test('creator/master ownership never substitutes for report.write permission',()=>{
 const row={created_by:7} as ReportTemplateRow,creator={id:7} as CurrentUser,other={id:8} as CurrentUser;

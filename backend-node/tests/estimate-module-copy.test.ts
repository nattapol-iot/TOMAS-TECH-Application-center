...
for(const scenario of ["success","approved","stale","unassigned","collision","empty","partial set","same name","audit failure","metadata failure","total failure"]) {
 test("copy module: "+scenario,async t=>{
  ...
  t.mock.method(sql.Request.prototype,"query",async function(this:sql.Request,s:string){
   ...
   if(s.includes("INSERT dbo.cost_items")){inserts++;assert.equal(this.parameters.target!.value,"Line L");assert.equal(this.parameters.actor!.value,7);assert.equal(this.parameters.source_id!.value,10+inserts);assert.equal(this.parameters.revision!.value,2);assert.match(s,/supplier_id,qty,unit,unit_cost,price_source,reference_no,reference_project,price_date/);assert.match(s,/@set_key,is_price_set,qty_per_set/);assert.match(s,/id=@source_id AND estimate_id=@id AND revision=@revision AND deleted_at IS NULL/);keys.push(this.parameters.set_key!.value);return {recordset:[{id:100+inserts}]};}
  ...
  try{const response=await app.inject({method:"POST",url:"/api/v1/estimates/1/cost-modules/copy",payload:{categoryCode:"01",module:"Line K",targetModule:scenario==="same name"?"Line K":"Line L",estimateRowVersion:version.toString("base64")}});
   assert.equal(response.statusCode,scenario==="success"?200:scenario==="same name"?400:scenario==="unassigned"?403:scenario.endsWith("failure")?500:409,response.body);assert.equal(committed,scenario==="success");
   if(scenario==="success"){assert.equal(response.json().copied,3);assert.equal(inserts,3);assert.equal(mappings,3);assert.equal(metadata,1);assert.equal(audits,1);assert.notEqual(keys[0],set);assert.equal(keys[0],keys[1]);assert.equal(keys[2],null);}else if(!scenario.endsWith("failure"))assert.equal(inserts,0);

...
test("template pagination totals use the same search filter as the rows", async () => {
  ...
  const users = { demandPermission: async () => {} } as unknown as CurrentUserService;
  ...
  try {
    ...
    for (const statement of statements) assert.ok(statement.includes("@search = N'' OR t.code LIKE"));
...
test("template JSON identifiers accept frontend numbers and reject invalid values before SQL", async () => {
  const app = Fastify();
  let transactions = 0;
  const database = { transaction: async () => { transactions++; return { reachedTransaction: true }; } } as unknown as Database;
  const users = { required: async () => ({ id: 1 }), demandPermission: async () => {} } as unknown as CurrentUserService;
  registerModuleTemplateRoutes(app, database, users);

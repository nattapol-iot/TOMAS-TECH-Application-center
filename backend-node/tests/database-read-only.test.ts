...
test("read-only SQL classification rejects database mutations without flagging literals or comments", () => {
  ...
  assert.equal(isReadOnlySql("DECLARE @scope TABLE(id bigint); INSERT @scope SELECT id FROM dbo.projects; SELECT * FROM @scope"), true);
  assert.equal(isReadOnlySql("SELECT * INTO dbo.copy FROM dbo.users"), false);
  assert.equal(isReadOnlySql("INSERT dbo.copy(id) SELECT id FROM dbo.users"), false);

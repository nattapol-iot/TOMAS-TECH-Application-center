...
const sqlTest = createSqlIntegrationConfig("Original");
const name = sqlTest.databaseName;
const args = sqlTest.sqlcmdArgs;
const storageRoot = sqlTest.storageRoot;
const run = (query: string) => execFileSync("sqlcmd", [...args, "-d", name, "-Q", query], sqlTest.sqlcmdOptions);
...
function workbook(description = "TEST ONLY sensor") {
  ...
  return Buffer.from(zipSync({
    ...
    "xl/worksheets/sheet1.xml": strToU8(`<worksheet><sheetData><row r="9">${cell("D9", "Hardware")}${cell("E9", description)}${cell("J9", 50)}${cell("K9", 2)}${cell("L9", 100)}</row><row r="10">${cell("L10", 100, "SUM($L9:$L9)")}</row></sheetData></worksheet>`),
  }));
}
try {
  execFileSync("sqlcmd", [...args, "-i", sqlTest.freshDatabaseScript, "-v", `DatabaseName=${name}`], sqlTest.sqlcmdOptions);
  ...
  application = await buildApp({ environment: "development", host: "127.0.0.1", port: 0, allowedHosts: ["localhost"], corsOrigins: ["http://localhost:3000"], businessTimeZone: "Asia/Bangkok", auth: { mode: "Development" }, database: { connectionString: sqlTest.connectionString, trustServerCertificate: true }, documentStorage: { mode: "Local", rootPath: storageRoot, maxFileSizeBytes: 1000000 }, email: { mode: "Disabled" }, pdfParserUrl: "http://pdf-parser:8000" });
  ...
  const body = { estimateRowVersion: row.row_version.toString("base64"), sourceName: "original.xlsx", sourceDate: "2026-09-10", sourceTotal: 100, hoursPerDay: 8, lines: [{ kind: "cost", categoryCode: "01", itemCode: "XL-Sum-9", module: "TEST ONLY sensor", quantity: 2, unitCost: 50, description: "TEST ONLY sensor", unit: "Lot", source: "Summary cost!9", brand: "", model: "", supplierName: "", remark: "Summary row 9; Unit not stated; Lot retains quoted quantity" }] };
  ...
  const invalidMultipart = Buffer.concat([Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="payload"\r\n\r\n${JSON.stringify(body)}\r\n--${boundary}\r\nContent-Disposition: form-data; name="file"; filename="original.xlsx"\r\nContent-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet\r\n\r\n`), Buffer.from("PK\x03\x04 invalid"), Buffer.from(`\r\n--${boundary}--\r\n`)]);
  const invalid = await app.inject({ ...request, payload: invalidMultipart }); assert.equal(invalid.statusCode, 400, invalid.body); assert.equal(invalid.json().code, "invalid_original_file"); assert.equal(files(storageRoot).length, 1, "invalid workbook must not be staged");
  const changedBody = { ...body, lines: [{ ...body.lines[0], description: "TEST ONLY changed" }] };
  const changedMultipart = Buffer.concat([Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="payload"\r\n\r\n${JSON.stringify(changedBody)}\r\n--${boundary}\r\nContent-Disposition: form-data; name="file"; filename="original.xlsx"\r\nContent-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet\r\n\r\n`), original, Buffer.from(`\r\n--${boundary}--\r\n`)]);
  const stale = await app.inject({ ...request, payload: changedMultipart }); assert.equal(stale.statusCode, 400, stale.body); assert.equal(stale.json().code, "original_preview_mismatch"); assert.equal(files(storageRoot).length, 1, "mismatched preview must not stage another original");
  ...
  execFileSync("sqlcmd", [...args, "-d", "master", "-Q", `IF DB_ID(N'${name}') IS NOT NULL BEGIN ALTER DATABASE [${name}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; DROP DATABASE [${name}]; END`], sqlTest.sqlcmdOptions);
  assert.equal(storageRoot, sqlTest.storageRoot); rmSync(storageRoot, { recursive: true, force: true });
}
